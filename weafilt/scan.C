...
int main(int argc, char *argv[]) {
  ...
  mvector<mvector<int> > histograms(8);
...
// Setup/initialize
  ...
  for (i = 0; i < histograms.xpoints(); i++) {
    histograms[i].resize(65536);
    histograms[i] = 0;
  }
...
// Perform scan -- max, min, average, and histogram
//    plus transferring data to the big data array for further work
  while (!feof(fin) && count < MAXOBS) {
    ...
    //histograms[0][(x.t19v*100+0.5)] ++;
    //histograms[1][(x.t19h*100+0.5)] ++;
    //histograms[2][(x.t22v*100+0.5)] ++;
    //histograms[3][(x.t37v*100+0.5)] ++;
    //histograms[4][(x.t37h*100+0.5)] ++;
    //histograms[5][(x.t85v*100+0.5)] ++;
    //histograms[6][(x.t85h*100+0.5)] ++;
    //histograms[7][(x.icecon*100+0.5)] ++;
...
// Print out the histogram, showing only values that have some obs
//  int sum;
//  for (int i = 0; i < histograms[0].xpoints(); i++) {
//    sum = 0;
//    for (int k = 0; k < histograms.xpoints(); k++) {
//      sum += histograms[k][i] ;
//    }
//    if (sum != 0) {
//      printf("%9d  ",i);
//      for (int k = 0; k < histograms.xpoints(); k++) {
//         printf(" %9d ",histograms[k][i]);
//      }
//      printf("\n");
//    }
//  }
