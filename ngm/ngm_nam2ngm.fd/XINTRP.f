      SUBROUTINE xintrp(Y,X,XBAR,N,AINTRP)
c     FUNCTION xintrp(Y,X,XBAR,N)
...
C$$$
        ...
        DIMENSION Y(40),P(50,50)
        REAL(KIND=K15) X(51),XBAR,R(50),RMIN
C
        DO 333 I = 1,N
          R(I) = X(I) - XBAR
          P(I,1) = Y(I)
c         print *,'in xintrp ',i,n,r(i),x(i),y(i),xbar
333     CONTINUE
        RMIN = 9.E+7
        DO 10 I = 1,N
...
c   J=JS and I=JS
        J=JS
        I=JS
c       print *,'in xintrp1 ',X(I),X(J-1),I,J-1
        P(I,J) = (P(J-1,J-1)*R(I) - R(J-1)*P(I,J-1))/(X(I)-X(J-1))
c   J=JS and I=JE
        J=JS
        I=JE
c       print *,'in xintrp2 ',X(I),X(J-1),I,J-1
        P(I,J) = (P(J-1,J-1)*R(I) - R(J-1)*P(I,J-1))/(X(I)-X(J-1))
c   J=JE and I=JE
        J=JE
        I=JE
c       print *,'in xintrp3 ',X(I),X(J-1),I,J-1
        P(I,J) = (P(J-1,J-1)*R(I) - R(J-1)*P(I,J-1))/(X(I)-X(J-1))
        AINTRP = P(JE,JE)
        RETURN
99      AINTRP = Y(IS)
