      SUBROUTINE BWDIR(NSTA,CSTA,RDATA,KDATA,ID,LPROJ,IMOS,SCALE,REF,
     *           NDATE,MAXSTA,NUMPRJ,IVPRJ,NVPRJ,ICND)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK                                    
C                .      .    .                                       .  
C SUBPROGRAM:    BWDIR       READS WIND FORECASTS AND STORES DIRECTION  
C   PRGMMR: GILBERT          ORG: W/OSD211   DATE: 92-07-20             
C                                                                       
C ABSTRACT: READS WIND DIRECTION FORECASTS.
C           THE FORECASTS ARE STORED IN KDATA(,) FOR
C           LATER PACKING, AND APPROPRIATE SCALING IS DONE.  MISSING    
C           VALUES (9999) ARE INSERTED FOR PROJECTIONS FOR WHICH        
C           FORECASTS ARE NOT VALID OR FOUND.                           
C                                                                       
...
C                                                                       
C        PURPOSE                                                        
C            READS WIND DIRECTION FORECASTS AND CALCULATES THE
C            THE FORECASTS ARE SORED IN KDATA(,) FOR
C            LATER PACKING, AND APPROPRIATE SCALING IS DONE.  MISSING   
C            VALUES (9999) ARE INSERTED FOR PROJECTIONS FOR WHICH       
C            FORECASTS ARE NOT VALID OR FOUND.                          
C                                                                       
...
C                MDATE = DATE FROM MOS FORECAST FILE                    
...
C        CHECK MOS FORECAST DATE WITH RUN DATE                          
 ...
 100        CONTINUE
          ...
          ENDIF
          NUM=NUM+1
C        FOR EACH STATION, DO APPROPRIATE SCALING. 
          DO 200 I=1,NSTA
            IF (RDATA(I).NE.9999..AND.RDATA(I).NE.9997.) THEN
              RTEMP=(RDATA(I)*RMULT)-REF
              KDATA(I,L)=NINT(RTEMP)
            ELSE
              KDATA(I,L)=9999
            ENDIF
 200      CONTINUE
