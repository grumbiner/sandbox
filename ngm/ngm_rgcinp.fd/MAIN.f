...
C   98-11-24  YING LIN REPLACED DAILY USAF ICE/SNOW DEPTH AND WEEKLY 
C             NESDIS/SAB ICE/SNOW WITH THE DAILY NESDIS/IMS ICE/SNOW
C   00-01-19  ERIC ROGERS CONVERTED NUMBER OF GDAS LEVELS BACK TO 42
C   02-04-25  ERIC ROGERS EXTENDED NUMBER OF GUESS LEVELS FROM 42 TO 64
C             WITH IMPLEMENTATION OF T254 GDAS/AVN. THIS IS PARAMETER KS IN
C             SEVERAL PLACES
C             
C
C USAGE:
...
C     UNIT22   -   INSNOICE
C                  NESDIS/IMS SNOW-COVER/SEA-ICE FIELD
C     UNIT23   -   INUSLP
...
C     UNIT43   -   INMSKIMS
C                  NESDIS/IMS LAND/SEA MASK ON THE 1024 X 1024 GRID
C
...
C
C JUANG START
      PARAMETER(IJLB=2000,KS=64,KSP=KS+1)
...
C
C
      CALL W3TAGB('NGM_RGCINP',2000,0040,0098,'NP22')                  
C
...
C
      CALL TASKC (INSNOICE,INSST1DG,INDXST,INUHLWC, INMSKIMS)
C
...
C
      CALL TASKC (INSNOICE,INAFSNO,INSST1DG,INDXST,INUHLWC)
C    1                       INUHLWC,INMSKSAB,INMSKAF)
C
 5050 CONTINUE
C
