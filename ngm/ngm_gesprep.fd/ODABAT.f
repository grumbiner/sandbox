      SUBROUTINE ODABAT(J,TS,PS,ISTO,KLEV)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    ODABAT      PERFORM ADIABATIC CHECK OF TEMPS
...
C   CODE FROM THE ISENTROPIC. SAME AS ADABAT EXCEPT FOR STRIPS
C   OF FIRST-GUESS VALUES.
C
C PROGRAM HISTORY LOG:
C   86-03-31  DIMEGO
C   88-06-30  DIMEGO      HALF PRECISION / PL1 / FTN200
C
C USAGE:    CALL ODABAT(J,TS,PS,ISTO,KLEV)
C   INPUT ARGUMENT LIST:
C     J        - INDEX OF STRIP BEING CHECKED
C     TS       - ARRAY OF TEMPERATURES (GUESS STRIP)
C     PS       - ARRAY OF PRESSURES (GUESS STRIP)
C     ISTO     - SWITCH FOR STORING CORRECTED/ADJUSTED VALUE:
C              - 0 DO NOT STORE ; NON ZERO STORE NEW VALUE
C     KLEV     - NUMBER OF LAYERS IN STRIP ARRAYS
C
...
C$$$
      ...
      INCLUDE "parmanl"
      DIMENSION TS( IMAX , 70 ),PS( IMAX , 70 )
      DIMENSION PT( 70 ),PI( 70 ),NCOUNT( 70 )
      DIMENSION DMEAN( 70 ),RMSTD( 70 ),TDMAX( 70 )
      DIMENSION MAXI( 70 ),MAXJ( 70 )
      DATA NCOUNT,DMEAN/ 70 *0, 70 *0.0/
      DATA RMSTD,TDMAX/ 70 *0.0, 70 *0.0/
      DATA MAXI,MAXJ/ 70 *0, 70 *0/
      KLEVM1= KLEV - 1
      ROCP = 287.05 / 1005.
      XSTO = 0.0
