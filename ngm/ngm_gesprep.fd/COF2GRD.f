...
C   99-01-28  ROGERS
C   99-08-21  ROGERS MADE SPECIAL VERSION FOR NGM GESPREP : GET GLOBAL
C             LAT LON GRID WITH DLON=0.375 DEG, THEN USE EVERY OTHER
C             LATITUDE CIRCLE FROM THAT GRID TO GET 360x120 GRID WHICH
C             MATCHES INPUT TERRAIN
C
...
C
      ...
      INTEGER IWIND
C
      print *,LUN1,NC,KMAX,IWIND,JROMB,
     1      JCAP
C
      POLEIGB = 0.375
C
...
C
      ...
      IEND = ISTART + IMAX - 1
      JSTART = ((ALONVT + 90.0) / POLEIGB) + 1
      JEND = JSTART + (JMAX - 1) * 2
...
C
        DO J = 1, JMAX
          DO I = 1, IMAX
            JINC = JSTART + (J-1) * 2 
            AGRID(I,J) = GRIDGB(ISTART+I-1,JINC)
          ENDDO
        ENDDO
C
      ELSE
...
C
        DO J = 1, JMAX
          DO I = 1, IMAX
            JINC = JSTART + (J-1) * 2 
            AGRID(I,J) = GRIDGB(ISTART+I-1,JINC)
            AGRID2(I,J) = GRIDGB1(ISTART+I-1,JINC)
          ENDDO
        ENDDO
C
      ENDIF
