       SUBROUTINE SWAPVBL(VBLCOPY,IADDRGDP)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    SWAPVBL     SWAP STUFF BETWEEN VBLCOPY AND VBL
C   PRGMMR: PARRISH          ORG: W/NMC22    DATE: 88-08-08
C
C ABSTRACT: EXCHANGE CONTENTS OF VBLCOPY AND VBL ARRAYS.
C
C PROGRAM HISTORY LOG:
C   88-08-08  PARRISH
C
C USAGE:    CALL SWAPVBL(VBLCOPY,IADDRGDP)
...
C
      COMMON /COM1WAY/ ITIMSTEP, ITBOUND, INGLB
         REAL VBLCOPY(1)
         REAL WORKWORK(IIJMAX)
         INTEGER IADDRGDP(5,INGRDUSE)
C--------
         DO 200 NG=1,INGRDUSE
           IJ=IMG(NG)*JMG(NG)
           MADDU=IADDRG(1,NG)-1
           MADDUC=IADDRGDP(1,NG)-1
           LVLUVTQH=4*KM+1
           DO 100 K=1,LVLUVTQH
             DO 50 I=1,IJ
               WORKWORK(I)=VBL(MADDU+I)
               VBL(MADDU+I)=VBLCOPY(MADDUC+I)
               VBLCOPY(MADDUC+I)=WORKWORK(I)
50           CONTINUE
             MADDU=MADDU+IJ
             MADDUC=MADDUC+IJ
100        CONTINUE
