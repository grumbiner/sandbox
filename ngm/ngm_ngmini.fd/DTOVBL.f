       SUBROUTINE DTOVBL(VBLCOPY,NG,IADDRGDP,IPRTDIAG)
C$$$  SUBPROGRAM DOCUMENTATION BLOCK
C                .      .    .                                       .
C SUBPROGRAM:    DTOVBL      MOVE STUFF FROM VBLCOPY TO VBL
C   PRGMMR: PARRISH          ORG: W/NMC22    DATE: 88-08-08
C
C ABSTRACT: MOVE EVERYTHING FROM VBLCOPY ARRAY TO VBL ARRAY.
C   THEN PRINT DIAGNOSTICS FOR DIFFERENCE BETWEEN VBL AND VBLCOPY.
C
C PROGRAM HISTORY LOG:
C   88-08-08  PARRISH
C
C USAGE:    CALL DTOVBL(VBLCOPY,NG,IADDRGDP,IPRTDIAG)
C   INPUT ARGUMENT LIST:
C     VBLCOPY  - CONTAINS STUFF TO BE MOVED
C     NG       - GRID NUMBER FOR VARIABLES TO MOVE.
C     IADDRGDP - ADDRESS ARRAY FOR WORK SPACE IN VBLCOPY.
C     IPRTDIAG - IF NE 0, THEN PRINT DIAGNOSTICS FOR DIFF BETWEEN
C              - VBL AND VBLCOPY (SHOWS CHANGE DUE TO INITIALIZATION)
C
C ATTRIBUTES:
...
C--------
         ...
         DO 100 K=1,LVLUVTQH
           IF(IPRTDIAG.NE.0)
     *       CALL FINALDIA(VBL(MADDU),VBLCOPY(MADDUC),IMG(NG),JMG(NG),
     *                     NG,K,VBL(MADDZ))
           DO 50 I=0,IJ-1
             VBL(MADDU+I)=VBLCOPY(MADDUC+I)
50         CONTINUE
           MADDU=MADDU+IJ
           MADDUC=MADDUC+IJ
100      CONTINUE
