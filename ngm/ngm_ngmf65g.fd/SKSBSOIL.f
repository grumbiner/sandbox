      SUBROUTINE SKSBSOIL ( NPAIR,NG,LADD3,LEN31,ITMAX,LPR )
C
...
C
      INCLUDE 'parmodel'
C...TRANSLATED BY FPP 3.00Z36 11/09/90  15:04:13  
...
7001  CONTINUE
C
CMIC$ DO ALL VECTOR SHARED(LENUPDT, Q1, H1, CG1) PRIVATE(IQ2)
...
C
      IF ( ISUB .EQ. 1 ) THEN
CMIC$ DO ALL VECTOR SHARED(LENUPDT, TG1, THOLD, ASWG) PRIVATE(IQ2)
         DO 5562 IQ2 = 1, LENUPDT
            THOLD(IQ2) = TG1(IQ2)
            ASWG(IQ2) = 0.
5562     CONTINUE
      ELSE
CMIC$ DO ALL VECTOR SHARED(LENUPDT, THOLD, TG1) PRIVATE(IQ2)
         DO 5563 IQ2 = 1, LENUPDT
            TG1(IQ2) = 0.5 * ( THOLD(IQ2) + TG1(IQ2) )
5563     CONTINUE
      END IF
C
777   CONTINUE
