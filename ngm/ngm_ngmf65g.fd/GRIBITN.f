      SUBROUTINE GRIBITN(GRID,IJOUT,PACKED,DESCRIPT,IDGRB1,
     1         IOUTGRIB,LABEL84)
...
C   99-09-02 ERIC ROGERS REMOVED USE OF ON84 LABEL
C     
...
C     START GRIBIT HERE.
C     
c     print *,'in gribit ioutgrib = ',ioutgrib
C
...
c     IF (IDGRB.EQ.255) THEN
c        WRITE(STDOUT,1020) DESCRIPT,LABEL84(1),LABEL84(2)
c1020    FORMAT('GRIBIT:  NO GRIB1 PARAMETER FOR ',A20,
c    X        ' ON84 Q AND S ARE ',2(I3,1X))
c        WRITE(STDOUT,*)'GRIBIT:  DID NOT POST THIS FIELD'
c        RETURN
c     ENDIF
C     
...
c     LABEL84(21)=MOD(IYR4-1,100)+1
c     do k = 1, 25
c       print *,'idgrb1 ',k,idgrb1(k)
c     enddo
c     CALL W3FI32(LABEL84_8,LAB84PAK)
C return full digit of year
c     LABEL84(21)=IYR4
c get century for w3fp12
c     ICENT=(IYR4-1)/100+1
c     IFLAG = CHAR(128)
c     print *,'gribit ',labpak(1),labpak(2),labpak(3),labpak(4)
c    1     labpak(5),labpak(6)
c     CALL W3FP12(LABPAK,IFLAG,PDS,ICENT,IDECI,IER)
C     
C     IF W3FP12 CAN NOT GENERATE A PDS, DO NOT POST THIS FIELD IN GRIB.
c     IF (IER.NE.0) THEN
c        WRITE(STDOUT,1030) IER,DESCRIPT,LABPAK(1)
c1030    FORMAT('GRIBIT:  ***W3FI12 ERROR IER=',I1,
c    X        ' FOR ',A20,' ON84 ',2(Z16,1X))
c        WRITE(STDOUT,*)'GRIBIT:  DID NOT POST THIS FIELD'
c        RETURN
c     ENDIF
C     
...
C     
     ...
     X            NPTS,KBUF,ITOT,IER)
      IF (IER.NE.0) THEN
         WRITE(STDOUT,1040) IER,DESCRIPT,IDGRB1(8)
