...
C              SPECIFY THE UNIT NUMBER OF THE PRINTER.
      ...
      IOUTUPRT = 6
      print *,'start oneway ', itimstep,itbound,inglb
C
...
C
       DO KER=1,KMLB
         print *,'after 1st latbnd read ',ker,itprv,itnxt
         if(ker.eq.1) then
          print *,'after 1st latbnd read ',htnd(689),HNXT(689)
         endif
         print *,'after 1st latbnd read ',hutnd(689,ker),HUNXT(689,ker)
         print *,'after 1st latbnd read ',hvtnd(689,ker),HVNXT(689,ker)
         print *,'after 1st latbnd read ',httnd(689,ker),HTNXT(689,ker)
         print *,'after 1st latbnd read ',hqtnd(689,ker),HQNXT(689,ker)
       ENDDO
C
C           CONSIDER RESTART
C
...
C           NGLB=2          DTLB =ITIMSTEP/2
       ...
       print *,'compute tendency ',itimstep,nglb,dtlb
C
C
...
C           ONE UPDATE TIME STEP TENDENCY
       ...
       PRINT *,' ** RDT DTLB ',RDT,DTLB,ITNXT,ITPRV
...
C
 200  CONTINUE
      print *,' jump into oneway with iupdate = ',iupdate,
     1             itnow,dtlb
...
C
     ...
     4                           ,(  HNXT(I)  ,I=1,IJHLB          )
C                  PARTIAL ECHO PRINT
       PRINT *,' ** READ LATERAL BOUNDARY DATA AT ',ITNXT,ITPRV
C
       DO KER=1,KMLB
         print *,'latbnd read ',ker,itprv,itnxt
         if(ker.eq.1) then
          print *,'latbnd read ',htnd(689),HNXT(689)
         endif
         print *,'latbnd read ',hutnd(689,ker),HUNXT(689,ker)
         print *,'latbnd read ',hvtnd(689,ker),HVNXT(689,ker)
         print *,'latbnd read ',httnd(689,ker),HTNXT(689,ker)
         print *,'latbnd read ',hqtnd(689,ker),HQNXT(689,ker)
       ENDDO
C
       IF( ITPRV .EQ. ITNXT ) GO TO 9940
       RDT = DTLB / ( ITNXT - ITPRV )
       PRINT *,' ** LATERAL BOUNDARY TENDENCY ',ITNXT,ITPRV,DTLB,RDT
C
CMIC$ PARALLEL SHARED(IJHLB, RDT, HNXT, HTND, KMLB, IJULB, IJVLB, HUNXT
CMIC$1   , HUTND, HVNXT, HVTND, HTNXT, HTTND, HQNXT, HQTND) PRIVATE(IQ2
CMIC$2   , K)
CMIC$ DO PARALLEL
      ...
      DO 89040 IQ2=1,IJULB
         ...
         HUTND(IQ2,K)=(HUNXT(IQ2,K)-HUTND(IQ2,K))*RDT
         if(iq2.eq.689) then
           print *,'hu at 689 ',ITNXT,iq2,k,HUNXT(IQ2,K),
     1       HUTND(IQ2,K),RDT,huold
         endif
89040 CONTINUE
      DO 89050 IQ2=1,IJVLB
         ...
         HVTND(IQ2,K)=(HVNXT(IQ2,K)-HVTND(IQ2,K))*RDT
         if(iq2.eq.689) then
           print *,'hv at 689 ',ITNXT,iq2,k,HvNXT(IQ2,K),
     1       HvTND(IQ2,K),RDT,hvold
         endif
89050 CONTINUE
      DO 89060 IQ2=1,IJHLB
         ...
         HQTND(IQ2,K)=(HQNXT(IQ2,K)-HQTND(IQ2,K))*RDT
         if(iq2.eq.689) then
           print *,'ht at 689 ',ITNXT,iq2,k,HTNXT(IQ2,K),
     1       HTTND(IQ2,K),RDT,htold
           print *,'hq at 689 ',ITNXT,iq2,k,HQNXT(IQ2,K),
     1       HQTND(IQ2,K),RDT,hqold
         endif
89060 CONTINUE
 230   CONTINUE
CMIC$ DO PARALLEL VECTOR
      DO 89080 IQ2=1,IJHLB
         ...
         HTND(IQ2)=(HNXT(IQ2)-HTND(IQ2))*RDT
         if(iq2.eq.689) then
           print *,'h at 689 ',ITNXT,iq2,k,HNXT(IQ2),
     1       HTND(IQ2),RDT,hold
         endif
89080 CONTINUE
CMIC$ END PARALLEL
...
C*****  Code Expanded From Routine:  RDHSCATR
      DO 77006 I6X = 1, IJULB
         OLDVBL=VBL(LSCRU(I6X)+IADU-1)
         VBL(LSCRU(I6X)+IADU-1) = HUNOW(I6X,K)
         if(i6x.eq.689) then
           print *,'u update in oneway ',k,i6x,itnxt,itprv,
     1      oldvbl,HUNOW(I6X,K)
         endif
77006 CONTINUE
      DO 77007 I7X = 1, IJVLB
         oldvbl=VBL(LSCRV(I7X)+IADV-1)
         VBL(LSCRV(I7X)+IADV-1) = HVNOW(I7X,K)
         if(i7x.eq.689) then
           print *,'v update in oneway ',k,i7x,itnxt,itprv,
     1      oldvbl,HVNOW(I7X,K)
         endif
77007 CONTINUE
      DO 77008 I8X = 1, IJHLB
         oldvbl=VBL(LSCRH(I8X)+IADT-1)
         VBL(LSCRH(I8X)+IADT-1) = HTNOW(I8X,K)
         if(i8x.eq.689) then
           print *,'t update in oneway ',k,i8x,itnxt,itprv,
     1      oldvbl,HTNOW(I8X,K)
         endif
77008 CONTINUE
      DO 77009 I9X = 1, IJHLB
         oldvbl=VBL(LSCRH(I9X)+IADQ-1)
         VBL(LSCRH(I9X)+IADQ-1) = HQNOW(I9X,K)
         if(i9x.eq.689) then
           print *,'q update in oneway ',k,i9x,itnxt,itprv,
     1      oldvbl,HQNOW(I9X,K)
         endif
77009 CONTINUE
...
C*****  Code Expanded From Routine:  RDHSCATR
      DO 77010 I10X = 1, IJHLB
         OLDVBL = VBL(LSCRH(I10X)+IADH-1)
         VBL(LSCRH(I10X)+IADH-1) = HNOW(I10X)
         if(i10x.eq.689) then
          print *,'h update in oneway ',i10x,itnxt,itprv,
     1      oldvbl,HNOW(I10X)
         endif
77010 CONTINUE
C*****  End of Code Expanded From Routine:  RDHSCATR
