...
C                                                                       
C SUBPROGRAM:    TJPROJ      COMPUTES 3-DIMENSIONAL TRAJECTORIES        
C   PRGMMR:  R. M. REAP      ORG: W/OSD21          DATE: 95-11-02       
C                                                                       
C ABSTRACT:  COMPUTES 3D TRAJECTORIES FOR 36 AND 48-HR PROJECTIONS      
C                                                                       
...
C        PURPOSE                                                        
C           COMPUTES 3D PARCEL TRAJECTORIES FROM INPUT WIND DATA FROM   
C           DISK DATASETS GENERATED BY CURRENT NCEP NUMERICAL MODELS.   
C           THE INFLUENCE OF A DETAILED TERRAIN GEOMETRY IS INCLUDED    
C           BY NOT ALLOWING THE TRAJECTORIES TO GO BELOW GROUND. X,Y,P  
C           COORDINATES DEFINING THE PARCEL POSITIONS AT 6-HR INTERVALS 
C           ARE OUTPUT ON DISK.                                         
C        DATA SET USE                                                   
C           FT87-89 (OUTPUT)                                            
C        COMMON BLOCKS                                                  
C           KDMY,BLOCK4,BLOCK5,BLOCKF                                   
C        SUBPROGRAMS CALLED:                                            
C           RDPE,WIND,SMTR9,REDISK,OVERT,INTPL1,MARIAH,
C           OUTPT3,TJANAL,TJFCST                                  
C        LIBRARY                                                        
...
C                                                                       
     ...
     1 XCORD(150),YCORD(150),RHUM(13,17,4),INCR                         
     ...
     1 WP(5,5,5)                                                        
      ...
      DIMENSION TERRA(51,65),LTAU(4),AP(858),APP(26,33),P(5),           
     1 KU(26,33,5),KV(26,33,5),KW(26,33,5),LU(26,33,5),LV(26,33,5),     
     2 LW(26,33,5),MU(26,33,5),MV(26,33,5),MW(26,33,5)                  
      DIMENSION JII(3),JPP(5),F3(26,33)                                 
      EQUIVALENCE (APP(1,1),AP(1))                                      
      INTEGER X                                                         
      DATA P/1000.,850.,700.,500.,300./,LTAU/0,12,24,36/                
      DATA JPP/160,133,112,80,48/,JII/48,49,5/                          
C     ***************************************************************** 
C        CONVERT U,V,W, WIND COMPS AND TRANSFER TO DISK FILE 89         
      NFILE=84                                                          
      NRCD=0                                                            
      DO 150 K=1,49,6                                                   
      JTAU=K-1                                                          
      ...
      CALL WIND(APP)                                                    
      NRCD=NRCD+1                                                       
      WRITE(89,REC=NRCD) (AP(KK),KK=1,858)                              
 110  CONTINUE                                                          
      DO 140 N=1,5                                                      
 ...
 140  CONTINUE                                                          
 150  CONTINUE                                                          
C        READ TERRAIN HEIGHTS FROM DISK 83                              
      NRCD=1                                                            
      READ(83,REC=NRCD,ERR=10) ((TERRA(I,J),J=1,65),I=1,51)             
      GO TO 12                                                          
  10  PRINT 11                                                          
  11  FORMAT(10X,'TJPROJ DISK READ PROBLEM -- TERRAIN')                 
  12  DO 100 KPR=1,2                                                    
      INCR=24+(KPR*12)                                                  
C        INITIALIZE TRAJECTORY TERMINAL POINTS AT 13X17 ARRAY OF        
C        GRID POINTS                                                    
      SFX=-0.42262                                                      
      CFX=0.90631                                                       
      CH=10.1                                                           
  ...
  16  PLP(I,J,N)=PSALT                                                  
  20  CONTINUE                                                          
C        WRITE TERMINAL POINT X,Y,P ON DISK 87 (36-H) AND DISK 88 (48-H)
      ND=86+KPR                                                         
      NRCD=1                                                            
      WRITE(ND,REC=NRCD) (((PLX(I,J,N),N=1,4),J=1,17),I=1,13)           
      NRCD=NRCD+1                                                       
      WRITE(ND,REC=NRCD) (((PLY(I,J,N),N=1,4),J=1,17),I=1,13)           
      NRCD=NRCD+1                                                       
      WRITE(ND,REC=NRCD) (((PLP(I,J,N),N=1,4),J=1,17),I=1,13)           
...
C        READ U,V,W WIND COMPS FROM DISK 25                             
...
C        START TRAJECTORY COMPUTATIONS                                  
  ...
  33  PLX(I,J,N)=PLX(I,J,N)-0.000001                                    
  34  IF(ABS(B-K)-1.5)36,35,40                                          
  35  PLY(I,J,N)=PLY(I,J,N)-0.000001                                    
...
C        PRINT 6-HR PARCEL POSITIONS                                    
  ...
  52  NSTAP=NRCD                                                        
      NRCD=25                                                           
      IF(KPR.EQ.2) NRCD=31                                              
      WRITE(ND,REC=NRCD) (((ZDF(I,J,N),N=1,4),J=1,17),I=1,13)           
C        OBJECTIVE ANALYSIS OF TEMP/MIXING RATIO AT ORIGIN POINTS       
      CALL TJANAL                                                       
C        COMPUTE 6-H CHANGE OF TEMP/MIXING RATIO FOR PARCELS MOVING     
C        ALONG 3D TRAJECTORIES. NONADIABATIC HEATING AND AIR-SEA        
C        HEAT AND MOISTURE FLUXES ARE INCLUDED IN PARCEL THERMODYNAMICS 
      CALL TJFCST(KPR)                                                  
 100  CONTINUE                                                          
      RETURN                                                            
      END                                                               
