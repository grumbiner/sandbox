...
// try reducing the upwelling shortwave to increase melting
        if (ll.lat > 0 && icec[xij] > 0) {
          netrad[xij] = (lwdown[xij] + swdown[xij] - lwup[xij] - (swup[xij]*nhswup) );
        }
        else {
          netrad[xij] = (lwdown[xij] + swdown[xij] - lwup[xij] - (swup[xij]*shswup) );
        }
