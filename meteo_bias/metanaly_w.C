...
int main(int argc, char *argv[], char *avrg_sw, char *avrg_lw, char *avrg_temp) {
  northgrid<float> tair, slp, rh, tsfc, swdn, lwdn, mask;
  char field[900], parameter[900];

  FILE *fin, *fout_sw, *fout_lw, *fout_temp;
  ...
  for (tloc.j=0; tloc.j < lwdn.ypoints(); tloc.j++) {
	...
	printf("Lat %f Lon %f; ", ll.lat, ll.lon);
        printf("Temp = %f; ", lwtemp);

        printf("DnSw = %f; ", swdn[tloc]);
        if(swdn[tloc] > 1367) 
         	printf("*DnSw out of range.*\n");
 	 
//	printf("%f ", tair[tloc]);

	printf("DnLw = %f; ", LW);
	if(LW <= 158.669847)
        	printf("*DnLw under range.*\n");  	                   		else if (LW >= 324.275273438)
		printf("*DnLw over range.*\n");
        n++;
	}
