...
ncepxlC $SRCDIR/harmonicquarter.C -I $INCDIR -O2 -DIBM \
                  -L /nwprod/lib/ -l ombf_4 -o harmonicquarter

if [ ! -f sstread -o ! -f sstquarter -o ! -f harmonicquarter ] ; then
  echo could not compile needed file
  exit 1
fi
cp sstread sstquarter harmonicquarter $RUNDIR
...
#Make the running count of statistics:
  if [ -s sstquarter.$tag ] ; then
    if [ -s count ] ; then
      ./harmonicquarter  sstquarter.$tag count > proc.$tag
      if [ $? -eq 0 ] ; then
        rm sstquarter.$tag sst.$tag
      fi
    else
      ./harmonicquarter  sstquarter.$tag       > proc.$tag
    fi
    tally=`expr $tally + 1`
  fi
...
./harmonicpost tcount newquarter tmpstats
