...
int main(int argc, char *argv[]) {
  ...
  GRIDTYPE<unsigned char> iage, oage;
  GRIDTYPE<float>  ice1, ice2;
...
//        #ifdef VERBOSE
        printf("Reset %4d %4d  %7.3f %7.3f  overage %2d from %3d ",
            x.i, x.j, ll.lon, ll.lat, iage[x], (int) oice[x]);
//        #endif

        if (imsice[tloc] < MAX_ICE && ll.lat > 0) {
//          #ifdef VERBOSE
          printf(" via imsice ");
//        #endif
          delta = oice[x] - imsice[tloc];
          oice[x] = imsice[tloc];
        }
...
//        #ifdef VERBOSE
        printf(" to %3d  %3d %3d %4d\n",oice[x], (int)imsice[tloc], (int)noice[tloc],(int) delta);
//        #endif
        oage[x] = 0; 
      }
