...
C                                                                       
C ABSTRACT: DISPLAYS ONLY THE NORTH AMERICAN PORTION                    
C   OF THE RGNL OI ANALYSIS GRID (HEMISPHERIC 180X60), SUBSET IS        
C   .       (46X31)    I=112,157 AND J=12,42.                           
C   OF THE RGNL OI ANALYSIS GRID (HEMISPHERIC 360X90), SUBSET IS        
C   .       (91X46)    I=223,313 AND J=17,62.                           
C                                                                       
...
C$$$                                                                    
C                                                                       
C THIS IS A MODIFIED VERSION OF EZSKCH (ON THE ABOVE DATA SET)          
C WHICH IS SET UP TO DISPLAY ONLY THE NORTH AMERICAN PORTION            
CCOF THE RGNL OI ANALYSIS GRID (HEMISPHERIC 180X60), SUBSET IS          
CC          (46X31)    I=112,157 AND J=12,42.                           
C OF THE RGNL OI ANALYSIS GRID (HEMISPHERIC 360X90), SUBSET IS          
C           (91X46)    I=223,313 AND J=17,62.                           
C                                                                       
...
C   3 CONTINUE                                                          
    4 CONTINUE                                                          
      NLV2   = NLV/2                                                    
      NLV2M1 = NLV2 - 1                                                 
      print*,'This WRITE is commented'
C INTERNAL WRITE NOW FORTRAN STANDARD ???                                  
c     WRITE(UNIT=XFOR,FMT=5)
c     ENCODE(52,5,YFOR)                                                 
C     ENCODE(52,5,YFOR)                                                 
c   5 FORMAT(52H( 21X,6A4,/,5X,3A4,5X,4A4,5X,7A4,//,  X,   30F  .1) )
      print*,'FORMAT is commented, too'
 ...
 50   CONTINUE                                                          
   ...
   66 print*,'Next WRITE'
      print*,'YFOR=',YFOR
c  66 WRITE(6,YFOR)(YTITLE(I),I=1,20),(XAXIS(J),J=1,NXAXIS)             
c     WRITE(6,YFOR)(YTITLE(I),I=1,20),(XAXIS(J),J=1,NXAXIS)
      print*,'After WRITE'
      NSK=IYD                                                           
      XD    = IXD                                                       
...
C     IF(IL.EQ.NPRL)WRITE(6,12345)Z,Z1,Z2,ZMIN                          
 ...
 250  IF (.NOT.AXIS) WRITE (6,260) HTIT,    (IQ(I),I=1,NPRC)            
  ...
  300 CONTINUE                                                          
   ...
   28 FORMAT(1X,/,7X,'MIN=',E11.4,4X,'MAX=',E11.4,/,1X)
   29 FORMAT(1X,/,7X,'MIN=',E11.4,4X,'MAX=',E11.4,/,1X)
      IF(ALLCON)GO TO 310                                               
      CON(11)=CON(1)                                                    
