...
C
      READ(14,100) IYRCYC,IMOCYC,IDACYC,IHRCYC
100   FORMAT(6X,I4,I2,I2,I2)
C
...
C
     ...
     &      CUPPT,CFRACH
         WRITE(LIST,*)'  READ  ',LABEL
         READ(nfcst) SOILTB,SFCEXC,SMSTAV,SMSTOT,GRNFLX,PCTSNO
         WRITE(LIST,*)'  READ  ',LABEL
...
C
         do j = 1, jm
           do i = 1, im
             if(mod(i,40).eq.0 .and .mod(j,20).eq.0) then
             write(6,12346)i,j,sh2o(i,j,1),sh2o(i,j,2),sh2o(i,j,3),
     &         sh2o(i,j,4),albedo(i,j),si(i,j),sm(i,j)
