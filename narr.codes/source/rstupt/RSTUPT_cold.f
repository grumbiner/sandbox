...
C
     ...
     &,sice(im,jm)
     &,smce(im,jm,nsoil),cmce(im,jm),stce(im,jm,nsoil)
C-----------------------------------------------------------------------
                             L O G I C A L * 1
     & PARTIAL, NEWSNOW
                             L O G I C A L 
...
C
         ...
         ELSE
           READ(nfcst) RUN,IDAT,IHRST,NTSD,LABEL
c          print*,'idat,ihrst,ntsd=',idat,ihrst,ntsd
           ntsd=0
           READ(nfcst) PDOMG,RESOMG
           WRITE(nrestrt) RUN,IDAT,IHRST,NTSD,LABEL
           WRITE(nrestrt) PDOMG,RESOMG
...
C
         do l = 1, lm
           write(6,12345)t(93,53,l),q(93,53,l),u(93,53,l),v(93,53,l)
12345      format(1x,'at i,j=93,53 ',4(1x,e12.5))
         enddo
C        
         DEALLOCATE(T)
         DEALLOCATE(U)
...
C
     ...
     &      CUPPT,CFRACH
C
C  Read 13 records to get to sh2o, albedo 
C
         do ier = 1, 10
           read(nfcste)
         enddo
...
C
         ie=203
         je=196
         write(6,12350)ie,je,smc(ie,je,1),stc(ie,je,1),
     &      albedo(ie,je),sno(ie,je),si(ie,je)
12350    format(1x,2i6,5(1x,e12.5))
...
c
         do j = 1, jm
           do i = 1, im
             ...
             write(6,12346)i,j,sh2o(i,j,1),smc(i,j,1),stc(i,j,1),
     &         sh2o(i,j,4),albedo(i,j),sno(i,j),si(i,j)
12346        format(1x,2i6,7(1x,e12.5))
             endif
