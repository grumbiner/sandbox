...
c     IF ( (IHRST .NE. IHRSTI) .OR. (IDAT(1) .NE. IDATI(1)) .OR.
c    1     (IDAT(2) .NE. IDATI(2)) .OR. (IDAT(3) .NE. IDATI(3)) ) THEN
c       PRINT *,'MIS-MATCH WITH INPUT EDAS DATE/TIME - STOP EXECUTION'
c       PRINT *,IHRST,IHRSTI,IDAT(1),IDATI(1),IDAT(2),IDATI(2),
c    1     IDAT(3),IDATI(3)
c       STOP 666
c     ENDIF
C
...
C
      ...
      ALLOCATE(IVGTPKI(IMI,JMI))
      ALLOCATE(ISLTPKI(IMI,JMI))
      ALLOCATE(ISPTPKI(IMI,JMI))
...
C
      ...
      READ(NRESTRTI) SH2OI
      print*,'after SH2OI'
C READ of dynamic albedo that follows SH2O is ignored.
C Dynamic albedo updated in CNSTS.f prior to call SFCEDAS.f (here)
      READ(NRESTRTI) DUM
      print*,'after next DUM read'
	    ...
	    DO 50 KRAD=1,5
	      DO 40 JY = JI-KRAD,JI+KRAD
                DO 30 IX = II-KRAD,II+KRAD
                  IF ( (IX .LT. 1) .OR. (IX .GT. IMI) ) GOTO 30
                  IF ( (JY .LT. 1) .OR. (JY .GT. JMI) ) GOTO 30
...
C TAKE NEAREST LAND NEIGHBOR STATES
  60      CONTINUE
          IVGTPK(I,J) =IVGTPKI(II,JI)
          ISLTPK(I,J) =ISLTPKI(II,JI)
          ISPTPK(I,J) =ISPTPKI(II,JI)
          CMC(I,J)    =CMCI(II,JI)
          DO 70 K=1,NSOIL
            SMC(I,J,K) =SMCI(II,JI,K)
            STC(I,J,K) =STCI(II,JI,K)
            SH2O(I,J,K)=SH2OI(II,JI,K)
  70      CONTINUE
 100    CONTINUE
 200  CONTINUE
