...
c
c     parameter(numbad=6059)
      parameter(numbad=784)
      integer idate(4)
      integer ibad(numbad),jbad(numbad)
c     real rlatb(numbad),rlonb(numbad)
      integer icetest(im,jm)
      real ice(im,jm)
c     real ice(92,141)
C----------------------------------------------------------------------
...
c       print*,'sinphi=',sinphi
c       print*,'asin(sinphi)=',asin(sinphi)
c     endif
...
C
      IF(JULD.LE.32) THEN
        ...
        WGT1 = S2/(S1+S2)
        WGT2 = S1/(S1+S2)
c       print*,'s1,s2,wgt1,wgt2=',s1,s2,wgt1,wgt2
        DO J = 1,180
        DO I = 1,361
        ALBC(I,J) = WGT1 * ALBC1(I,J) + WGT2 * ALBC4(I,J)
c       print*,'i,j,albc1(i,j),albc4(i,j),albc(i,j)=',
c    *   i,j,albc1(i,j),albc4(i,j),albc(i,j)
        END DO
        END DO
C-----------------------------------------------------------------------
...
c           print*,'glat,glon,elat,elon=',
c    *       glat(i,j),glon(i,j),elat,elon
            ILON1=INT(ELON)
            DIF=ELON-ILON1
...
c     jpds(21)=icent-1
c
c Get AF snow in meters on native 32 km NARR E-grid
c
        call baopenr(41,'fort.41',iret)
        write(0,*) 'baopenr on unit41', ' iret=', iret
        print*,'snow jpds=',jpds
        CALL GETGB(41,0,IM*JM,0,JPDS,JGDS,KF,K,KPDS,KGDS,BIT,SI,IRET)
        print*,'snow kpds=',kpds
        print*,'snow kgds=',kgds
...
10      format('iret=',i3,' kf=', i6,' fld=', i3,
     &     2x,'AF SNOW', 4i2.2)
c
c Get sea-ice on native 32 km NARR E-grid
c
        call baopenr(44,'fort.44',iret)
        write(0,*) 'baopenr on unit44', ' iret=', iret
        print*,'ice jpds=',jpds
        CALL GETGB(44,0,IM*JM,0,JPDS,JGDS,KF,K,KPDS,KGDS,BIT,SICE,IRET)
        print*,'ice kpds=',kpds
        print*,'ice kgds=',kgds
...
11      format('iret=',i3,' kf=', i6,' fld=', i3,
     &     2x,'SEA ICE', 4i2.2)
c
c Get SST on native 32 km NARR E-grid
c
        call baopenr(39,'fort.39',iret)
        write(0,*) 'baopenr on unit39', ' iret=', iret
        print*,'sst jpds=',jpds
        CALL GETGB(39,0,IM*JM,0,JPDS,JGDS,KF,K,KPDS,KGDS,BIT,SST,IRET)
        print*,'sst kpds=',kpds
        print*,'sst kgds=',kgds
...
C    SI      SNOW DEPTH (M) (ALWAYS ZERO OVER OPEN SEA)
...
C              SICE=1, ALBEDO=0.65 OVER SEAICE
C-----------------------------------------------------------------------
C
      DO J=1,JM
        DO I=1,IM
...
C  SEA
            ...
            IF(SICE(I,J).GT.0.    ) THEN
C-----------------------------------------------------------------------
C  SEA-ICE      
              SM(I,J)=0.
	      SI(I,J)=0.10
	      SNO(I,J)=SI(I,J)*.10    !! RR assumes 1:10 ratio
              TG(I,J)=TI0
...
C   USING UNIVERSAL SNOW DENSITY FROM 1:10 RATIO.
          ELSE
            EPSR(I,J)=1.0
      ...
      write(*,*) " STC(82,382,1:4) ", STC(82,382,1:4) 
c
c Print ice
c     print 1234
c1234  FORMAT(////' ETA ICE ( ice=1, no ice=0 )'/)
c     DO 600, JJ=1,JM,4
c     do 600, jj=1,jm
c     J = JM + 1 - JJ
c     PRINT 73, J, (nint(SICE(I,J)), I=1,IM,3)
c     print 75, j, (nint(sice(i,j)), i=1,im)
c  73 FORMAT(' ', ' J=', I3, 1X, 115I1)
c  75 format(' ', ' J=', i3, 1x, 287i1)
c 600 CONTINUE
c     call printeta(si,sm)
c     sice=0
c     si=0
c     sst=0
c     sno=0
...
C ----------------------------------------------------------------------
      CALL SFCEDAS
c     CALL SFCEDASC
c     readsfc_tim=timef()-btim
c     print *,'ok after readsfc, time =',readsfc_tim

      ELSE IF (IUPDTSFC .EQ. 1) THEN
C
C     INITIALIZE NARR STREAMS
C
      CALL SFCEDAS_RRSTREAM
C ----------------------------------------------------------------------
C cold start:  determine liquid soil water content (SH2O)
      DO J = 1, JM
        DO I = 1, IM
      ...
      DO J = 1,JM
        DO I = 1,IM
          IF(SICE(I,J).GT.0.5) THEN
           TG(I,J) = TI0
C      write(*,*)" SICE(I,J).GT.0.5 TG=TI0",I,J,SICE(I,J),TG(I,J)
      write(*,*)" SICE(I,J).GT.0.5 STC=",I,J,N,(STC(I,J,N),N=1,NSOIL)
          END IF
