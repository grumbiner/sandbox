C
      SUBROUTINE INTERPS(NR,ARRAY,HLAT,HLON,VLAT,VLON,ARRAYE,LTOTAL,REF)
...
C-----------------------------------------------------------------------
                            D I M E N S I O N
     &  ARRAY(IMAX,JMAX),ARRAYE(IM,JM),REF(IM,JM)
...
C
        IF(NR.EQ.3.OR.NR.EQ.4) THEN
          ARRAYE(I,J) = (W1V * ARRAY(IETAV,JETAV)
     1                +  W2V * ARRAY(IETAV+1,JETAV)
     2                +  W3V * ARRAY(IETAV,JETAV+1)
     3                +  W4V * ARRAY(IETAV+1,JETAV+1))
C
         ELSE
C
...
c              write(6,*)"one only at i,j,i,j ",ietah,jetah,i,j
          ...
          END IF
          END IF
          ARRAYE(I,J) = (W1H * ARRAY(IETAH,JETAH)
     1                +  W2H * ARRAY(IETAH+1,JETAH)
     2                +  W3H * ARRAY(IETAH,JETAH+1)
     3                +  W4H * ARRAY(IETAH+1,JETAH+1))
        END IF
      ENDDO
      ENDDO
 220  CONTINUE
