...
c       if(j.eq.387) print*,'i,j,pd(i,j)=',i,j,pd(i,j)
        PDORG(I,J) = PD(I,J)
C
...
c------------------------------------------------------------------
      ELSE
        HLDM=H(I,J,LDM)
        WRITE(6,*)"SFC TROUBLE"
c       print*,'i,j,l2,l3=',i,j,l2,l3
c       print*,'psfc(i,j),rspl(i,j),h(i,j,l2),hgt(i,j)=',
c    *    psfc(i,j),rspl(i,j),h(i,j,l2),hgt(i,j)
c       print*,'h(i,j,l3),pt,ref(i,j)=',h(i,j,l3),pt,ref(i,j)
...
c     DO I=1,IM
c       L1K=LDT1(I,J,L)
c       DLT(I,J)=ALPR(I,J,L1K+1)*LOG(PINT(I,J,L1K+2)/PINT(I,J,L1K))
c    1        *(-ALPR(I,J,L1K))
C
c       L1K=LDT1(I,J,L)
c       L2=L1K+1
...
c       H3M1=H3-H1A
C
c       ALPL3=ALP(I,J,L3)*ALP(I,J,L3)
c       ALPL1K=ALP(I,J,L1K)*ALP(I,J,L1K)
c       ALPL2=ALP(I,J,L2)*ALP(I,J,L2)
C
c       B=(H3M2*(ALPL3-ALPL1K)
c    1    -H3M1*(ALPL3-ALPL2))/DLT(I,J)
c       C=(H3M1*(ALP(I,J,L3)-ALP(I,J,L2))
...
c    1    -H3M2*(ALP(I,J,L3)-ALP(I,J,L1K)))/DLT(I,J)
c
c       ARGLOG=PT+PD(I,J)*ETA(L)
c       ALPET=LOG(ARGLOG)
c       ARGLG2=PT+PD(I,J)*(ETA(L)+0.5*DETA(L))
c       ALPM=LOG(ARGLG2)
c       HETA=H2+B*(ALPET-ALP(I,J,L2))+C*(ALPET**2-ALPL2)
