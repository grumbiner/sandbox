...
c    &, LP1=LM+1,nglats=190,nglons=384)
      parameter(numbad=6059)
      integer idate(4)
      integer ibad(numbad),jbad(numbad)
      integer jpds(25)
      real ice(im,jm)
      integer ibad(numbad),jbad(numbad)
C----------------------------------------------------------------------
...
c     do j=1,180
c      do i=1,361
c       print*,'i,j,snalmx(i,j)=',i,j,snalmx(i,j)
c      enddo
c     enddo
C-----------------------------------------------------------------------
C READ ALBEDO FILES
...
C
      print*,'idat(1),idat(2)=',idat(1),idat(2)
      JULD = JULM(IDAT(1)) + IDAT(2)
      print*,'julm(idat(1)),juld=',julm(idat(1)),juld
C-----------------------------------------------------------------------
...
c             print*,'interpolating mxsnal at i,j=',i,j
c             print*,'ilon1,ilon2,ilat1,ilat2=',ilon1,ilon2,ilat1,ilat2
c             print*,'ar1,ar2,ar3,ar4=',ar1,ar2,ar3,ar4
c             print*,'SNALMX(ILON2,ILAT2)=',SNALMX(ILON2,ILAT2)
c             print*,'SNALMX(ILON2,ILAT1)=',SNALMX(ILON2,ILAT1)
c             print*,'SNALMX(ILON1,ILAT1)=',SNALMX(ILON1,ILAT1)
c             print*,'SNALMX(ILON1,ILAT2)=',SNALMX(ILON1,ILAT2)
...
C Use nearest land neighbor:
              IF (ARX .GT. 0.) THEN
c             print*,'i,j,ilonx,ilatx=',i,j,ilonx,ilatx
c             print*,'snalmx(ilonx,ilatx)=',snalmx(ilonx,ilatx)
...
C CONVERT ALBEDO UNITS: PERCENT TO FRACTION
        ...
        ENDDO
      ENDDO
c      print *,'303,143 ',albase(303,143),mxsnal(303,143)
c      print *,'309,143 ',albase(309,143),mxsnal(309,143)
...
c  For the Regional Reanalysis only:  SNOHIRES will read the Air Force Snow
c  only.
c
       btim=timef()
       CALL SNOHIRES (SI,SM,GLAT,GLON,LIST,LUSAF,DTR,idate)
       snohires_tim=timef()-btim
       print *,'ok after snohires, time = ',snohires_tim
C
       btim=timef()
       CALL SSTHIRES (SST,SM,GLAT,GLON,IDAT,LIST,DTR,idate)
...
c
c When the Great Lakes snow is available, uncomment out these lines.
C
c      btim=timef()
c      CALL SST14K   (SST,SM,GLAT,GLON)
c      sst14k_tim=timef()-btim
c      print *,'ok after sst14, time = ',sst14k_tim
C
      ENDIF  ! if NUSNOSST
C
      TPH=SB-DPH
...
C
      ...
      ENDDO
c
c For Regional Reanalysis:  at this point we have the Air Force snow (SI on
c land), and the SST (over the ocean).  We need to bring in Bob Grumbine's
c ice field and incorporate that into the snow field (SI) over water
c
c First we'll read the ice from Bob Grumbine's ice file, already on the
c Eta grid (80- or 32-km)
c
c     call baopen(icein,'fort.44',ireto)
c     j=0
c     jpds=-1
...
c     jpds(10)=-1
c     jpds(21)=icent
c     call getgb(icein,0,im*jm,0,jpds,jgds,kf,knum,kpds,kgds,
c    *     bitice,ice,iret)
c     WRITE(6,*) 'AFTER GETGB FOR GRUMBINE ICE, IRET=', IRET
      ice=0
c
c For now, Bob's ice file is in binary not grib
c
c     read(icein) ice
c     do j=1,jm
c     do i=1,im
c      print*,'i,j,ice=',i,j,ice(i,j)
c     enddo
c     enddo
c
c Read in file containing the problematic ice points
c
      do i=1,numbad
       read(iubad,1111) ibad(i),jbad(i)
c      print*,'i,ibad,jbad=',i,ibad(i),jbad(i)
1111   format(i3,1x,i3)
      enddo
c
c Now, for water points only:
c
       do j=1,jm
        do i=1,im
         if( SM(I,J) .GT. 0.9 ) then
c
c Decide if the point we're on is one of the problematic points
c
          do nbad=1,numbad
c           print*,'i,ibad(nbad),j,jbad(nbad)=',
c    *        i,ibad(nbad),j,jbad(nbad)
            if(i.eq.ibad(nbad).and.j.eq.jbad(nbad)) then
c            print*,i,j,' is bad'
c
c For the time being, use the SST as a proxy for ice.  If the SST is
c equal to 271.35, you may make the point an ice point.  This is not
c recommended by Diane Stokes.
              if(sst(i,j).lt.272.0) then
                print*,'Changing point to ice: ',i,j
                si(i,j)=1.0
              else
                si(i,j)=0.0
              endif  !  sst.lt.272
c             goto 2222
c
c Close our loops and if blocks
c
             endif   !  i.eq.ibad.and.j.eq.jbad
           enddo     !  nbad=1,numbad loop
         endif       !  sm.gt.0.9
2222    continue
        enddo        !  i=1,im loop
       enddo         !  j=1,jm loop
...
C ****  DO TIME INTERPOLATION ****
       JULD=JULM(IDAT(1))+IDAT(2)
c mbek - 29 Nov 2001
c number of days to 'center' time interpolation for veg greenness
       NDAYS=0
C ----------------------------------------------------------------------
C add 2 days in order to use julian day corresponding to center time of
C EDAS (12hr) + Eta (84hr) cycle = 48 hr = 2 days
C ----------------------------------------------------------------------
c      JULD=JULD+2
c mbek - 29 Nov 2001
c generalize this expression
       JULD=JULD+NDAYS
C ----------------------------------------------------------------------
       IF(JULD.LE.15) JULD=JULD+365
       MON2=IDAT(1)
c mbek - 29 Nov 2001
c also include NDAYS in date check here
c       IF(IDAT(2).GT.15) MON2=MON2+1
       IF(IDAT(2).GT.15-NDAYS) MON2=MON2+1
...
c     do j=1,jm
c      do i=1,im
c       print*,'i,j,mxsnal=',i,j,mxsnal(i,j)
c      enddo
c     enddo
      WRITE(NHIBU)MXSNAL
      WRITE(NHIBU)EPSR
