...
c     real uearth(jmaxot),vearth(jmaxot)
...
c         !$OMP PARALLEL DO PRIVATE(U11,V11,U21,V21,U12,V12,U22,V22)
     ...
     &                W12(I)*V12+W22(I)*V22
          ...
          ENDIF
c
c Save the earth-relative winds - 11 December 2002 Perry Shafran
c
c       uearth(i)=uout(i)
c       vearth(i)=vout(i)
        ENDDO
c       if (mod(kgdsout(6)/8,2).eq.1) kgdsout(6)=kgdsout(6)-8
        
        IBOUT=IBIN
        IBS(1:NOUT)=0
c          !$OMP PARALLEL DO PRIVATE(UROT,VROT)
c       DO I=1,NOUT
c         LOUT(I)=WO(I).GE.0.5
c         IF(LOUT(I)) THEN
c           UOUT(I)=UOUT(I)/WO(I)
c           VOUT(I)=VOUT(I)/WO(I)
c           UROT=CROT(I)*UOUT(I)-SROT(I)*VOUT(I)
c           VROT=SROT(I)*UOUT(I)+CROT(I)*VOUT(I)
c           UOUT(I)=UROT
c           VOUT(I)=VROT
c         ELSE
c           IBS(I)=1
...
c       ENDDO
      IBOUT=MAXVAL(IBS)
      IF(KGDSOUT(1).EQ.0) CALL POLFIXV(NOUT,NOUT,1,
     &     RLAT,RLON,IBOUT,LOUT,UOUT,VOUT)
      RETURN
      END
