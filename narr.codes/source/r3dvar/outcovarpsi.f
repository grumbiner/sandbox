subroutine outcovarpsi(isofilter_t, &
...
!     generate output maps of covariance, correlations at selected points
  ...
  call regular_ad_raf(tcor,isofilter_t)
        ...
        if(kout.eq.1.and.mype.eq.0) then
         open(unit=icor,file='psicor.des',form='formatted')
         open(unit=idat,file='psicor.dat',form='unformatted')
         ntime=koutmax
         rlonmap0=1.
         dlonmap=1.
         rlatmap0=1.
...
920      format('psicor ',i5,' 99 psicor')
         next=next+1
