subroutine outcovarchi(isofilter_t, &
...
!     generate output maps of covariance, correlations at selected points
  ...
  allocate(tcor(ims:ime,jms:jme,kms:kme))
  tcor=0.
  k=kds
  do j=jds,jde,50
   do i=ids,ide,50
    k=min(kde,k+2)
    if(i.ge.ips.and.i.le.ipe.and.j.ge.jps.and.j.le.jpe.and.k.ge.kps.and.k.le.kpe) tcor(i,j,k)=1.
   end do
  end do
  ...
  call regular_ad_raf(tcor,isofilter_t)
        ...
        if(kout.eq.1.and.mype.eq.0) then
         open(unit=icor,file='chicor.des',form='formatted')
         open(unit=idat,file='chicor.dat',form='unformatted')
         ntime=koutmax
         rlonmap0=1.
         dlonmap=1.
         rlatmap0=1.
...
920      format('chicor ',i5,' 99 chicor')
         next=next+1
