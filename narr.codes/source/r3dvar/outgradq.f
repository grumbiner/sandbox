subroutine outgradq(qg,qcor, &
         ...
         data dsname/'qcor.dat'/
         data title/'qcor'/
         dimension qg( ims:ime, jms:jme, kms:kme )
         dimension qcor( ims:ime, jms:jme, kms:kme )
        ...
        if(kout.eq.1.and.mype.eq.0) then
         open(unit=iqcor,file='qcor.des',form='formatted')
         open(unit=iqdat,file='qcor.dat',form='unformatted')
         ntime=koutmax
         rlonmap0=1.
         dlonmap=1.
         rlatmap0=1.
...
910      format('qg  ',i5,' 99 qg   ')
         next=next+1
         write(datdes(next),920)np
920      format('qcor ',i5,' 99 qcor')
         next=next+1
         write(datdes(next),980)
980      format('ENDVARS')
         last=next
         write(iqcor,2000)(datdes(i),i=1,last)
2000     format(a80)

        end if

  do k=kme,kms,-1
   call gather_grid(qg(ims:ime,jms:jme,k),out,ims,ime,jms,jme, &
               ids, ide, jds, jde, &                              ! domain indices
               ips, ipe, jps, jpe, &                              ! patch indices
               ims, ime, jms, jme, &                              ! memory indices
         inpes, jnpes, mype, npes, pe_of_injn, in_of_i, jn_of_j ) ! processor info
   if(mype.eq.0) write(iqdat)out
  end do
  do k=kme,kms,-1
   call gather_grid(qcor(ims:ime,jms:jme,k),out,ims,ime,jms,jme, &
               ids, ide, jds, jde, &                              ! domain indices
               ips, ipe, jps, jpe, &                              ! patch indices
               ims, ime, jms, jme, &                              ! memory indices
         inpes, jnpes, mype, npes, pe_of_injn, in_of_i, jn_of_j ) ! processor info
