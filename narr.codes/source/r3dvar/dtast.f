      subroutine dtast(iwrite,type,res,scale,n,rspres,pbot,ptop,mesage,var)
!$$$  subprogram documentation block
!                .      .    .                                       .
! subprogram:    dtast       print table of residuals by data type.
...
!   90-10-11  parrish
!
! usage call dtast(type,res,scale,n,rspres,pbot,ptop,mesage)
!   input argument list:
...
!--    get ioff and make sure it is broadcast to all pe's
      ...
      ioff=-999
      if(n.gt.0) then
       ioff=int(abs(type(1))/100)*100
      end if
      call mpi_allreduce(ioff,ioffall,1,mpi_integer,mpi_max,my_comm,ierror)
      ioff=ioffall
...
!--------
       do 100 i=1,n
        if(res(i).gt.1.e19.or.type(i).lt.0..or.rspres(i).lt.ptop.or.rspres(i).gt.pbot) go to 99
        ress=res(i)*scale
        absr=abs(ress)
        k=1
        if(absr.gt.1.) k=2
