subroutine st_simpin2f(wgts,iwgts,iflag,x1in,x2in,nin,iord,lhalf,lbig, &
          x1grid_start,dx1grid,x2grid_start,dx2grid,n1grid,n2grid,istag1)

  !  compute interpolation weights for 2-d simplex interpolation on a staggered 
  !     grid with uniform spacing in each direction.
  ...
  !   --> x1in,x2in:  coordinates of interpolatees
  ...
  !   --> n1grid:   number of points in x1 direction
  !   --> n2grid:   number of points in x2 direction
...
!    print *,' n1pgrid,n2pgrid=',n1pgrid,n2pgrid
  ...
  call simpin2f(wgts,wgts,wgts,wgts,wgts,wgts,iwgts,iflag,x1pin,x2pin,nin,iord,lhalf,lbig, &
                x1pgrid,x2pgrid,n1pgrid,n2pgrid,1_4,0_4,0_4,0_4,0_4,0_4)
  ...
  do k=1,lbig
   do i=1,nin
    if(iflag(i).ne.0) then
     i2p=iwgts(i,k)/n1pgrid
     i1p=iwgts(i,k)-n1pgrid*i2p
     i2p=i2p+1
     i2=i1p+i2p-k1add-k2add
     if(i2.ge.1.and.i2.le.n2grid) then
      i1=(i1p-i2p-k1add+k2add-ishift(i2)+1)/2
      if(i1.ge.1.and.i1+ishift(i2)+1.le.n1grid) then
       iwgts(i,k)=i1+(i2-1)*n1grid
      else
       iwgts(i,k)=1
       wgts(i,k)=0.
       iflag(i)=0
...
!       do k=1,lbig
!        i2=iwgts(i,k)/n1grid
!        i1=iwgts(i,k)-n1grid*i2
!        i2=i2+1
!        i1=2*i1-1+ishift(i2)
!        print *,' i1,i2,wgt=',i1,i2,wgts(i,k)
!       end do
!       x1s=1.+(x1in(i)-x1grid_start-istag1*dx1grid)/dx1grid
!       x2s=1.+(x2in(i)-x2grid_start)/dx2grid
!       print *,' x1s,x2s=',x1s,x2s
!       if(nprint.gt.5) exit
!      end if
!     end do
...
!        if(nbad.ne.0) write(0,*)' IN ST_SIMPIN2F_TST, NBAD=',nbad

  deallocate(ishift)

return
end subroutine st_simpin2f
