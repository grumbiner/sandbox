...
!--------  Ht_psi, Hp_psi: psi is balanced variable. These define balanced part of T, pdr
  ...
  integer(4) iwgts_bightp(2,nxc*nyc)
           ...
           if(mype.eq.0) write(0,*)' at 1 in bigcop_bayes'
  allocate(resq(max(1,mqdata)))
           if(mype.eq.0) write(0,*)' at 2 in bigcop_bayes'
  allocate(respw(max(1,mpwdata)))
           if(mype.eq.0) write(0,*)' at 3 in bigcop_bayes'
  allocate(resw(max(1,mwdata)))
           if(mype.eq.0) write(0,*)' at 4 in bigcop_bayes'
  allocate(resps(max(1,mpdata)))
           if(mype.eq.0) write(0,*)' at 6 in bigcop_bayes'
  allocate(rest(max(1,mtdata)))
           if(mype.eq.0) write(0,*)' at 7 in bigcop_bayes'
  allocate(resrad(max(1,mraddata)))
           if(mype.eq.0) write(0,*)' at 8 in bigcop_bayes'
  allocate(qetadir(imeta,jmeta,lmetaex))
           if(mype.eq.0) write(0,*)' at 9 in bigcop_bayes'
  allocate(wetadir(imeta,jmeta,lmetaex,2))
           if(mype.eq.0) write(0,*)' at 10 in bigcop_bayes'
  allocate(petadir(imeta,jmeta))
           if(mype.eq.0) write(0,*)' at 11 in bigcop_bayes'
  allocate(tetadir(imeta,jmeta,lmetaex))
           if(mype.eq.0) write(0,*)' at 12 in bigcop_bayes'
