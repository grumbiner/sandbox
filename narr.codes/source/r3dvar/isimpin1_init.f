subroutine isimpin1_init(ixi,tlout,alocalout,blocalout,wgtsi0,iwgtsi0,iord,lbig,x1grid,n1grid)

  ! compute all taylor matrices and pre-integration weights ever needed for 1-d integration

  ! <-- ixi:            local coordinate order counter
  ! <-- tlout:             output taylor matrices
  ! <-- alocalout,blocalout:  used to transform to local coordinates around each grid-point
  ! <-- wgtsi0:         preintegration weights
  ! <-- iwgtsi0:        preintegration addresses
...
!  now get integration weights for each interval
  ...
  do n=1,nin
   if(iflag(n).gt.0) then
    ...
    do i=0,iord
     l=l+1
     iwgtsi0(l,n)=i1ref(n)+ix1sign(n)*ixi(i)
    end do
    if(ix1sign(n).eq.1) then
     r=x1grid(i1ref(n))
     s2=x1grid(i1ref(n)+1)
     s=r+.5*(s2-r)
    else
    ...
    rp=alocalout(n)*(r-x1grid(i1ref(n)))+blocalout(n)
    sp=alocalout(n)*(s-x1grid(i1ref(n)))+blocalout(n)
    l=0
    do i=0,iord
     l=l+1
     tlint=(sp**l-rp**l)/(l*alocalout(n))
     do k=1,iord+1
      wgtsi0(k,n)=wgtsi0(k,n)+tlout(l,k,n)*tlint
     end do
    end do
   end if
  end do
