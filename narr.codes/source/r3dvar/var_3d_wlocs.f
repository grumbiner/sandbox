subroutine var_3d_wlocs(rlon,rlat,rpres,theta,theta0,mdata,ivar,varlats,varpres,nvarlats,nvarpres,npes)

  include 'mpif.h'
      include "my_comm.h"

  real(4) rlon(max(1,mdata)),rlat(max(1,mdata)),rpres(max(1,mdata)),varlats(nvarlats),varpres(nvarpres)
  real(4) theta(max(1,mdata))
   ...
   do j=1,nvarlats
    ...
    if(mdata.gt.0) then
     do i=1,mdata
      ...
      if(del.le.delmin.and.abs(theta(i)-theta0).lt.1.) then
       nearest=i
       delmin=del
      end if
     end do
...
!    print *,' in var_3d_wlocs, mype,ivar,varlat,oblat,varpres,obpres,theta0,theta=', &
!                       mype,theta0,ivar(j,k),varlats(j), &
!                         rlat(ivar(j,k)),varpres(k),spres(ivar(j,k)),theta0,theta(ivar(j,k))
!   end if
