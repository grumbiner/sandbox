subroutine rdtest(inbufr,nbufdat,mbufdat,nbufrad,mbufrad, &
           iayear,iamonth,iaday,iahour,delhour,delhour_sfc, &
           plimit_sfcq,plimit_sfct,plimit_sfcw, &
           userad,drift, &
...
!     delhour_sfc - time window in hours for sfc data
!
...
!---------
  ...
  logical pressure_flag_q,pressure_flag_t,pressure_flag_w
...
!       ---------------
      ...
      KX    = NINT(HDR(5))
      delhour0=delhour
      if((kx.ge.180.and.kx.le.189).or.(kx.ge.280.and.kx.le.289)) delhour0=delhour_sfc
      if(time.lt.-delhour0.or.time.gt.delhour0) go to 9901
...
!       enddo
        ...
        pressure_flag_q=.true.
        pressure_flag_t=.true.
        pressure_flag_w=.true.
        if((kx.ge.181.and.kx.le.189).or.(kx.ge.281.and.kx.le.289)) then
         if(ppb.lt.plimit_sfcq) pressure_flag_q=.false.
         if(ppb.lt.plimit_sfct) pressure_flag_t=.false.
         if(ppb.lt.plimit_sfcw) pressure_flag_w=.false.
        end if
...
!  ----------------------------
        ...
        if(q_on.and.qqq.and.pressure_flag_q) then
...
!  --------------------------------
        ...
        if(t_on.and.qtm.and.pob.lt.1.e8.and.tob.lt.1.e8.and.pressure_flag_t) then
...
!  ---------------
        ...
        if(w_on.and.qwd.and.pob.lt.1.e8.and.uob.lt.1.e8.and.vob.lt.1.e8.and.pressure_flag_w) then
