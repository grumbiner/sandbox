subroutine outcovart(isofilter_t, &
...
!     generate output maps of covariance, correlations at selected points
  ...
  call regular_ad_raf(tcor,isofilter_t)
        ...
        if(kout.eq.1.and.mype.eq.0) then
         open(unit=icor,file='tcor.des',form='formatted')
         open(unit=idat,file='tcor.dat',form='unformatted')
         ntime=koutmax
         rlonmap0=1.
         dlonmap=1.
         rlatmap0=1.
