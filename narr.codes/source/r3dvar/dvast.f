      subroutine dvast(iwrite,type,res,scale,n,rspres,pbot,ptop,etheta,azmin,azmax,mesage,var)
...
!--    get ioff and make sure it is broadcast to all pe's
      ...
      if(n.gt.0) then
       do i=1,n
        itype=int(abs(type(i)))
        if(itype.lt.500) then
         ioffthis=int(abs(type(i))/100)*100
         ioff=max(ioff,ioffthis)
        end if
       end do
      end if
...
!--------
       do 100 i=1,n
        if(nint(type(i)).gt.500) go to 99
