subroutine getetaanl(tetaanl,qetaanl,q2etaanl,cwmetaanl,uetaanl,vetaanl,pdetaanl,imeta,jmeta,lmeta)

!    note that tetaanl is virtual temp, and pdetaanl = pd*res*.01 == dp/deta in mb

! transfer current contents of eta model common block variables to output arrays
         ...
         if(idim2-idim1+1.ne.imeta.or.jdim2-jdim1+1.ne.jmeta) then
          print *,' incorrect local dimensions in getetaanl'
          stop
         end if
...
!  finally, transfer to output arrays
         ...
         do j=jdim1,jdim2
          ...
          do i=idim1,idim2
           ii=i-idim1+1
           pdetaanl(ii,jj)=pd(i,j)*res(i,j)*.01
          end do
         end do
         ...
         do k=1,lm
          do j=jdim1,jdim2
           ...
           do i=idim1,idim2
            ii=i-idim1+1
            tetaanl(ii,jj,k)=t(i,j,k)*(1.+.608*q(i,j,k))
            qetaanl(ii,jj,k)=q(i,j,k)
            q2etaanl(ii,jj,k)=q2(i,j,k)
            cwmetaanl(ii,jj,k)=cwm(i,j,k)
            uetaanl(ii,jj,k)=u(i,j,k)
            vetaanl(ii,jj,k)=v(i,j,k)
           end do
          end do
         end do

       return
       end subroutine getetaanl
