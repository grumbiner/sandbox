subroutine simpin3f(wgts,wgtsx1,wgtsx2,wgtsx3, &
       wgtsx11,wgtsx12,wgtsx13,wgtsx22,wgtsx23,wgtsx33,iwgts, &
       iflag,x1in0,x2in0,x3in0,nin,iord,lhalf,lbig, &
       x1grid0,x2grid0,x3grid0,n1grid,n2grid,n3grid, &
       nf,ndx1,ndx2,ndx3,ndx11,ndx12,ndx13,ndx22,ndx23,ndx33)

  ! compute interpolation weights for 3-d simplex interpolation 
  !             (j. purser scheme)
  ...
  ! --> x1in0,x2in0,x3in0: coordinates of interpolatees
  ...
  !                 if lhalf = 0, lbig = (iord*(iord*(iord+6)+11)+6)/6
  !                 if lhalf = 1, lbig = (iord+1)**3
  ! --> x1grid0,x2grid0,x3grid0: coordinates of interpolator grid
  !                         (can be monotonic increasing or decreasing)
  ! --> n1grid,n2grid,n3grid: dimensions of interpolator grid
  ...
  ! --> ndx3:  =1, then return interpolation weights for df/dx3
  ! --> ndx11: =1, then return interpolation weights for d2f/(dx1*dx1)
  ! --> ndx12: =1, then return interpolation weights for d2f/(dx1*dx2)
  ! --> ndx13: =1, then return interpolation weights for d2f/(dx1*dx3)
  ! --> ndx22: =1, then return interpolation weights for d2f/(dx2*dx2)
  ! --> ndx23: =1, then return interpolation weights for d2f/(dx2*dx3)
  ! --> ndx33: =1, then return interpolation weights for d2f/(dx3*dx3)
  ...
  ! <-- wgtsx3:   interpolation weights for df/dx3 ( wgtsx2(nin,lbig) )
  ! <-- wgtsx11:  interpolation weights for d2f/(dx1*dx1)
  ! <-- wgtsx12:  interpolation weights for d2f/(dx1*dx2)
  ! <-- wgtsx13:  interpolation weights for d2f/(dx1*dx3)
  ! <-- wgtsx22:  interpolation weights for d2f/(dx2*dx2)
  ! <-- wgtsx23:  interpolation weights for d2f/(dx2*dx3)
  ! <-- wgtsx33:  interpolation weights for d2f/(dx3*dx3)
  !                 note: if any of these 10 are not asked for, they
  ...
  dimension x3in0(nin),x3grid0(n3grid)
  dimension wgts(nin,lbig),iwgts(nin,lbig)
  dimension wgtsx1(nin,lbig),wgtsx2(nin,lbig),wgtsx3(nin,lbig)
  dimension wgtsx11(nin,lbig),wgtsx12(nin,lbig),wgtsx13(nin,lbig)
  dimension wgtsx22(nin,lbig),wgtsx23(nin,lbig),wgtsx33(nin,lbig)
  dimension iflag(nin)

  intent(in)  ::x1in0,x2in0,x3in0,nin,iord,lbig
  intent(in)  ::x1grid0,x2grid0,x3grid0,n1grid,n2grid,n3grid
  intent(in)  ::nf,ndx1,ndx2,ndx3,ndx11,ndx12,ndx13,ndx22,ndx23,ndx33
  intent(out) ::wgts,wgtsx1,wgtsx2,wgtsx3,wgtsx11,wgtsx12, &
                 wgtsx23,wgtsx33,iwgts

  dimension x1grid(n1grid),x2grid(n2grid),x3grid(n3grid)
  real(4),allocatable::x1in(:),x2in(:),x3in(:)
  ...
  real(4),allocatable::wgt3(:,:),wgt3x3(:,:),wgt3x33(:,:)
  integer(4),allocatable::iwgt3(:,:),iflag3(:)
  real(4),allocatable::du1dx1(:),du1dx11(:)
  real(4),allocatable::du2dx2(:),du2dx22(:)
  real(4),allocatable::du3dx3(:),du3dx33(:)
  real(4),allocatable::wgtsu1(:,:),wgtsu2(:,:),wgtsu3(:,:)
  dimension tl(8,lbig,lbig)
  dimension ixi(0:iord)
  dimension i1ref(nin),i2ref(nin),i3ref(nin)
  dimension ix1sign(nin),ix2sign(nin),ix3sign(nin)
  dimension x1p(nin),x12p(nin),x123p(nin)
  dimension ix1temp(nin),ix2temp(nin),ix3temp(nin)
...
!   convert coordinates to grid units, using simpin1
  ...
  allocate(iflag1(nin))
  mf1=1
  mdx1=max(ndx1,ndx11,ndx12,ndx13)
  mdx11=ndx11
  lbig1=iord+1
  ...
  deallocate(ixi1) ; deallocate(tl1)
  deallocate(alocal1) ; deallocate(blocal1)
  do i=1,n1grid
   x1grid(i)=i
  end do
  allocate(x1in(nin))
...
!  obtain coordinate derivatives as needed
  ...
  allocate(iflag2(nin))
  mf2=1
  mdx2=max(ndx2,ndx22,ndx12,ndx23)
  mdx22=ndx22
  allocate(ixi1(0:iord))
...
!  obtain coordinate derivatives as needed
  ...
  allocate(iwgt3(nin,iord+1)) ; allocate(wgt3(nin,iord+1))
  allocate(wgt3x3(nin,iord+1)) ; allocate(wgt3x33(nin,iord+1))
  allocate(iflag3(nin))
  mf3=1
  mdx3=max(ndx3,ndx33,ndx13,ndx23)
  mdx33=ndx33
  allocate(ixi1(0:iord))
  allocate(tl1(lbig1,lbig1,2*n3grid))
  allocate(alocal1(2*n3grid))
  allocate(blocal1(2*n3grid))
  call simpin1_init(ixi1,tl1,alocal1,blocal1,iord,lbig1,x3grid0,n3grid)
  call simpin1(wgt3,wgt3x3,wgt3x33,iwgt3,iflag3,x3in0,nin,iord,iord+1,x3grid0,n3grid,mf3,mdx3,mdx33, &
                                     ixi1,tl1,alocal1,blocal1)
  deallocate(ixi1) ; deallocate(tl1)
  deallocate(alocal1) ; deallocate(blocal1)
  do i=1,n3grid
  ...
  do k=1,iord+1
   do i=1,nin
    x3in(i)=x3in(i)+wgt3(i,k)*x3grid(iwgt3(i,k))
   end do
  end do
...
!  obtain coordinate derivatives as needed
  ...
  if(mdx3.eq.1) then
   ...
   do k=1,iord+1
    do i=1,nin
     if(iflag3(i).gt.0) du3dx3(i)=du3dx3(i)+wgt3x3(i,k)*x3grid(iwgt3(i,k))
    end do
   end do
  end if
  if(mdx33.eq.1) then
   allocate(du3dx33(nin))
   du3dx33=0.
   do k=1,iord+1
    do i=1,nin
     if(iflag3(i).gt.0) du3dx33(i)=du3dx33(i)+wgt3x33(i,k)*x3grid(iwgt3(i,k))
    end do
   end do
  end if
  deallocate(iwgt3) ; deallocate(wgt3) ; deallocate(wgt3x3) ; deallocate(wgt3x33)


  !  get i1ref, i2ref, i3ref, indices of nearest grid point to each interpolatee
  ...
  nmax2right=n2grid-iximax ; nmax3right=n3grid-iximax
  do i=1,nin
   i1ref(i)=nint(x1in(i))
   ...
   i3ref(i)=nint(x3in(i))
   i3ref(i)=max(nminleft,min(i3ref(i),nmax3right))
   if(x3in(i)-i3ref(i).lt.0._4) ix3sign(i)=-1
  end do
  ...
  do i=0,iord
   ...
   do j=0,iord-i*mhalf
    ...
    do k=0,iord-(i+j)*mhalf
     l=l+1
     ix3temp=0
     do n=1,nin
      if(i3ref(n).le.0) iflag(n)=0
      ix3temp(n)=i3ref(n)+ix3sign(n)*ixi(k)
      if(ix3temp(n).lt.1.or.ix3temp(n).gt.n3grid) iflag(n)=0
      iwgts(n,l)=((ix3temp(n)-1)*n2grid+(ix2temp(n)-1))*n1grid+ix1temp(n)
     end do
    end do
   end do
  end do
  do n=1,nin
   iflag(n)=min(iflag(n),iflag1(n),iflag2(n),iflag3(n))
  end do
  deallocate(iflag1) ; deallocate(iflag2) ; deallocate(iflag3)
  do l=1,lbig
   do n=1,nin
    ...
    do ix3sign0=-1,1,2
     ...
     do i=0,iord
      ...
      do j=0,iord-i*mhalf
       ...
       do k=0,iord-(i+j)*mhalf
        l=l+1
        x3temp=x3grid(ix30+ix3sign0*ixi(k))-x3grid(ix30)
        lp=0
        x1p0=1.
        do ip=0,iord
         x12p0=x1p0
         do jp=0,iord-ip*mhalf
          x123p0=x12p0
          do kp=0,iord-(ip+jp)*mhalf
           lp=lp+1
           tl(n,l,lp)=x123p0
           x123p0=x123p0*x3temp
          end do
          x12p0=x12p0*x2temp
         end do
         x1p0=x1p0*x1temp
  ...
  call vinvmm(tl,tl,lbig,lbig,lbig,8,8)
  ...
  allocate(itype(nin))
  do n=1,nin
   if(ix1sign(n).eq.-1.and.ix2sign(n).eq.-1.and.ix3sign(n).eq.-1) itype(n)=1
   if(ix1sign(n).eq.-1.and.ix2sign(n).eq.-1.and.ix3sign(n).eq. 1) itype(n)=2
   if(ix1sign(n).eq.-1.and.ix2sign(n).eq. 1.and.ix3sign(n).eq.-1) itype(n)=3
   if(ix1sign(n).eq.-1.and.ix2sign(n).eq. 1.and.ix3sign(n).eq. 1) itype(n)=4
   if(ix1sign(n).eq. 1.and.ix2sign(n).eq.-1.and.ix3sign(n).eq.-1) itype(n)=5
   if(ix1sign(n).eq. 1.and.ix2sign(n).eq.-1.and.ix3sign(n).eq. 1) itype(n)=6
   if(ix1sign(n).eq. 1.and.ix2sign(n).eq. 1.and.ix3sign(n).eq.-1) itype(n)=7
   if(ix1sign(n).eq. 1.and.ix2sign(n).eq. 1.and.ix3sign(n).eq. 1) itype(n)=8
  end do
  ...
  if(nf.eq.1) then  ! no derivative
   ...
   do ip=0,iord
    ...
    do jp=0,iord-ip*mhalf
     ...
     do kp=0,iord-(ip+jp)*mhalf
      lp=lp+1
      z0(1:nin,lp)=x123p(1:nin)
      x123p(1:nin)=x123p(1:nin)*  &
                (x3in(1:nin)-x3grid(max(1,i3ref(1:nin))))
     end do
  ...
  if(ndx3.eq.1.or.ndx33.eq.1) then     ! 1st derivative in 3rd direction
   allocate(wgtsu3(nin,lbig))
   lp=0
   x1p=1.
   do ip=0,iord
    x12p=x1p
   ...
   do k=1,lbig
    ...
    do j=1,lbig
     do n=1,nin
      if(iflag(n).gt.0) wgtsu3(n,k)=wgtsu3(n,k)+z0(n,j)*tl(itype(n),j,k)
     end do
    end do
   end do
   ...
   if(ndx3.eq.1) then
    do k=1,lbig
     do i=1,nin
      wgtsx3(i,k)=du3dx3(i)*wgtsu3(i,k)
     end do
    end do
   end if
  end if
  ...
  if(ndx12.eq.1) then     ! 1st derivative in 1st and 2nd directions
   lp=0
   x1p=1.
   do ip=0,iord
    x12p=ip*x1p
  ...
  if(ndx13.eq.1) then     ! 1st derivative in 1st and 3rd directions
   lp=0
   x1p=1.
   do ip=0,iord
    x12p=ip*x1p
  ...
  ! get interpolation weights
   ...
   do k=1,lbig
    ...
    do j=1,lbig
     do n=1,nin
      if(iflag(n).gt.0) wgtsx13(n,k)=wgtsx13(n,k)+z0(n,j)*tl(itype(n),j,k)
     end do
    end do
   end do
   ...
   do k=1,lbig
    do i=1,nin
     wgtsx13(i,k)=wgtsx13(i,k)*du1dx1(i)*du3dx3(i)
    end do
   end do
  end if
  ...
  if(ndx23.eq.1) then     ! 1st derivative in 2nd and 3rd directions
   lp=0
   x1p=1.
   do ip=0,iord
    x12p=x1p
   ...
   do k=1,lbig
    ...
    do j=1,lbig
     do n=1,nin
      if(iflag(n).gt.0) wgtsx23(n,k)=wgtsx23(n,k)+z0(n,j)*tl(itype(n),j,k)
     end do
    end do
   end do
   ...
   do k=1,lbig
    do i=1,nin
     wgtsx23(i,k)=wgtsx23(i,k)*du2dx2(i)*du3dx3(i)
    end do
   end do
  end if

  if(ndx33.eq.1) then     ! 2nd derivative in 3rd direction
   lp=0
   x1p=1.
   do ip=0,iord
    x12p=x1p
    do jp=0,iord-ip*mhalf
     ...
     do kp=0,iord-(ip+jp)*mhalf
      lp=lp+1
      z0(1:nin,lp)=kp*(kp-1)*x123p(1:nin)
      if(kp.gt.1) &
        x123p(1:nin)=x123p(1:nin)*  &
                  (x3in(1:nin)-x3grid(max(1,i3ref(1:nin))))
     end do
   ...
   do k=1,lbig
    ...
    do j=1,lbig
     do n=1,nin
      if(iflag(n).gt.0) wgtsx33(n,k)=wgtsx33(n,k)+z0(n,j)*tl(itype(n),j,k)
     end do
    end do
   end do
   ...
   do k=1,lbig
    do i=1,nin
     wgtsx33(i,k)=wgtsx33(i,k)*du3dx3(i)**2+wgtsu3(i,k)*du3dx33(i)
    end do
   end do
  end if
  ...
  if(ndx3.eq.1.or.ndx33.eq.1) then
   deallocate(wgtsu3)
  end if

return
end subroutine simpin3f
