...
int main(int argc, char *argv[])
{
  ...
  if (nin == NULL || nout == NULL || nlin == NULL ) {
    ...
    if (nin == NULL ) printf("No input file\n");
    if (nout == NULL) printf("No output file\n");
    if (nlin == NULL) printf("No land file\n");
    return -1;
  }
...
/*        printf("i, j, smap %3d %3d %d\n", i, j, tc); */
        fprintf(sout,"%c", (unsigned char) (5 + (fmin(104, fmax(0, tc ) ) - 15) 
                            / cres +  cbase) );
      }
    }
    fprintf(sout,"\",\n");
