...
C$$$
      ...
      INTEGER JPDS(LPDS),JGDS(LGDS),KPDS(LPDS),KGDS(LGDS)
      LOGICAL LB(JF)
      REAL F(JF)
...
C  SEARCH FOR REQUEST
      IF(J.GE.0.AND.LUGI.EQ.LUX) THEN
        ...
        DOWHILE(LGRIB.EQ.0.AND.KJ.LT.NNUM)
          ...
          IP=1
          DOWHILE(LMATCH.AND.IP.LE.LPDS)
            LMATCH=JPDS(IP).EQ.-1.OR.JPDS(IP).EQ.KPDS(IP)
            IP=IP+1
          ENDDO
          IF(LMATCH.AND.JPDS(3).EQ.255) THEN
            CGDS=CBUF((KJ-1)*NLEN+54:(KJ-1)*NLEN+95)
            KPTR=0
            CALL FI633(CGDS,KPTR,KGDS,IRET)
            IP=1
            DOWHILE(LMATCH.AND.IP.LE.LGDS)
              LMATCH=JGDS(IP).EQ.-1.OR.JGDS(IP).EQ.KGDS(IP)
              IP=IP+1
            ENDDO
          ENDIF
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
C  READ AND UNPACK GRIB DATA
      ...
      ELSE
        IRET=96
        PRINT *,'Error reading index file, index unit = ',LUGI
      ENDIF
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      RETURN
      END
