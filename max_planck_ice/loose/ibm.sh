...
#Restart/nudging control
SSTASSIM=${SSTASSIM:-.FALSE.}
ICEASSIM=${ICEASSIM:-.TRUE.}
export SSTASSIM ICEASSIM
...
#Assumed to be ibm for now
  RUNDIR=/gpfstmp/wd21rg
  FBASE=/emc1/ice/mrffix/
  EXDIR=/u/wx21rg/nfsuser/icemodel/execs
  REFDIR=/u/wx21rg/nfsuser/icemodel/running
  SSDIR=/nfsuser/g01/marine/climate/
  LANDDIR=/nfsuser/g01/marine/climate/
  OUTBASE=/u/wx21rg/nfsuser/iceout2
  OUTNAUGHT=/emc1/ice/model.icerun
...
#Prepare for the forecast/hindcast cycle, establish the dates:
tag=${base}
tagm=`expr $tag - 1`
tagm=`dtgfix3 $tagm`
...
fcsttimes='12 24 36 48 60 72 84 96 108 120 144 168 192 204 216 240 252 264 276 288 300 312 324 336 348 360 372 384'
...
#Required directories, bail if not present
...
#######################################################################
#time  $EXDIR/getmet.sh ${CC}$tag
#time  $EXDIR/getmet.sh $tag
...
if [ $HARDSTART = 'yes' ] ; then
  ...
  do
    ...
    $EXDIR/ic.sh $ICEASSIM $SSTASSIM .FALSE.  $ANUDGE 0.5 0.5
...
do
  ...
  if [ -s $HINDBASE/$pole/restart.$pole ] ; then
    ...
    $EXDIR/ic.sh $ICEASSIM $SSTASSIM .FALSE.  0.875 0.5 0.6
  ...
  do
    ...
    if [ ! -s ${FBASE}/met.${pole}.${tag}${time} ] ; then
      if [ $cycletype = hcst ] ; then
        if [ ${#tag} -eq 6 ] ; then
          ${EXDIR}/met.sh ${FBASE}/flxf06.${CC}${tag}${time}
        else
          ${EXDIR}/met.sh ${FBASE}/flxf06.${tag}${time}
        fi 
      else 
        if [ ${#tag} -eq 6 ] ; then
          ${EXDIR}/met.sh ${FBASE}/flxf${time}.${CC}${tag}00
        else
          ${EXDIR}/met.sh ${FBASE}/flxf${time}.${tag}00
        fi
      fi
    ...
    if [ -s $OUTBASE/$pole/restart.$pole ] ; then
      ...
      if [ $time -eq 00 ] ; then
        ...
        $EXDIR/ic.sh $ICEASSIM $SSTASSIM .TRUE.  $ANUDGE 0.25 0.1
      else
        echo calling off time ic check
        $EXDIR/ic.sh $ICEASSIM $SSTASSIM .TRUE.  $OFFNUDGE 0.25 0.1
      fi
      ...
      $EXDIR/ic.sh $ICEASSIM $SSTASSIM .FALSE.  0.875 0.5 0.1
    ...
    #gprof seaice.${pole}.$dttag >> ~/nfsuser/icemodel/running/stats.$pole 
    #./seaice.${pole}.$dttag 
    #-------------------------------------------------------
    ...
    if [ $cycletype = hcst ] ; then
# Added to preserve restart files
      cp RESTARTn $HINDBASE/$pole/restart.$tag.$pole
