...
CD      SINWAT=-0.4226E+00
CD      COSWAT=0.9063E+00
...
CD      COSGAM=.9135
      READ (21, 9002) RTC
      RTC = RTC * 8.64E4/DT
CD      RTC=10.0*86400.0/DT
      READ (21, 9002) STC
      STC = STC * 8.64E4/DT
CD      STC=300.0*86400.0/DT
      READ (21, 9002) QTOC
CD      QTOC=2.00
C-----------------------------------------------------------------------
...
C**CHOICE OF ABL ACC. TO ABLFIX AND ECMTYP.  According to SURTYP
      IF (SURTYP .EQ. 0.) THEN
       ...
       ELSE
        ...
        STOP
      ENDIF
CD      IF (ABLFIX.EQ.1.) THEN
CD       CALL OBUDGET(RA,QT, LWDN, SWDN)
CD      ELSE
CD       IF (ECMTYP.EQ.1.) THEN
CD        CALL ECMBDO(RA,QT, LWDN, SWDN)
CD       ELSE
CD        CALL EKMAO(RA,QT, LWDN, SWDN)
CD       END IF
CD      END IF
C-----------------------------------------------------------------------
...
C**CHOICE OF ABL ACC. TO ABLFIX AND ECMTYP:
      ...
      ENDIF
CD       IF (ABLFIX.EQ.1.) THEN
CD        CALL BUDGET(SH,TICM(0,0,K), LRHS,K, LWDN, SWDN)
CD       ELSE
CD        IF (ECMTYP.EQ.1.) THEN
CD         CALL ECMBDI(SH,TICM(0,0,K), LRHS,K, LWDN, SWDN)
CD        ELSE
CD         CALL EKMAH(SH,TICM(0,0,K), LRHS,K, LWDN, SWDN)
CD        END IF
CD       END IF
...
C-----------------------------------------------------------------------
       ...
       ENDIF
CD       ABLFIX = 0.
CD       CDWIN = CDWIN*ABLFIX +
CD     1     0.25*(CD(I,J)+CD(I-1,J)+CD(I,J-1)+CD(I-1,J-1))*(1.-ABLFIX)
