...
C     LWUP is not used by stossel model.  It is included here for the
C       Navy model, and written out.
      REAL LWUP(0:L,0:M), LWDN(0:L,0:M), SWDN(0:L,0:M)
C=======================================================================
      INTEGER I, J, POLE, INTYP
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      IF (NRREC .NE. 0) THEN
       DO 420 K=1,NRREC
CD       READ(9,901,ERR=100)((U(I,J,1), V(I,J,1), I=1,L), J=1,M)
CD       READ(9,901,ERR=100)((H(I,J,1), A(I,J,1), I=0,L), J=0,M)
CD       READ(9,901,ERR=100)((HSN(I,J,1), TICE(I,J), I=0,L), J=0,M)
CD       READ(9,901,ERR=100)((QT(I,J), QS(I,J), QH(I,J), I=0,L), J=0,M)
CD       READ(9,901,ERR=100)((QTB(I,J), QSB(I,J), QHB(I,J), I=0,L), J=0,M)
CD       READ(9,901,ERR=100)((QDT(I,J), QDS(I,J), I=0,L), J=0,M)
CD       READ(9,901,ERR=100)(((TICM(I,J,N), I=0,L), J=0,M), N=1,NLEVEL)
         READ (9) U
         READ (9) V
         READ (9) H
         READ (9) A
...
C-----------------------------------------------------------------------
        DO 1000 J = 1, M-2
          DO 1100 I = 2, LM
            IF (NRREC .EQ. 0) QTB(I,J) = -1.5
            IF (NRREC .EQ. 0) QSB(I,J) = 34.7
            UWAT(I,J) = 0.0
            VWAT(I,J) = 0.0
 1100     CONTINUE
 1000   CONTINUE
...
CD      READ(12,802,ERR=100)((UWAT(I,J), VWAT(I,J), I=2,LM), J=1,MM)
        IF (NRREC .EQ. 0) READ (12) QTB
        IF (NRREC .EQ. 0) READ (12) QSB
...
C-----------------------------------------------------------------------
   ...
   30 CONTINUE
      ...
      END
C=====================================================================--
      SUBROUTINE getflx(slpf, swdn, lwup, lwdn, t2, q2, uten, vten, 
     1    cloud, mask, iunit )
C=====================================================================--
C  Programmed by:
...
C     Arguments to the GRIB extraction routine W3AI08.
      ...
      LOGICAL lbms(ijdim), lbms2(ijdim2)
      REAL    grid(ijdim), grid2(ijdim2)
      CHARACTER*1 pack(maxgrb), pack2(maxgrb2)
...
CD       WRITE(14, 9101) ((U(I, J, LNEW), V(I, J, LNEW), I=1, L), J=1, M)
CD       WRITE(14, 9101) ((H(I, J, LNEW), A(I, J, LNEW), I=0, L), J=0, M)
CD       WRITE(14, 9101) ((HSN(I, J, LNEW), TICE(I, J), I=0, L), J=0, M)
CD       WRITE(14, 9101) ((QT(I, J), QS(I, J), QH(I, J), I=0, L), J=0, M)
CD       WRITE(14, 9101) ((QTB(I, J), QSB(I, J), QHB(I, J), I=0, L),
CD     1                                                    J=0, M   )
CD       WRITE(14, 9101) ((QDT(I, J), QDS(I, J), I=0, L), J=0, M)
CD       WRITE(14, 9101) (((TICM(I, J, K), I=0, L), J=0, M), K=1, NLEVEL)
        REWIND (14)
        WRITE (14) U
        WRITE (14) V
        WRITE (14) H
...
C     For Stossel model, assign i=0, j=0 points
 ...
 3100 CONTINUE
     ...
     1  ps, precip, acl, lwup, lwic, swic, step)
C=====================================================================--
C  Programmed by:
C     Robert W. Grumbine         NMC  Camp Springs, MD          Dec. '92
C  Purpose:
...
C     Read in data here
      ...
      iunit = 50+step
      CALL getflx(slpf, swdn, lwup2, lwdn, t2, q2, uten, vten, 
     1            cloud, mask, iunit)
      CLOSE (iunit)
...
C     Interpolate the data fields
      CALL terp(slpf, ps, pole, proj)
 
      CALL terp(t2, t850, pole, proj)

      CALL terp(swdn, swic, pole, proj)

      CALL terp(lwdn, lwic, pole, proj)

      CALL terp(lwup2, lwup, pole, proj)
      
      CALL terp(q2, rh850, pole, proj)
      DO 1200 j = 1, M
        DO 1300 i = 1, L
          rh850(i,j) = rh850(i,j)/1.E4           ! q2 in 0.1 g/kg
     1                *ps(i,j)*EPSI              ! ps in hpa
     2                 /(W3FA09(t850(i,j))*10.)  ! qsat in kpa
     3     *100.
          IF (rh850(i,j) .GT. 100.) THEN
            PRINT *,' rh too large ',i, j,
     1                rh850(i,j), ps(i,j), t850(i,j)
            rh850(i,j) = 100.
          ENDIF
...
C-----------------------------------------------------------------------
      ...
      DO 84 I=0,L
       ...
       FLSE(I,J)=FLSE1(K)
       FLLA(I,J)=FLLA1(K)
CD       IF (ABS(FH(I,J))*CLO .GT. 350./2.) THEN
CD         WRITE (*,9001) I, J, FH(I,J)*CLO, FLSE(I,J), FLLA(I,J), 
CD     1     D3*T1(K)**4 - SWDN(I,J)-LWDN(I,J)
CD       ENDIF
   84 CONTINUE
CD      PRINT *,'leaving 84, budget'

   87 CONTINUE

 9001 FORMAT (1x, 2I3, 4F8.1, ' Ice')
...
C-----------------------------------------------------------------------
      ...
      DO 81 I=0,L
       ...
       FLSE(I,J)=FLSE1(K)
       FLLA(I,J)=FLLA1(K)
CD       IF ( ABS(FH(I,J))*CLO .GT. 1500.) THEN !W/m2
CD         WRITE (*,9001) I, J, FH(I,J)*CLO, FLSE(I,J), FLLA(I,J), 
CD     1     D3*QT1(K)**4 - SWDN(I,J) - LWDN(I,J)
CD       ENDIF
   81 CONTINUE

CD      PRINT *,'leaving 84, obudget'

 9001 FORMAT (2I3, 4F8.1, ' Ocean')
