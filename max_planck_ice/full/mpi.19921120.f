...
CBG   SPACE added to make size of /WORK/ compatible with definition in
CBG     GROWTH
...
CBG       QT(I,J)=QT(I,J)-(QT(I,J)-TFREZ+TMP3(I,J)*CLO/CC/QH(I,J))
CBG     1         *FLAGI(I,J)*OM(I,J)
...
CBG   Index range redefined by BG to be 0:L, 0:M for compatibility
CBG     with definitions in the rest of the program (particularly,
CBG     ICEMODEL.
...
CBG   SPACE added to make size of /work/ compatible with definition
CBG     in GROWTH
...
C=======================================================================
      COMMON/DRV/DXSQ,DYSQ,SX2,SY2,SXY
      COMMON/COORD/PM(0:L,0:M),PN(0:L,0:M),DNDX(L,M),DMDY(L,M)
CBG   Index range redefined to be 0:L, 0:M by BG for compatibility
CBG     with declaration in ICEMODEL.
CBG   Dummy array SPACE(1:L,1:M,11) Added to make size consistent
CBG     with INITREL declaration of /WORK/
...
CBG  Original line, debug fix by Stossel 6/18/92
CBG        TMP(I,J) = SH(I,J)
   10 CONTINUE
C-----------------------------------------------------------------------
...
CBG  Line fixed in debugging by Stossel 18 June 1992.
CBG      RH(I,J)=SH(I,J)-TMP2(I,J)*RHOSNO/RHOICE
...
C-----------------------------------------------------------------------
CBG     NOLRREC was uninitialized.  Set to zero by BG. 4/22/92
        NOLRREC = 0
...
C=======================================================================
      ...
      COMMON/VEL/U(L,M,3),V(L,M,3)
      COMMON/THCK/H(0:L,0:M,2),A(0:L,0:M,2),HSN(0:L,0:M,2)
CBG   Index ranges set to 0:L, 0:M by BG for compatibility with 
CBG     ICEMODEL declaration.
...
C-----------------------------------------------------------------------
CBG      PRINT *,'Deleting the acceleration terms  BG'
CBG      PRINT *,'DT = ',DT
      DO 20 J=2,MM
      DO 20 I=1,L
CBG        PRINT *,'in loop, i, j= ',i,j
CBG        IF (BU(I,J) .NE. 0 .AND. BV(I,J) .NE. 0) THEN
CBG          PRINT *,'BU, BV = ',I, J, BU(I,J),BV(I,J), AMAS(I,J),
CBG     1   1./BU(I,J) - AMAS(I,J)/DT
CBG        ENDIF
CBG    The IF test is added by BG to avoid division by zero. 
       IF(BU(I,J) .NE. 0)
...
C-----------------------------------------------------------------------
CBG      PRINT *,'Calling RELAX  BG'
      CALL RELAX(1,2)
CBG      PRINT *,'Looping after RELAX  BG'
      DO 30 J=1,M
      DO 30 I=1,L
       U(I,J,1)=U(I,J,2)
       V(I,J,1)=V(I,J,2)
...
C=======================================================================
      ...
      COMMON/COORD/PM(0:L,0:M),PN(0:L,0:M),DNDX(L,M),DMDY(L,M)
      COMMON/VEL/U(L,M,3),V(L,M,3)
CBG   Index range reset to 0:L, 0:M by BG for compatibility with 
CBG     declaration in ICEMODEL.
CBG   SPACE added to make size of /work/ consistent with declaration
CBG     in INITREL
...
C=======================================================================
     ...
     1,UG(0:L,0:M),TA(0:L,0:M),RPREC(0:L,0:M)
      ...
      COMMON/FLUX/FLSE(0:L,0:M),FLLA(0:L,0:M),WMUE1(0:L,0:M)
     1,UST1(0:L,0:M),TMPL1(0:L,0:M)
CBG   SPACE added to make size of /work/ compatible with definitions
CBG     in GROWTH.
...
C=======================================================================
      COMMON/THCK/H(0:L,0:M,2),A(0:L,0:M,2),HSN(0:L,0:M,2)
      COMMON/PRESS/P(L,M)
CBG   Index range reset by BG to be 0:L, 0:M by BG for compatibility
CBG     with declaration in ICEMODEL.
...
C-----------------------------------------------------------------------
      ...
      DO 50 I=0,L
       ...
       A1=EXP(-TMP(I,J)/QHW)
       FNOT=2.*QW(I,J)*A1+9.81*QRHO(I,J)*TMP(I,J)
       F1=-2.*QW(I,J)*A1/QHW+9.81*QRHO(I,J)
CBG    BG change to make term (1.-FLAG) rather than 1-flag.
CBG      For compatibility with lower precision machines.
...
C=======================================================================
      COMMON/IPARM/H0,HNU,HNU2,ARMIN,ARMAX,HMIN
      COMMON/DRV/DXSQ,DYSQ,SX2,SY2,SXY
CBG   Add dummy(2*l*m) in order to keep common block aligned with
C       other definitions. 7/7/92.
...
CBG   Index range reset to 0:L,0:M by BG for compatilibity with 
CBG     declaration in ICEMODEL.
CBG   SPACE added to make size of WORK agree with declaration in
CBG     INITREL
...
CBG   Index range reset to 0:L,0:M by BG for compatibility with
CBG     declaration in ICEMODEL
CBG   SPACE added to make size consistent with declaration of /work/
CBG     in INITREL
      COMMON/WORK/TRK(1:L,1:M),E11(1:L,1:M),E22(1:L,1:M),E12(1:L,1:M)
     1 ,SPACE(1:L,1:M,8)
C=======================================================================
...
C=======================================================================
      COMMON/IPARM/H0,HNU,HNU2,ARMIN,ARMAX,HMIN
      COMMON/DRV/DXSQ,DYSQ,SX2,SY2,SXY
CBG   Add DUMMY to occupy space and keep COMMON/COORD/ same size as
CBG     other routines. 7/7/92.
      COMMON/COORD/PM(0:L,0:M),PN(0:L,0:M),DUMMY(2*L*M)
      DIMENSION TMP(0:L,0:M),DFX(0:L,0:M),DFY(0:L,0:M),RH(0:L,0:M),
     1H(0:L,0:M,2)
C=======================================================================
...
C     Local variables
     ...
     6 STICM(0:L,0:M)
      REAL FLAG, FLAG1, HWX, ZQ, HEX, HEX1, HEX2, HEXI
...
C  SUMMATION OF VARIABLES FOR CONTINUOUS STATISTICS
C-----------------------------------------------------------------------
CD       PRINT *,'Before do 130'
       DO 130 J=0,M
       DO 130 I=0,L
        QTMA(I,J)=QTMA(I,J)+QTM(I,J)
        SHA(I,J)=SHA(I,J)+SH(I,J)*CLO/DT
  130  CONTINUE
CD       PRINT *,'Exited do 130 '
C**NEXT COMPUTATIONS DO NOT START BEFORE SPECIFIED TIME STEP:
       IF(IIC.GE.NFLD)THEN
CD         PRINT *,'Inside if iic >=nfld '
C-----------------------------------------------------------------------
C  SELECT DAYS FOR INSTANTANEOUS ICE COMPACTNESS PLOTS
C-----------------------------------------------------------------------
...
C  AVERAGE OCEANIC AND ATMOSPHERIC HEAT FLUXES
C-----------------------------------------------------------------------
CD        PRINT *,'Inside if mod(iic,nstat)=0 '
        Z1=FLOAT(NSTAT)
        DO 135 J=0,M
        DO 135 I=0,L
...
CD         PRINT *,'before do 390 '
  ...
  390   CONTINUE
       END IF
CD         PRINT *,'after do 390, iic, nrst = ',IIC, NRST
C-----------------------------------------------------------------------
C  WRITE OUT RESTART DATA
C-----------------------------------------------------------------------
      IF(MOD(IIC,NRST).EQ.0)THEN
CD        PRINT *,'Writing out restart data '
...
C=======================================================================
      DO 2 N=1,K
       IF(RI(N).GE.0.)GOTO1
CBG Original, incorrect version, used for comparison with documented run.
CBG       ZA=MAX(30.,((PA1(N)-100000.)*8.))
...
C  DAILY MEAN COSINE OF ZENITH DISTANCE:
   ...
   2  CONTINUE
      RETURN
      END
      SUBROUTINE twoget(pole, sigma, proj, u850, v850, t850, rh850,
...
C     Arguments to return
      REAL u850(nx, ny), v850(nx, ny)
      REAL rh850(0:nx, 0:ny), ps(0:nx, 0:ny)
      REAL t850(0:nx, 0:ny), precip(0:nx, 0:ny), cloud(0:nx, 0:ny)
      INTEGER step
...
C     Begin the routine ____________________________________
      ...
      iunit = 31
      REWIND (31)
      CALL getsig(slp, temp, ztopo, qsig, iunit, sigma)
      iunit = 50+step
...
C     Carry out the wrap-around
      DO 3000 j = 1, ny
        ps(0,j)   = ps(nx-2,j)
        t850(0,j) = t850(nx-2,j)
        rh850(0,j) = rh850(nx-2,j)
        precip(0,j) = precip(nx-2,j)
        cloud(0,j) = cloud(nx-2,j)

        ps(1,j)   = ps(nx-1,j)
        t850(1,j) = t850(nx-1,j)
        rh850(1,j) = rh850(nx-1,j)
        precip(1,j) = precip(nx-1,j)
        cloud(1,j) = cloud(nx-1,j)
        u850(1,j) = u850(nx-1,j)
        v850(1,j) = v850(nx-1,j)

        ps(nx,j)     = ps(2,j)
        t850(nx,j)   = t850(2,j)
        rh850(nx,j)  = rh850(2,j)
        precip(nx,j) = precip(2,j)
        cloud(nx,j)  = cloud(2,j)
        u850(nx,j)   = u850(2,j)
        v850(nx,j)   = v850(2,j)
 3000 CONTINUE
