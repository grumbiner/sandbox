...
C-----------------------------------------------------------------------
     ...
     1 PN, PM, OM, NOUT, IOUT, JOUT)
CD      PRINT *,'finished output smoothing '
C-----------------------------------------------------------------------
...
CD      PRINT *,'Finished cyclic calls '
...
CD      PRINT *,'Finished setting up the output '
C-----------------------------------------------------------------------
...
CD       DO 125 J=1,MM
CD       DO 125 I=1,LM
CD        QV(I,J)=(.25*(SQRT(U(I,J,LNEW)**2+V(I,J,LNEW)**2)
CD     1               +SQRT(U(I+1,J,LNEW)**2+V(I+1,J,LNEW)**2)
CD     2               +SQRT(U(I,J+1,LNEW)**2+V(I,J+1,LNEW)**2)
CD     3               +SQRT(U(I+1,J+1,LNEW)**2+V(I+1,J+1,LNEW)**2))
CD     4               +.01)**3*OM(I,J)
CD  125  CONTINUE
C-----------------------------------------------------------------------
...
CD      INTEGER K1(0:L,0:M), K2(0:L,0:M)
C=======================================================================
      INTEGER I, J
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      IF (PTYPE .EQ. 1 .OR. PTYPE .EQ. 2) THEN
       ...
       ELSE IF (PTYPE .EQ. 3) THEN
        READ(10,301) ((VM(I,J), I=1,L), J=1,M)
        READ(10,302) ((HM(I,J), I=0,L), J=0,M)
        READ(10,302) ((OM(I,J), I=0,L), J=0,M)
CD        DO 1000 J = 0, M
CD          DO 1100 I = 0, L
...
CD 1100     CONTINUE
CD 1000   CONTINUE
CD        WRITE(20,303) ((K1(I,J), I=0,L), J=0,M)
CD        WRITE(20,303) ((K2(I,J), I=0,L), J=0,M)
CD        PRINT *, OM
       ELSE
...
C-----------------------------------------------------------------------
      IF (PTYPE .EQ. 2 .OR. PTYPE .EQ. 1) THEN
        CALL twoget(2, 1.0, UWIN, VWIN, TAIR, TD, PA, RPREC, ACL,
     1     LWDN, SWDN, IIC)

        ELSE IF (PTYPE .EQ. 3) THEN
CD        PRINT *,'Calling thregt '
        CALL thregt(2, 1.0, UWIN, VWIN, TAIR, TD, PA, RPREC, ACL, 
     1       LWDN, SWDN, IIC)
...
CD      PRINT *,'Opened fixed input files and the output files.'
C-----------------------------------------------------------------------
...
CD       WRITE (15) SINPHI
CD       WRITE (15) FM
CD      PRINT *,'Finished setting up the coriolis grid.'
C-----------------------------------------------------------------------
...
C        Polar Stereographic
  ...
  213   CONTINUE
      ...
      ENDIF
CD      PRINT *,'Set up the map factors '
CD      WRITE (15) PM
CD      WRITE (15) PN
       ...
       lengrb = LENGTH(iunit)
        IF (rcrgrb .EQ. -1.) THEN
         ELSE
          ...
          STOP
        ENDIF
CD        PRINT *,'preparing to call w3ai08 '
...
C       Now finished de-gribbing.
 1000 CONTINUE
CD      PRINT *,'preparing to return from getflx '
...
CD      PRINT *,'returned from sphert calls '
...
CD      PRINT *,'finished call to terp(ps) '
 
      CALL terp(tempf, t850, pole, proj)
CD      PRINT *,'finished call to terp(t850) '

      CALL terp(swdn, swic, pole, proj)
CD      PRINT *,'finished call to terp(swic) '

      CALL terp(lwdn, lwic, pole, proj)
CD      PRINT *,'finished call to terp(swic) '
      
CD  Alternate - use flux file for t      CALL terp(t2, t850, pole, proj)
  
      CALL terp(q2, rh850, pole, proj)
CD      PRINT *,'finished call to terp(rh850) '
      DO 1200 j = 1, M
        DO 1300 i = 1, L
 ...
 1300   CONTINUE
 1200 CONTINUE
CD      PRINT *,'finished rh computation'

      CALL terp(cloud, acl, pole, proj)
CD      PRINT *,'Finished call to terp(cloud)'

      CALL terp(uten, tempa, pole, proj)
CD      PRINT *,'Finished call to terp(uten)'

      CALL terp(vten, tempb, pole, proj)
CD      PRINT *,'Finished call to terp(vten)'
...
C     Average u, v onto the half-grid points.
 ...
 2100   CONTINUE
 2000 CONTINUE
CD      PRINT *,'finished derivation of u, v '
...
CD      PARAMETER (DX     = DXDEG*DLON )
CD      PARAMETER (DY     = DXDEG*DLAT )
      PARAMETER (DT     = 4.32E4    )
      PARAMETER (PTYPE  = 3         )
      PARAMETER (POLEI  = 28.)
      PARAMETER (POLEJ  = 26.)
