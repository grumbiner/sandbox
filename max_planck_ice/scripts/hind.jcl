#!/bin/sh
#Hindcast, requires that the following be set in the environment:
...
if [ -z $HINDBASE ] ; then
  echo No hindcast directory named
  exit -1 
fi
if [ -z $SSDIR ] ; then
...
if [ -z $tag ] ; then
  echo No date specified to run hindcast for
  exit -1
fi 
...
echo `time  $EXDIR/gethind.jcl` gethind.jcl
