...
!Implement a simple version of cressman (1960) filtering/objective analysis
      ...
      INTEGER maxobs
      PARAMETER (maxobs = 100*1000*1000)
...
! Main, slow, loop:
! spiral outward from point nearest the given (lat2, lon2) until
!  all points are farther away than distlimit
      SUBROUTINE spiral(firstlat, firstlon, dlat, dlon, lat2, lon2, 
     1        distlimit, obs, tempor, maxincr, 
     2        grid_inc, grid_weight, nx, ny)
      IMPLICIT none
      REAL firstlat, firstlon, dlat, dlon
      REAL lat2, lon2, distlimit
      INTEGER nx, ny
      REAL maxincr, obs
      REAL tempor(nx, ny), grid_inc(i,j), grid_weight(i,j)

      REAL i,j, centeri, centerj
      REAL d, w, incr
