...
// Inherited over to working on reanalysis grids (t62 gaussian)
template <class T>
class gaussian62 : public grid2<T> {
  public:
     gaussian62(void);
};
template <class T>
gaussian62<T>::gaussian62(void):nx(194),ny(62) {
}
...
// Data to read in:
  gaussian62<int> count, ndays;
  gaussian62<float> high, low, dhigh, dlow;
  gaussian62<double> sx, sx2, sx3, sx4;
// Input mask (land, ocean, inland water):
  gaussian62<unsigned char> orimask;

// Harmonic analyses:
  mvector<gaussian62<double> > ampl(NPER), phase(NPER);
  mvector<gaussian62<double> > sum_sin(NPER), sum_cos(NPER);

// Output mask (land, ocean, inland water):
  gaussian62<unsigned char> outmask;

// Output diagnostics
  gaussian62<double> mean, var2, var3, var4, skew, kurt;
...
// Local and flag values
  ...
  ijpt loc, rloc, tloc;
  latpt ll;
  float tmp, offset = 273.15;
  float tcrit = offset + 2.00, tcrit2 = offset - 9.99;
  double rm3 = 0, rm4 = 0;
  int tcount, range = 1;
  int i, n;
...
// Ocean
      ...
      if (low[loc] >= offset + 26.0) {  // Hurricane magic SST
        outmask[loc] = 158;
      }
      else if (low[loc] >= offset + 23.0) {
        outmask[loc] = 159;
      }
      else if (low[loc] >= offset + 20.0) {
        outmask[loc] = 160;
      }
      else if (low[loc] >= offset + 15.0) {
        outmask[loc] = 161;
      }
      else if (low[loc] >= offset + 10.0) {
        outmask[loc] = 162;
      }
      else if (low[loc] >= offset +  5.0) {
        outmask[loc] = 163;
      }
      else if (low[loc] >= offset +  2.0) {
        outmask[loc] = 164;
      }
      else if (low[loc] >= offset +  -1.0) {
        outmask[loc] = 165;
      }
      else if (low[loc] >= offset +  -2.5) {
        outmask[loc] = 166;
      }
      else {
...
// Inland water
      ...
      if (low[loc] >= offset + 8.0) {
        outmask[loc] = 170;
      }
      else if (low[loc] >= offset +  4.0) {
        outmask[loc] = 171;
      }
      else if (low[loc] >= offset +  1.0) {
        outmask[loc] = 172;
      }
      else if (low[loc] >  offset +  0.0) { //Note strictly greater, fresh water
        outmask[loc] = 173;
      }
      else if (low[loc] >= offset +  -2.0) {
        outmask[loc] = 174;
      ...
      if (outmask[loc] == undefined) {
        ...
        if (tcount > 0) {  // then we managed to find a fix in range
          ...
          printf("iter %2d latefix with %f count %1d at %7.3f %7.3f\n",
                 iter, outmask[loc], tcount, ll.lat, ll.lon );
        }
      }
    }
    }
...
// Finally, make output files and compute some integrals
  ...
  printf("mean max min avg %6.2f %6.2f %6.2f\n",
                      mean.gridmax(), mean.gridmin(), mean.average());
  printf("low  max min avg %6.2f %6.2f %6.2f\n",
                      low.gridmax(), low.gridmin(), low.average());
  printf("high max min avg %6.2f %6.2f %6.2f\n",
                      high.gridmax(), high.gridmin(), high.average());
