...
C     Variables for the inversion/smoothing
      ...
      OPEN (11, FILE='texttime.9601', FORM='FORMATTED', STATUS='OLD')
      DO 100 j = 1, ny
      DO 100 i = 1, nx
        READ(11, 9200) i1, i2, epsilon(i,j)
  100 CONTINUE
 9200 FORMAT (I3,1x,I3, F7.2)
...
C  Now begin the actual work of performing the inversion
 ...
 2101     CONTINUE
          ...
          CALL wght1(y, a, b, c, r, dt, epsilon(i,j), nhi, w, bctype)
CD          CALL wght1(y, a, b, c, r, dt, 1., nhi, w, bctype)
...
C     Write out results
 ...
 9004   FORMAT ("imp.96010",I1)
        ...
        IF (k .EQ. 15) THEN
          DO 3900 j = 1, ny
          DO 3900 i = 1, nx
            WRITE (*,9100) i, j, atmp(i,j)
 3900     CONTINUE
 9100     FORMAT (2I4, F7.1)
        ENDIF
        CLOSE (10)
 3000 CONTINUE
...
C     Bob Grumbine 7 April 1994.
      ...
      DO 1000 j = 2, N
        ...
        bet    = b(j)-a(j)*gam(j)
        IF (bet .EQ. 0.) THEN
CD          PRINT *,'Beta = 0 in inversion, cannot invert'
          u(j) = 0.0
CD          STOP
         ELSE
