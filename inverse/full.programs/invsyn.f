...
CD 9999   CONTINUE
 9002 FORMAT (I4, 3F7.3)
        ...
        CALL correl(x, z, n, rxy, d1, d2, d3, d4)
        WRITE (*,9003) rxy, d1, d2, d3, d4, FLOAT(m)/FLOAT(n)
C       Transfer the possible points to x, y, z from the
C         weight term (i.e., drop gaps) in the w2, y2 series.
        npts = 0
        DO 2000 i = 1, m
          IF (w2(i) .NE. 0) THEN
            npts = npts+1
 ...
 2000   CONTINUE
        ...
        WRITE (*,9003) rxz, d1, d2, d3, d4, FLOAT(npts)/FLOAT(m)
        CALL correl(x, y, npts, ryz, d1, d2, d3, d4)
        WRITE (*,9003) ryz, d1, d2, d3, d4, FLOAT(npts)/FLOAT(m)
        sxy = sxy+rxy
        sxz = sxz+rxz
        syz = syz+ryz
...
C     Currently ignoring. 11-24-93 BG      
 ...
 1000 CONTINUE
      ...
      RETURN
      END
      SUBROUTINE mksmpl(x, n, tzero, tline, thi)
      INTEGER n
      REAL x(n)
      INTEGER i, j
      INTEGER tzero, tline, thi
