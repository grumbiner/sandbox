...
C                            correlated errors.
C     12-01-93  Version 6b: Compute autocorrelation of vectors.
C
C     Subroutines:
...
C       anoise - 11-30-93 Add autocorrelated noise to a vector.
...
C       Autocor - 12-01-93 Compute autocorrelations.
...
C       Generate the noisily sampled observed series
        CALL veceq(x, y, n)
CD        CALL mkrand(x, y, n, rsize, seed)
        IF (alpha .NE. 0.) 
     1          CALL anoise(y, n, rsize, alpha, seed)
        CALL sample(w, n, gaps, seed)
CD        PRINT *,'done sampling the series' 
...
C       Synthesize a series for gappy series
 ...
 9003 FORMAT (6F7.3, I4)
      ...
      STOP
      END
      SUBROUTINE autocor(t, x, n)
C     Compute autocorrelations
      INTEGER n
      REAL x(n), y(2000), z(2000), t(n)
      INTEGER i, lag
 ...
 1010   CONTINUE
        ...
        WRITE (*,9001) lag, r2
 1000 CONTINUE
 9001 FORMAT (I3, 3F7.3)

      RETURN
      END
      SUBROUTINE anoise(y, n, rsize, alpha, seed)
C     Create autocorrelated noise with magnitude rsize and persistence
C       alpha.  Modifies original series.  11-30-93.
      INTEGER n, seed
      REAL y(n), rsize, alpha
      ...
      DO 1000 i = 2, n
        noise = rsize*(.5-RAN2(seed))*2
        y(i) = y(i) + alpha*onoise + (1-alpha)*noise
        onoise = alpha*onoise+(1-alpha)*noise
 1000 CONTINUE
...
C     12-01-93 Autocorrelation not dealt with by gamma.  Dropped.
