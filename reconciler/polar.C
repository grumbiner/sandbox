...
//         problems.
 ...
 // Now incorporate the relevant parts of field into tot_field
    ...
    for (tloc.i = 0; tloc.i < field.xpoints(); tloc.i++ ) {
      ...
      #if defined(resops) || defined(psgrids)
        if (ll.lon - tot_field.firstlon() > 360.) {
        printf("minlon reset %f %f\n",ll.lon, tot_field.firstlon());
      }
      #else
        printf("error, polar.C only works for resops or psgrids\n");
        exit(1);
      #endif
        fflush(stdout);
        ll.lon -= 360.;
...
//    if (south == -90.0) {
//      mvector<latpt> tloc(gshhsheader.n *2 );
//      int ti;
//      latpt tmplat;
...
//      for (ti = 0; ti < locations.xpoints() ; ti++) {
//        printf("ti = %d\n",ti); 
//        tloc[ti + locations.xpoints() ].lon = locations[ti].lon;
//        tloc[ti + locations.xpoints() ].lat = -89.5;
//      }
//
//      locations.resize(gshhsheader.n *2);
//      locations = tloc;
//      for (ti = 0 ; ti < locations.xpoints(); ti++) {
...
// Loop forward until we find a point on the grid.
  ...
  ij1.i = -1;
  ij1.j = -1;
  while ( ( (ij1.i == -1. && ij1.j == -1.) ||
            (ij1.i == -2. && ij1.j == -2.) )   // -2,-2 is off-grid for psgrids
           && i < nx - 1 ) {
...
////   algorithm.
  if (field_nx*field_ny > 1.25e5) {
    ...
    exit(1);
    #endif
    grid2_base<latpt> llvals;
    //int range = (int) (0.5 + sqrt( (field_nx*field_ny)/10000.) );
    int range = (int) (0.5 + ( (field_nx*field_ny)/10000.) );
    int ti, tj;
    llvals.resize(field.xpoints(), field.ypoints() );
// Precompute the lat-longs
    for (loc.j = 0; loc.j < field.ypoints(); loc.j++) {
    for (loc.i = 0; loc.i < field.xpoints(); loc.i++) {
      llvals[loc] = field.locate(loc);
    }
    } 

    while (range >= 1) {
      for (loc.i = range - 1; loc.i < field_nx; loc.i += range) {
      for (loc.j = range - 1; loc.j < field_ny; loc.j += range) {
      if (field[loc] == undef) {
        if (llvals[loc].lon > lonmax && llvals[loc].lon - 360. > lonmin) {
           llvals[loc].lon -= 360.;
        }
        if (llvals[loc].lon < lonmin && llvals[loc].lon + 360. < lonmax) {
           llvals[loc].lon += 360.;
        }
        if (llvals[loc].lon > lonmax || llvals[loc].lon < lonmin) {
           field[loc] = unfill;
           continue;
        }
        if (wn_PnPoly(llvals[loc], locations, nx ) != 0 ) {
  //      Note that boundaryFill will segfault if loc is not inside the curve
          ...
          //fcount = 0;
          //floodfill(loc.i, loc.j, unfill, boundary, field);
          field[loc] = unfill;
        }
      }
    } // loc.i, loc.j
...
// Now repeat with a step of 1 to fill in all the small patches:
      ...
      if (field[loc] == undef) {
        llvals[loc] = field.locate(loc);
        if (llvals[loc].lon > lonmax && llvals[loc].lon - 360. > lonmin) llvals[loc].lon -= 360.;
        if (llvals[loc].lon < lonmin && llvals[loc].lon + 360. < lonmax) llvals[loc].lon += 360.;
        if (llvals[loc].lon > lonmax || llvals[loc].lon < lonmin) {
           field[loc] = unfill; 
           continue;
        }
...
void aa_fill(mvector<latpt> &locations, psgrid<unsigned char> &field, 
                unsigned char boundary, unsigned char fill, 
                unsigned char unfill, unsigned char undef, int npts ) {
  float latmin, latmax; 
...
// Now put in boundary flags 
  for (i = 1; i < npts; i++) {
    lineFill(ijlocs[i-1], ijlocs[i], field, boundary);
  }

// Taking structure s.t. loc.i = 0, loc.j = max -> antarctic
// Specific to llgrids.  New version for polar grids:
  ll.lat = -90.0; ll.lon = 0.0;
  loc = field.locate(ll);
  printf("loc = %d %d\n",loc.i, loc.j); fflush(stdout);
  fcount = 0;
  while (loc.j >= 0 && field[loc] != boundary ) {
...
void floodfill(const int x, const int y, const unsigned char fill,
  ...
  if ( x < 0 || x > field.xpoints() - 1 ||
       ...
       fcount > 200000 ) {
    if (fcount > 200000) ; //printf("fcount too large \n");
    return;
  }
