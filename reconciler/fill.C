...
//CDvoid subset(TARGET<unsigned char> &tot_field, 
//CD            FAMILY<unsigned char> &field, 
//CD      float west, float east, float north, float south, ijpt &delta); 
...
//    shift involved tot_field[loc+delta] = field[loc]
    //CDsubset(tot_field, field, west, east, north, south, delta); 
    field = tot_field;
 ...
 // Now incorporate the relevant parts of field into tot_field
    ...
    for (loc.i = 0; loc.i < field.xpoints(); loc.i++ ) {
      if (field[loc] == fill || 
        ...
        tloc = loc;
        tloc += delta;
        if (tloc.i < 0 && tot_field.iscyclicx() ) tloc.i += tot_field.xpoints();
        if (tloc.i > tot_field.xpoints() - 1 && tot_field.iscyclicx() ) tloc.i -= tot_field.xpoints();
...
void subset(TARGET<unsigned char> &tot_field, 
  ...
  V[3] = ful;
  if (west - augment < 0) {
    V[3].i -= tot_field.xpoints();
    V[0].i -= tot_field.xpoints();
  }
...
////   subset if there are a large number of points
  if (field_nx*field_ny > 2.0e5) {
    int running = 0;
    int rescale =  (int)(0.5+1.+sqrt((field_nx*field_ny)/2.0e5) ) ;
    bcount = 0;
    ...
    for (loc.i = 0; loc.i < field_nx; loc.i++) {
      ...
      if ( field[loc] == undef ) {
        ...
        if (wn_PnPoly(loc, ijlocs, npts) != 0) {
         #ifdef VERBOSE
            running += 1; 
            printf("calling bndyfil = %d, i,j %d %d = \n",running,loc.i, loc.j); 
            fflush(stdout);
            if (running > 700 && running < 800) {
             sprintf(fname,"field4.%d.xpm",running-700 );
             field.xpm(fname,1,gg);    
            }
          #endif
          //DEBUG boundaryFill(loc.i, loc.j, fill, boundary, field);
          field[loc] = fill;
        }
        else {
          field[loc] = unfill;
