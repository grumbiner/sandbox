...
void subset(TARGET<unsigned char> &tot_field, 
  ...
  //  while (ll.i < -360./dlon-1 && ll.i < (lr.i-360./dlon-1) ) {
  //    ll.i += (int)(0.5 + 360./dlon); 
  //  }
  //  nx = lr.i - ll.i + 1;
  //  printf("new nx %d lr.i = %d, ll.i = %d delta = %f\n",nx, lr.i, ll.i,dlon);
  //}
  ny = ur.j - lr.j + 1;
...
// Note that the following has assumed we're working with llgrids 
  ...
  //if (nx > tot_field.xpoints() || ny > tot_field.ypoints() ) {
  //  printf("trying to stay with original field size\n"); fflush(stdout);
  //  delta.i = 0; delta.j = 0;
  //  field.dlat = tot_field.dlat;
  //  field.dlon = tot_field.dlon;
  //  field.firstlat = tot_field.firstlat;
  //  field.firstlon = tot_field.firstlon;
  //  nx = tot_field.xpoints();
  //  ny = tot_field.ypoints();
  //}
  #ifdef VERBOSE
...
void field_fill(mvector<latpt> &locations, 
  ...
  #ifdef VERBOSE
     printf("done with resize ijlocs\n"); fflush(stdout);
  #endif
  ...
  locations[nx - 1] = locations[0]; // For winding number routine
...
//       -- per C. Lozano.
      ...
      if ( field[loc] == undef ) {
        if (wn_PnPoly(ll1, locations, npts) != 0) {
