...
void gridbounds(FAMILY<unsigned char> &x, float &n, float &s, float &e, float &w);
...
int main(int argc, char *argv[]) {
  ...
  float grid_north, grid_south, grid_east, grid_west;
...
////////////////////////////////////////
  ...
  gridbounds(tot_field, grid_north, grid_south, grid_east, grid_west); 
...
//  This if test is to avoid using overkill on the various small grids.
    printf("k %d %f %f %f %f\n",k, north, south, east, west); fflush(stdout);
    if (k < 15 || south < -78.0) {
    #if RESOPS >= 2
      ...
      printf("subset %d %5d %5d\n",k, field.xpoints(), field.ypoints() ); fflush(stdout);
      #else
...
////   algorithm.
  ...
  if (range > 1) {
    //#if RESOPS == 1
    //printf("Too many points in grid, skipping. %d range = %d\n",
    //    field_nx*field_ny, range); fflush(stdout);
    //exit(1);
    //#endif
