...
//  Set the flags for filling
    ...
    //   being 1/2 degree global, the execution speed is 42 with 1e6, 
    //   13.1 with 1.  (arbitrary units corresponding to seconds on my 
    //   system today, 24 Feb 2003.)
...
// Loop forward until we find a point on the grid.
  ...
  ij1.i = -1;
  ij1.j = -1;
  while (ij1.i == -1. && ij1.j == -1. && i < locations.xpoints()-1 ) {
...
////   subset if there are a large number of points
  if (field_nx*field_ny > 2.0e5) {
    printf("Too many points in grid, stopping now\n"); fflush(stdout);
    exit(1);
  }
...
//// Find a point inside the polygon
  ...
  for (loc.j = 0; loc.j < field_ny; loc.j++) {
    if (field[loc] == undef) {
      ...
      //if (wn_PnPoly(loc, ijlocs, npts) != wn_PnPoly(ll1, locations, locations.xpoints())) {
      //  printf("wn = %d  %f %f  %d %d\n",
      //         wn_PnPoly(ll1, locations, locations.xpoints()),
      //         ll1.lon, ll1.lat, loc.i, loc.j );
      //  fflush(stdout);
      //}
      if (wn_PnPoly(ll1, locations, locations.xpoints() ) != 0 ) {
