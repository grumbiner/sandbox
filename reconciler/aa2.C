...
// Use even/odd crossing to determine interior/exterior.
  for (loc.i = 0; loc.i < field.xpoints(); loc.i++) {
    loc.j = 0;
    int count = 0, lastj = 0;
    unsigned char ref, alt;
    if (field[loc] != undef && field[loc] != boundary) {
      count = 0;
      ref = field[loc];
      if (ref == fill) {
        alt = unfill;
      ...
      printf("ref, alt %d %d i = %d\n",ref, alt, loc.i);

      for (loc.j = 1; loc.j < field.ypoints(); loc.j++) {
        ijpt t1; t1.i = loc.i; t1.j = loc.j - 1;
        if (field[loc] == boundary && field[t1] != boundary) {
            count += 1;
        }
        else if (field[loc] == undef) {
          if ( (count % 2) == 0) {
            field[loc] = ref;
          }
          else {
