...
//  field.set(100);
  ...
  while (!feof(fin) ) {
    ...
    //printf("lakeno = %d area = %f\n",lakeno, area); fflush(stdout);
    i = 0; match = false;
    while (!match && i < n) {
      ll.lat = lat[i];
      ll.lon = lon[i];
      if (wn_PnPoly(ll, locations, locations.xpoints() ) != 0) {
        match = true;
        used[i] = true;
        //printf("lake %4d area %8.1f matches %s %5.1f %5.1f\n",
        //        lakeno, area, arname[i], lat[i], lon[i]);
        //fflush(stdout);
        region_assess(field, land, arname[i], locations, (double) area) ;
      }
      i += 1;
    }
    lakeno += 1;
  ...
  for (i = 0; i < n; i++) {
    if (!used[i]) {
      printf("Never used %s at %f N %f E\n",arname[i],lat[i], lon[i]);
    }
  }
