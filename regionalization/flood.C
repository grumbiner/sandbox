...
void locmax(grid2<float> &x, ijpt &loc) ;
...
//  Look for the unique
  ...
  // compute the area (rather than just grid point counts) covered
  areas.set((float) 0);
  for (tloc.j = 0; tloc.j < areas.ypoints(); tloc.j++) {
  for (tloc.i = 0; tloc.i < areas.xpoints(); tloc.i++) {
    sum = 0.;
    if (counts[tloc] != 0) {
      xx = region_locs[tloc];
    ...
    if (areas[tloc] != 0) {
      ...
      printf("%3d %3d  %7.2f %7.2f  %e area km^2\n",
           tloc.i, tloc.j, tll.lat, tll.lon, areas[tloc]/1.e6);
    }
  }
  }
// location of maximum area's point:
  locmax(areas,tloc);
  tll = areas.locate(tloc);
  printf(" max at: %3d %3d  %7.2f %7.2f  %e\n",tloc.i, tloc.j, tll.lat, tll.lon,  areas[tloc]/1.e6);
