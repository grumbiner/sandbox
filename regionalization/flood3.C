...
int main(int argc, char *argv[]) {
  ...
  ijpt tloc;
  latpt tll;
  float rcrit, min_area;

///  Here starts a lot of setting up:

  int max_regions = 20*1000;
  //min_area = 1.e3*1e6; // km^2
  //min_area = (510e6/720./360.)*4*1e6; // km^2, avg >= 4 grid cells
  rcrit = atof(argv[1]);
  min_area = atof(argv[2]); min_area *= 1e6; // convert to m^2 from input km^2
  printf("rcrit = %f min_area = %f \n",rcrit, min_area);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  global_ice<float> areas, belongs;
  stack<ijpt> xx;
  double sum = 0., areasum = 0.0, amax = 1.e30;
  ijpt stackloc;
  int pass = 0;
  ...
  for (k = MAXPTS ; k > 0; k--) {
    ...
    region_find(tloc.i, tloc.j, loc[k], indices, visited, x, region_locs, region_scores, rcrit);
    ...
    printf(" #points in its region: %5d area %f \n", 
         (int)region_locs[tloc].size(),sum/1e12 ); fflush(stdout);
  }
