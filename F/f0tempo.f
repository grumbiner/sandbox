...
C     Statement functions for the thermodynamics
      ...
      swnet(t) = -(1.-albedo(ts(i,j), ta(i,j),  0.0 , 0.0 ) )*
     1            swdown(i,j)
...
CD        PRINT *,'Freezing'
          ...
          qextra(i,j) = f(ts(i,j))
        ENDIF
CD          WRITE (*,9001) i, j, ta(i,j), ts(i,j), f(ts(i,j)), 
CD     1     swdown(i,j), swnet(ts(i,j)), 
CD     1     lwup(ts(i,j))-lwd(i,j) , lwd(i,j)
