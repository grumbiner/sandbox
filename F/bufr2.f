...
cc  SSM/I brightness data.
 ...
 2000 CONTINUE
      ...
      count = 0
       ...
       OPEN (iunt, FILE="fort.14", FORM="UNFORMATTED", STATUS="OLD")
       OPEN (ount, FILE="fort.51", FORM="UNFORMATTED", STATUS="NEW")
       ...
       DO WHILE (ireadmg(iunt,subset,idate).eq.0) 
         DO WHILE (ireadsb(iunt).eq.0)
           ...
           DO index1 = 1, 9
             XIDENT(index1) = XIDENT8(index1)
           ENDDO
CD           IF (XIDENT(1) .GT. F15 .OR. XIDENT(1) .LT. F11 ) GO TO 1000
           IF (XIDENT(1) .NE. F13) GO TO 1000
           SATNO = XIDENT(1)
           CALL UFBINT(IUNT,xloc8  ,4, 64,iret,XLCST)
           DO index1 = 1, 4
           DO index2 = 1, 64
             xloc(index1,index2) = xloc8(index1, index2)
           ENDDO
           ENDDO
           ...
           DO index2 = 1, 7
           DO index3 = 1, 64
             BRT(index1, index2, index3) = BRT8(index1, index2, index3)
           ENDDO
           ENDDO
           ENDDO
           ...
           DO index1 = 1, 4
           DO index2 = 1, 192
             xloc85(index1, index2) = xloc858(index1, index2)
           ENDDO
           ENDDO
           ...
           DO index2 = 1, 2
           DO index3 = 1, 192
             brt85(index1, index2, index3) = 
     1            brt858(index1, index2, index3)
           ENDDO
           ENDDO
           ENDDO
...
CD            DO 20 kscan = 1, kret
CD              xlat = xloc85(1, kscan)
CD              xlon = xloc85(2, kscan)
CD              sftg = xloc85(3, kscan)
CD              posn = xloc85(4, kscan)
CD              tmp1 = brt85(2, 1, kscan)
CD              tmp2 = brt85(2, 2, kscan)
CD              kwrit = kscan + iret
CD              creturns = hiresout(kwrit, xlat, xlon, sftg, posn, 
CD     1                            tmp1, tmp2)
CD  20       CONTINUE
