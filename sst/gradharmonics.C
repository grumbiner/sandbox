...
////////////////////////////////////////////////////////////////////////
template <class T>
void enmask(grid2<short int> &land, grid2<T> &x, T flag) ;
...
// For local reading and accumulating
  ...
  global_quarter<float> ftmp, tdx, tdy, tmag;
...
// Start scan through observations:
  for (i = 2; i < argc; i++) {
    ...
    fclose(fin);
    conv(tmp, ftmp);
    ftmp /= 100.; // convert to degrees
    printf("read in file %d max = %e\n",i, ftmp.gridmax() ); fflush(stdout);
// find gradient magnitude:
    enmask(land, ftmp, flag);
    gradients(ftmp, tdx, tdy, tmag, flag);
    conv(tmag, stmp);
...
// Defer this declaration for memory considerations
  fout = fopen("mag_dood_harmonics_out", "w");
  ...
  fout = fopen("mag_reference_fields","w");
