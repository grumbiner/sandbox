...
int main(int argc, char *argv[]) {
  ...
  climo.ftnin(fin);
  fclose(fin);
  //printf("climo max min etc. %f %f %f %f\n",climo.gridmax(), climo.gridmin(), climo.average(), climo.rms() );
  if (climo.gridmax() > 200) climo -= 273.15;

  fin = fopen(argv[2],"r");
  obsd.binin(fin);
  fclose(fin);
  //printf("obsd max min etc. %f %f %f %f\n",obsd.gridmax(), obsd.gridmin(), obsd.average(), obsd.rms() );
...
// && fabs(climo[loc]) < 1.e-5 ) {
        ll = obsd.locate(loc);
        printf("%8.4f %8.4f  %4d %4d  %5.2f %5.2f %6.2f\n",ll.lon, ll.lat, loc.i, loc.j, climo[loc], obsd[loc], -climo[loc] + obsd[loc]);
      }
    } 
