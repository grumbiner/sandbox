...
// Second pass -- work with residuals, in floating point centi-degrees
// on desk, long double is 12 bytes, while double is 8, float is 4.
...
// For reading and accumulating
  global_quarter<float> tmp, tmax, tmin, trange;
  global_quarter<long double> tmp2, stmp, sum, sum2, sum3, sum4;
// For trend
  global_quarter<long double> sumx, sumx2, sumxy;  // sumy = sum, sumy2 = sum2;
...
// Initialize:
  tmax.set((float) -5000);
  tmin.set((float)  5000);
  sum.set((long double) 0);
  sum2.set((long double) 0);
  sum3.set((long double) 0);
  sum4.set((long double) 0);
...
// Start scan through observations:
  for (i = 1; i < argc; i++) {
    ...
    conv(tmp, tmp2);
    printf("step %5d max %9.4f\n",i-1, (float) tmp2.gridmax() ); fflush(stdout);
    tmp2 *= -1; // residual range expanded rather than contracted?!
    stmp = tmp2; 
...
// Find things of interest and write them out:
  ...
  fout = fopen("second_pass_out", "w");
  // convert to plain float for ease with grads et al.
  conv(tmax, ftmp);
    ftmp.binout(fout);
  conv(tmin, ftmp);
    ftmp.binout(fout);
...
// x = time, y = sst.
  ...
  for (loc.i = 0; loc.i < sx.xpoints(); loc.i++) {
      slope[loc] = (npts*sxy[loc] - sx[loc]*sy[loc]) /
                   (npts*sx2[loc] - sx[loc]*sx[loc]);       
