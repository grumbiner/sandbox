...
! temperatures -- replacement for 'weaver'
      ...
      INTEGER ratio, nscale
      PARAMETER (nscale = 1)
      INTEGER scales(nscale)
      ...
      limit = 2.e-4
      ratio = 6
      CALL reduced(sst, mask, nx, ny, ratio, landflag, dlat, dlon, limit)
...
!  not land, and in the arctic, it's analyzed.
      ...
      RETURN
      END
      SUBROUTINE reduced(sst, mask, nx, ny, ratio, landflag, dlat, dlon, limit)
      IMPLICIT none
      REAL landflag
      REAL dlat, dlon
      INTEGER nx, ny, ratio
