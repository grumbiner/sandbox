...
////////////////////////////////////////////////////////////////////////////
  ...
  printf("back from assemble2, about to do the laplacean fill\n"); fflush(stdout);
  ...
  latpt ll;
  i = 0;
  for (loc.j = 0; loc.j < expect_high.ypoints(); loc.j++) {
  for (loc.i = 0; loc.i < expect_high.xpoints(); loc.i++) {
    if (expect_high[loc] == maskval) {
      ll = expect_high.locate(loc);
      tloc = expect_fill.locate(ll);
      expect_high[loc] = expect_fill[tloc];
      i++;
    }
  }
  }
...
// precompute some variables
  ...
  for (loc.i = 0; loc.i < land.xpoints(); loc.i++) {
    if (land[loc] == flag) {
      ...
      if (ip.i > land.xpoints() - 1) ip.i -= land.xpoints();
