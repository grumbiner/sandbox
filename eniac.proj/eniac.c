...
/*	Declare temporary arrays -
	...
	float s[19][19], t[16][16], v[19][16];
	float r[19][16], f[19][16], h[19][16];
	float a[19][16], b[19][16];
	float alpha[19][16], beta[19][16];
	float zp[19][16]   , zetap[19][16];
	
/*	Declare data, physical info. */
	float zeta[19][16], eta[19][16];
...
/*     END OF THE PRELIMINARY SET UP SECTION
	...
	for (i = 1; i <=  p-1; i++) {
	    alpha[i][j] = 0.0;
	    for (l = 1; l < p-1; l++) {
	      alpha[i][j]=alpha[i][j]+s[l][i]*zeta[l][j];  
            }
        }  
        }
