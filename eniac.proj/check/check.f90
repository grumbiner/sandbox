...
!  BEGIN THE ITERATIVE SOLUTION OF THE EQUATIONS
  !DO k = 0, 3*n-1 ! can't go the full 3 days because the instability starts throwing * in the output
  DO k = 0, 60 
    ...
    delta = z1 - z2
    WRITE (*,9004) k, tk, MAXVAL(delta), MINVAL(delta), SUM(delta*delta)
