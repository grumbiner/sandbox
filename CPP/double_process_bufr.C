...
/* Peel off data from the record.  Transfer to physical units. */
  for (j = 0; j < NSPOTS; j++) {
    ...
    if (k == 0) {
      ...
      tlat = ( (float)a->full[j].clat[0]);
      tlon = ( (float)a->full[j].clon[0]);
    }
    else {
      t89v = ( (float)a->full[j].obs[AMSR_T89Vb].tmbr );
      t89h = ( (float)a->full[j].obs[AMSR_T89Hb].tmbr );
      tlat = ( (float)a->full[j].clat[1]);
      tlon = ( (float)a->full[j].clon[1]);
    }
...
*/
  ...
  } /* end for k loop -- 89 ghz spots*/
  } /* end for j loop -- data in */
