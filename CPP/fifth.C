...
int main(int argc, char *argv[]) {
  ...
  global_12th<float> dist, grad, gradients, lap;
  global_12th<bool> checked;
  FILE *fin, *fout;
  ijpt loc;
  bool clear = false;
  unsigned char land = 157, water = 0, coast = 195;
  float tmp, flag = -99.0, laplim = 0.001;
  ...
  checked.set(false);
  laplacean(dist, lap, flag);
  printf("idist %d %d\n",idist.gridmax(), idist.gridmin() ); fflush(stdout);
  printf("lap max, etc %e %e %e %e\n",lap.gridmax(), lap.gridmin(), lap.average(), lap.rms() );
  fflush(stdout);
...
// -- need to fix pole-crossing in distperim, anyperim
  ...
  for (loc.i = 0; loc.i < mask.xpoints(); loc.i++ ) {
    if ((grad[loc] > 1 || fabs(lap[loc]) > laplim) && !checked[loc]) {
      checked[loc] = true;
...
// now look for points 'near' land with 'high' gradients
  ...
  while (gradlim > 1.0) {
    ...
    while (!clear && passno < 50 && count >= 1) {
      clear   = true;
      passno += 1;
      count   = 0;
      for (loc.j = 0; loc.j < mask.ypoints(); loc.j++ ) {
      for (loc.i = 0; loc.i < mask.xpoints(); loc.i++ ) {
        if ((fabs(lap[loc] > laplim) || grad[loc] > gradlim) && dist[loc] < distlim && !checked[loc]) {
          checked[loc] = true;
          tmp = distperim(mask, land, idist[loc], loc);
          if (tmp < dist[loc] ) {
            clear = false;
            ...
            printf("nofix %4d %4d  %10.2f %10.2f  %e %e\n",loc.i, loc.j, tmp, dist[loc],
               grad[loc], lap[loc]); fflush(stdout);
          }
        }
      }
      }
