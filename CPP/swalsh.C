...
int main(void) {
  ...
  southwalsh<float> month[12], covar[LAGMAX], var, avg, correl, tmp;
  southwalsh<float> soicor[2*LAGMAX - 1];
  southgrid<float> sh;
  mvector<float> time1(NMONTHS), time2(NMONTHS); 
  time_series<float> times(NMONTHS), soi(NMONTHS);
  float landmask = 9.9, rij;
  ijpt loc, loc2;
  latpt lloc1, lloc2;
  int i, j;
...
//  (0 -> ocean/land);
  ...
  for (loc.i = 0; loc.i < land.xpoints(); loc.i++) {
     if (month[0].operator[](loc) > 1.01 ) {
       land[loc] = 1.57; // Wire this in as 9.9 is standard in SH Walsh
     }
     else {
       land[loc] = 0.0;
     }
     if (land[loc] > 1.01) { avg[loc] = land[loc]; }
     else { avg[loc] /= 12.; }
  }
  }
  land *= 100.;
...
// Print out monthly mean fields
  for (i = 0; i < 12; i++) {
   ...
   month[i] *= 100;
   sh.fromall(month[i], land, land.gridmax(), 0.);
   sh.xpm(fname, 7, gg);
  }
