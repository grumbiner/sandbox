...
// Robert Grumbine 14 July 2003

#include "ncepgrids.h"

void score2(grid2<float> &fcst, grid2<float> &obs, 
            grid2<float> &land, mvector<float> &score) ;
void score3(grid2<float> &fcst, grid2<float> &obs, grid2<float> &ref, 
            grid2<float> &land, mvector<float> &score) ;
...
int main(int argc, char *argv[]) {
  GRIDTYPE<float> fcst, ref, obs, land;
...
// Done getting data, now construct scores:
  //if (argc == 3 ) {
    score2(fcst, obs, land, scores);
  //}
  //else {
  //  score3(fcst, obs, ref, land, scores);
  //}
...
// Score 5, 6, 7 are delta (forecast - obs) statistics:
...
// score 8 is anomaly correlation (not squared):
...
// score 8 is anomaly correlation (not squared):
   correl_(f, o, count, r2, xmean, ymean, sig2x, sig2y);
   score[8] = r2;
