      PROGRAM ttest
C     Program to run a t test on a set of ice drift forecast experiments
C     Robert Grumbine 19 November 1996
 ...
 2200     CONTINUE
     ...
     1                  FLOAT(count-1) )
          ...
          IF (sigmay .NE. 0 .AND. sigmaz .NE. 0) THEN
            ty(i,j) = (sumy/SQRT(FLOAT(nday)) / sigmay)
            tz(i,j) = (sumz/SQRT(FLOAT(nday)) / sigmaz)
