...
// Pre-skip' the points through the observation grid:
  ...
  while (!feof(resin) ) {
    ...
    for (loc.j = 0; loc.j < reduced->ypoints(); loc.j++) {
    for (loc.i = 0; loc.i < reduced->xpoints(); loc.i++) {
      if (reduced->operator[](loc) > 100) reduced->operator[](loc) = landval;
    }
    }
    ...
    model.fromall(*reduced, landval, nonval);
      ...
      for (loc.i = 0; loc.i < model.xpoints(); loc.i++) {
        if (model[loc] < 0.15) model[loc] = 0.0;
      }
      }
