...
c
      ...
      character*1 line1(21),line2(9),line3(39),line4(26),line5(41),
     1            hdrfxp(23),hdrnfp(38),bln(4),indx(10),old(20),new(36),
     2            line5a(42)
      character*1 oldpts(207,20),newpts(1000,36),akoldp(160,20),km,jn,q,
     1            aknewp(200,36),lf,cr,eom,blk,b,c,d,e,f,g,h,ji,jj,jk,a,
     2            jl,o,p,r,s,x,w,glbmsg(1280,31),akmsg(1280,8),ldr(40),
     3            t,aps,copt(17),cnpt(33)
c
...
c
     ...
     1           '0','0','0','0','0','0','<','<','@'/
      ...
      DATA line3/'P','A','R','T',' ','A','A',' ','A','L','A','S',
     1           'K','A',' ','S','E','A',' ','I','C','E',' ',
...
c6010 format(40a1)
 ...
 8010          oldpts(j,18)=cr
      ...
      do i=1,36
         do j=1,1000
            if(i.lt.34)then
               if(j.gt.200)go to 8020
               aknewp(j,i)=blk
 8020          newpts(j,i)=blk
            else
               if(j.gt.200)go to 8030
               aknewp(j,34)=cr
               aknewp(j,35)=cr
               aknewp(j,36)=lf
 8030          newpts(j,34)=cr
...
c
 ...
 7000 format(i2,i2,i2)
      ih=00
      if(time.lt.100)then
         igtot=40+21+9+39+26+41+23+4+(ii-1)*20+4+38+4+(ik-1)*36+1
         iputg=igtot/1280
         left=mod(igtot,1280)
         if(left.gt.0)iputg=iputg+1
         write(06,7100)igtot,iputg
 7100    format(' igtot=',i5,' iputg=',i2/)
      else
         igtot=40+21+9+39+26+42+23+4+(ii-1)*20+4+38+4+(ik-1)*36+1
         iputg=igtot/1280
         left=mod(igtot,1280)
         if(left.gt.0)iputg=iputg+1
         write(06,7100)igtot,iputg
...
c7103 format(' ',9a1)
      ...
      line3(13)=A
      line3(14)=jL
c     if(time.eq. 12)go to 7100
      if(time.eq. 24)line3(7)=A
c     if(time.eq. 36)line3(7)=C
      if(time.eq. 48)line3(7)=B
c     if(time.eq. 60)line3(7)=E
      if(time.eq. 72)line3(7)=C
c     if(time.eq. 84)line3(7)=G
      if(time.eq. 96)line3(7)=D
c     if(time.eq.108)line3(7)=jI
      if(time.eq.120)line3(7)=E
c     if(time.eq.132)line3(7)=jk
      if(time.eq.144)line3(7)=F
c     if(time.eq.156)line3(7)=km
      if(time.eq.168)line3(7)=G
c     if(time.eq.180)line3(7)=o
      if(time.eq.192)line3(7)=h
c     if(time.eq.204)line3(7)=q
      if(time.eq.216)line3(7)=ji
c     if(time.eq.228)line3(7)=s
      if(time.eq.240)line3(7)=jj
      call W3AI19(line3,39,glbmsg,igtot,next)
...
c
 ...
 7116 format(//)
      if(time.lt.100)then
c        if(il.eq.999)then
c           iatot=40+21+9+39+26+41+23+4+(ij-1)*20+1
c           iputa=iatot/1280
c           left=mod(iatot,1280)
c           if(left.gt.0)iputa=iputa+1
c        else
            iatot=40+21+9+39+26+41+23+4+(ij-1)*20+4+38+4+(il-1)*36+1
            iputa=iatot/1280
            left=mod(iatot,1280)
            if(left.gt.0)iputa=iputa+1
c        endif
         write(06,7117)iatot,iputa
 7117    format(' iatot=',i5,' iputa=',i2/)
      else
c        if(il.eq.999)then
c           iatot=40+21+9+39+26+42+23+4+(ij-1)*20+1
c           iputa=iatot/1280
c           left=mod(iatot,1280)
c           if(left.gt.0)iputa=iputa+1
c        else
            iatot=40+21+9+39+26+42+23+4+(ij-1)*20+4+38+4+(il-1)*36+1
            iputa=iatot/1280
            left=mod(iatot,1280)
            if(left.gt.0)iputa=iputa+1
c        endif
...
c     write(06,7103)line2
      ...
      line3(13)=jK
      line3(14)=A
      call W3AI19(line3,39,akmsg,iatot,next)
c     write(06,7104)line3
