      SUBROUTINE geowin(ug, vg, temp, lnp, ztopo, twave)
C     Compute the geostrophic wind from the spherical harmonics.
C     Bob Grumbine 2 June 1994.
      ...
      REAL, DIMENSION(:), ALLOCATABLE :: cosl
...
C     Set up the required arrays:
      ALLOCATE(work(mwave*2))
      ALLOCATE(cosl(nlat))

C     Convert temperature to regular grid.
      CALL SPHERT(-101, temper, temp, 0, fac, nlong, nlat, twave, 0)
...
C     Find the gradients of LnP and z
      psmean = lnp(1)
      CALL SPHERD(-101, dlnpdx, dlnpdy, psmean, lnp, work, cosl,
     1   nlong, nlat, twave, 0)

      zmean = ztopo(1)
      CALL SPHERD(-101, dzdx, dzdy, zmean, ztopo, work, cosl,
     1   nlong, nlat, twave, 0)
