...
// Utilities to compute the gradient
// #include "lapl.C"
...
// Compute the squared gradient:
  printf("sst max, min, average %f %f %f\n", sst.gridmax(), sst.gridmin(), 
           sst.average() );
  gradsq(sst, grad2, (float) -273.15);  // last is a flag value -- nonexistent
  printf("sst max, min, average %e %e %e\n", sst.gridmax(), sst.gridmin(), 
           sst.average() );
  sst = grad2;
  sst /= grad2.average();  // need rescaling for the %f format to show values
  printf("sst max, min, average %e %e %e\n", sst.gridmax(), sst.gridmin(), 
           sst.average() );
