...
c	due to pressure (kg-m**2/s) to Earth orientation 
c	excitation functions (mas and ms).
	...
	write (6,*)
	write (6,'(1x,a,$)') 'Enter input  pressure OAM file name: '
	read  (5,'(a)') inf
	open  (1,file=inf,form='formatted',status='old')
	rewind 1

	write (6,'(1x,a,$)') 'Enter output pressure chi file name: '
	read  (5,'(a)') outf
  ...
  10	continue
	...
	xout = xin * 1.61d0 / omega / cmina / 1.44d0		! radians
	yout = yin * 1.61d0 / omega / cmina / 1.44d0		! radians
	zout = zin / omega / cmoi * 0.756d0			! days
