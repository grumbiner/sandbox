...
int main(int argc, char *argv[]) {
  ...
  f1.fval = 1.0 ; f2.fval = 5.;
  ref_code.newgene(0, 9, FLOAT_TYPE,  f1, f2);
  f1.fval = 0.0 ; f2.fval = 1.0;
  ref_code.newgene(1, 2, FLOAT_TYPE, f1, f2);
  f1.ival = 1 ; f2.ival = 4;
...
//
      ...
      for (loc.j = 1; loc.j < weight.ypoints() - 1 ; loc.j++) {
        ...
        for (loc.i = 1; loc.i < weight.xpoints() - 1 ; loc.i++) {
          printf("%1d ",weight[loc][2].ival);
        }
        printf("   a ");
        for (loc.i = 1; loc.i < weight.xpoints() - 1 ; loc.i++) {
          printf("%4.2f ",weight[loc][0].fval);
        }
        printf("\n");
      }
...
int get_exact(grid2<double> &x, grid2<double> &y) {
  ...
  do {
    ...
    printf("exact iter del = %d %f rms delta %e rms.lap^3 %e\n",
        iter, del, 1./working.rms(), pow(1./lap.rms(),2.) ); fflush(stdout);
    iter += 1;
  }
