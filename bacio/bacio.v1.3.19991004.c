...
/* Check for illegal combinations of options */
  if (( BAOPEN_RONLY & *mode) && ( (BAOPEN_WONLY & *mode) || (BAOPEN_WONLY_TRUNC & *mode) ) ) {
     #ifdef VERBOSE
...
/*   be able to open the files Fortran is expecting to be opened.    */
  ...
  if ( (BAOPEN_RONLY & *mode) || (BAOPEN_WONLY & *mode) || 
       (BAOPEN_WONLY_TRUNC & *mode) || (BAOPEN_RW & *mode) ) {
    #ifdef VERBOSE
...
/* Read data as requested */
  if (BAREAD & *mode && ( (BAOPEN_WONLY & *mode) || (BAOPEN_WONLY_TRUNC & *mode) ) ) {
    #ifdef VERBOSE
