...
C       distributions.
      ...
      DO 1000 j = 1, lenrun*INT(secpyr/deltat)
        ...
        CALL advstr(deltaz, deltat, phost, kappa, w, j)
       
        IF (MOD((j-1),INT(FLOAT(outoft)*secpyr/deltat)) .EQ. 0) THEN
          WRITE (10, 9002) (i,CHAR(9),phos(i),CHAR(9),phost(i),
     1                        i=1, nlayer)
          sum1 = 0.D0
...
C         with a different scheme.
 ...
 1000 CONTINUE
CD      i = 2
CD        f2(i) = (   kappa(i+1)          *t(i+1)
CD     1            -(kappa(i+1)+kappa(i))*t(i)
CD     2            + kappa(i)            *t(i-1) )
CD     3    /deltaz/deltaz
CD        fp(i) = f2(i)
...
C     Put the temporary data into the saved vector (n-1 on the next step)
 ...
 2000 CONTINUE
      ...
      END
C*************************************************----------++++++++++!!
      SUBROUTINE advstr(deltaz, deltat, t, kappa, w, j)
C     Compute a purely advective-diffusive solution, no
C       internal sources or sinks, and no flux boundary conditions.
C     BG 5/17/91.
