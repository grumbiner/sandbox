C*************************************************----------++++++++++!!
      SUBROUTINE advdif(deltaz, deltat, t, kappa, w, ngas, step)
...
C     Add source at the surface
 
      IMPLICIT none
      INTEGER nlayer, ngas
      PARAMETER (nlayer = 76)
 
      DOUBLE PRECISION deltaz, deltat, q
...
C         with a different scheme.
        CALL advstr(deltaz, deltat, t, kappa, w, ngas, step)
        RETURN
      ENDIF
...
C       on the next step, it will be n-1 level.
 ...
 1200 CONTINUE
      ...
      CALL gasses(4, tnm1(1), q)
 
C     Now apply a no flux diffusion condition
      t(nlayer)   = t(nlayer-1)
      t(1)        = t(2) + q*2.*deltat/deltaz
	  t(2)        = t(1)
