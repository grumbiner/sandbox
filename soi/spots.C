...
int main(int argc, char *argv[]) {
  ...
  time_series<float> soi(NYEARS*12);
...
//Initialize:
  ...
  soiread(soi);  //read in sunspot list;
  
  // Remove the monthly averaged values so as to work with anomalies:
  soiavg = (float) 0.0;
...
// Set up the genome
  ...
  f1.ival = 0; f2.ival = 127;
  x.newgene(2, 7, INT_TYPE, f1, f2);
  f1.ival = 0; f2.ival = 127;
  x.newgene(3, 7, INT_TYPE, f1, f2);
...
/////////////////////////////////
void displayer(mvector<int> &genome, 
  ...
  for (i = LEADER; i < soi.xpoints() - lead ; i++) {
    est = weights[0].fval* soi[i-weights[2].ival] + 
...
void soiread(time_series<float> &spots) {
   float d1, d2, d3, d4;
   int i;
   FILE *fin;
