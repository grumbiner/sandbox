...
c               grid cell dimensions (12.5 km, 25.0 km, or 50.0 km).
c
...
c
c     cell    : Size of the SSM/I grid ( 12.5 km, 25.0 km, 50.0 km)
c
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
        ...
        real SGN, delta
        integer numy(2,3)
        real cell(3), xydist(2,2)
        data numy / 896, 664, 448, 332, 224, 166 /
        data cell     / 12.5 , 25.0 , 50.0 /
...
c      
             ...
             read *,i,j
c
c Convert I,J pairs to x and y distances from origin. The grid will be
c 'flipped' in the 'Y' direction.
c
             x=((i-1)*cell(gtype))-(xydist(1,ihem)-cell(gtype)/2.)
             kk=numy(ihem,gtype)-(j-1)           
