...
C     ECCENTRICITY
      READ(*, 200, END=9000 )NEF
  200 FORMAT(BZ, I3)
  201 FORMAT(BZ, 5X, 3F15.10) 
 9000 DO 1 I=1, NEF 
        READ(*, 201, END=9001 )AE(I), Y, Z
 9001   BE(I)=Y*PIRR
        CE(I)=Z*PIR 
    1 CONTINUE
C 
C     OBLIQUITY 
      READ(*, 200, END=9002 )NOB
 9002 XOD = 23.320556 
      DO 3 I = 1, NOB 
        READ(*, 201, END = 9003 )AOB(I), Y, Z 
 9003   BOB(I) = Y*PIRR 
        COB(I) = Z*PIR
    3 CONTINUE
...
C     GENERAL PRECESSION IN LONGITUDE 
      XOP = 3.392506
      PRM = 50.439373 
      READ(*,200,END = 9004 )NOP
 9004 DO 31 I = 1,NOP 
        READ(*,201,END = 9005 )AOP(I),Y,Z 
 9005   BOP(I) = Y*PIRR 
        COP(I) = Z*PIR
   31 CONTINUE
...
C     DEGREES AWAY FROM THIS. THIS CAN CAUSE CONFUSION. ) 
      ...
      IF (ABS(PERH) .GT. 360.) THEN 
        IF (PERH .GT. 360.) THEN
         ...
         ELSE 
          PERH = PERH + 360.* (INT(ABS(PERH))+1.) 
        ENDIF 
      ENDIF 
      IF (PERH .LT. 0.) PERH=PERH+360.
