...
C   Variant sphert94 copied by Bob Grumbine from Mark Iredell directory
C     source on C-90, 2 June 1994.
C
...
C$$$
      ...
      DIMENSION A(K),W(K)
      DIMENSION AWORK((K+1)/2,(K+1)/2+1),BWORK((K+1)/2*2)
      ...
      DO JS=1,KHE
        DO J=1,KHE
          AWORK(JS,J)=COS(2*(JS-1)*(J-1)*DLT)
        ENDDO
      ENDDO
      DO JS=1,KHE
        AWORK(JS,KHE+1)=-1./(4*(JS-1)**2-1)
      ENDDO
      CALL MINV(AWORK,KHE,KHE,BWORK,DA,1.E-12,1,0)
      DO J=1,KHE
        W(J)=AWORK(J,KHE+1)
      ENDDO
CDIR$ IVDEP
