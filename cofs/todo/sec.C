...
template <class T>
void interpolator_test(metricgrid<T> &ll1, metricgrid<T> &ll2, T maskval, T nonval) ;
...
int main(void) {
  ...
  llgrid<float> *g1, *g2;
//Global grids:
  global_sst<float> ll1, ll2;
  //global_ice<float> ll1, ll2;
  //chalgrid<float> ll1, ll2;
  //mrf1deg<float> ll1, ll2;
  //global_otis<float> ll1, ll2;
  //global_wave<float> ll1, ll2;
  //NEW_global_wave<float> ll1, ll2;
  //nh_ocean_weather<float> ll1, ll2;
// Regional grids
  //chalreg<float> ll1, ll2;
  //reg_otis<float> ll1, ll2;
  //ECG_wave<float> ll1, ll2;
  //ECG_hur<float> ll1, ll2;
  //cfsreg<float> ll1, ll2;
...
// Test the grid to grid interpolation.
  ...
  //global_sst<float> g1, g2;
  //global_ice<float> g1, g2;
  //chalgrid<float> g1, g2;
  //mrf1deg<float> g1, g2;
  //global_otis<float> g1, g2;
  //global_wave<float> g1, g2;
  //NEW_global_wave<float> g1, g2;
  //nh_ocean_weather<float> g1, g2;
  //global_ll<float> g1, g2;
// Regional grids
  //chalreg<float> g1, g2;
  //reg_otis<float> g1, g2;
  //ECG_wave<float> g1, g2;
  //ECG_hur<float> g1, g2;
  //cfsreg<float> g1, g2;

  g1 = new global_sst<float>;
  g2 = new global_sst<float>;
  g2->set(5.0);
  for (ij.j = 0; ij.j < g1->ypoints() ; ij.j++) {
  for (ij.i = 0; ij.i < g1->xpoints() ; ij.i++) {
    lats = g1->locate(ij);
    if (lats.lon < 0.) lats.lon += 360.;
    g1->operator[](ij) = lats.lon * mag;
  }
  }

  interpolator_test(*g1, *g2, maskval, nonval);
...
template <class T>
void interpolator_test (metricgrid<T> &ll1, metricgrid<T> &ll2, T maskval, T nonval) {
//Local grid types to try to interpolate to:
  llgrid<float> llhigh(360/.25, 180/.25, 0.25, 0.25, -89.875, .125);
  psgrid<float> ps(50, 90, 25.0, 45.0, 60.0, -10.0, 1.0, 125.e3, 125.e3);
  psgrid<float> ps2(250, 450, 125.0, 225.0, 60.0, -10.0, 1.0, 25.e3, 25.e3);
