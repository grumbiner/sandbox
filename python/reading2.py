...
for i in range (0, nx):
  for j in range (0, ny):
    ...
    if ( (thick[j,i] > 0. and conc[j,i] == 0.) or (conc[j,i] > 0. and thick[j,i] == 0.) ):
      ...
      print("conflict ",i,j,round(conc[j,i],3), round(thick[j,i],3), 
round(latitude[j,i],3), round(longitude[j,i],3), round(area[j,i]/1.e6,3) )


#General diagnostics
print("total extent = ",round(extent/1e12,3) )
print("total area   = ",round(tarea/1e12,3) )
print("total vol    = ",round(vol/1e12,3) )
print("global effective concentration ",round(tarea / extent,6) )
print("global effective thickness     ",round(vol / tarea,6) )
