...
C------------------------------------------------------------------ 
     ...
     3      MSNAUT, MAPRIM, OUTDAT, DELTAX, DELTAT, 
     4      DHPDT(0:MXSB), HPCALL, ISSB 
...
C        ADDED 8-16-85 BG.
     ...
     1     CALL ANOMALY(LAT, TOFYR, -STRTIM+TIMELP/ SECPYR, ANOM) 
        SNLXIN = SNLMPT+ 2.6163E4* ANOM* ALP
       ELSE 
...
C       TEMPERATURE DEPENDANT ACCUMULATION RATE.
     ...
     1        (I* DELTAX- SNLXIN))
           ...
           ELSE 
            AHAT(I)=-ABLAT-ALPHA* BWON* GAM* SLOPE* 
     1        (I* DELTAX- SNLXIN) 
...
C-----INTERIOR POINTS 
          DO 3100 I = ISNB, ISSB-1
            DMP = DMC 
            DMC = SINHLF(I)*(HGT(I+1,H)-HGT(I,H))**3
     1         *(HGT(I,H)+HGT(I+1,H)+HGT(I,HP)+HGT(I+1,HP))**5
...
C           SPECIAL TREATMENT FOR ISSB TO ENSURE MASS BALANCE 
            DMP = DMC 
            DMC = SINHLF(I)*(HVIRT(H)-HGT(I,H))**3
     1        *(HGT(ISSB,H)+HGT(ISSB,HP))**5
            FRCNG(ISSB) = (MU1-DTAF2*BHAT(ISSB))*HGT(ISSB,H)
     1                   -DHPDT(ISSB) 
     2                   +DELTAT*AHAT(ISSB) 
     3                   +DTDXSN(ISSB)*(DMC-DMP)
...
C     NOTE: G(I) IS UPVCT(I) IN CODE
            ...
            G11 =  HGT(ISNB-1,H) + HGT(ISNB, H) 
     1            + HGT(ISNB-1,HP) + HGT(ISNB, HP)
            G21 =  HGT(ISNB, H) + HGT(ISNB+1,H) 
     ...
     1           * ( (5.0*G12+3.0*G11)*G10 +
     2                (5.0* G22- 3.0*G21)*G20 ) 
            BI = -DTDXSN(ISNB) * (5.0*G12-3.0*G11) * G10
            CI = -DTDXSN(ISNB) * (5.0*G22+3.0*G21) * G20
            FI = FRCNG(ISNB) - (MU2 + DTAF2*BHAT(ISNB))*
     1           HGT(ISNB,H) + DTDXSN(ISNB)*(G10*G11*G12+G20*G21*G22) 
...
C-----CAL AI,BI,CI,FI AND GAMMA(I),G(I) FOR INTERIOR PTS
            ...
            DO 3200 I = ISNB+1, ISSB-1
  
              G11 =  G21
              G21 =   HGT(I, H) + HGT(I+1, H) 
...
C       SPECIAL TREATMENT AT ISSB 
              G11 =  G21
              G21 =   HGT(ISSB, H) + HGT(ISSB+1, H) 
     1              +HGT(ISSB,HP) + HGT(ISSB+1,HP)
              G12 =  G22
              G22 = HVIRT(H)-HGT(ISSB,H)
              G10 = -G20
              G20 =  SINHLF(ISSB) * G21**4 * G22**2 
  
              AI = (MU2 + DTAF2*BHAT(ISSB)) - DTDXSN(ISSB)
     1             * ((5.0*G12+3.0*G11)*G10 + (5.0*G22-3.0*G21)*G20)
              BI = -DTDXSN(ISSB) * (5.0*G12-3.0*G11) * G10
              CI = -DTDXSN(ISSB) * (5.0*G22+3.0*G21) * G20
              FI = FRCNG(ISSB) - (MU2 + DTAF2*BHAT(ISSB)) * HGT(ISSB,H) 
     1             + DTDXSN(ISSB)*(G10*G11*G12+G20*G21*G22) 
  
              GAMMA(ISSB) = CI / (AI - BI*GAMMA(ISSB-1))
              UPVCT(ISSB) = (FI - BI*UPVCT(ISSB-1)) / 
     1          (AI - BI*GAMMA(ISSB-1)) 
  
C-----CAL AI,BI,CI,FI AND GAMMA(ISSB), G(ISSB) FOR SOUTH EDGE PT
...
C       PERIODS ARE, IN YEARS 
      ...
      IF (ASTRO) CALL ANOMSET(LAT, TOFYR, 0.0, ANOM)
      RADPDG = PI / 180.0 
      NBCLAT = 15.0*RADPDG
