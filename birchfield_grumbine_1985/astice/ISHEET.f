...
C------------------------------------------------------------------ 
     ...
     1      ACMRAT, SLOPE, TIMELP, TIMSTP, GRDSTP,
     2      SNLMPT, SNLXIN, TRIPS, RNETAC, POLFLX,
     3      MSNAUT, MAPRIM, OUTDAT, DELTAX, DELTAT, DHPDT(0:MXSB),
     4      HPCALL, ISSB
  
C     /ISCOM/ VARIABLE DECLARATION
      REAL HGT, ACMRAT, SLOPE, TIMELP 
...
C     DATA ARRAYS 
      REAL FRCNG(0:MXSB), SINHLF(0:MXSB), DTDXSN(0:MXSB)
...
C     COMPUTED CONSTANTS
      REAL RADSTP, RADPDG, MTPDEG 
      REAL SECPYR, ICEFAC, MHPOIS, NBCLAT 
      REAL DTAF2,MU1,MU2
  
C     VARIABLES FOR ACCUMULATION RATE COMPUTATION 
      REAL BNAUT, BWON, ALPHA, GAM, ANOM, STRTIM, TSTAR, BETA 
      REAL AHAT(0:MXSB), BHAT(0:MXSB) 
...
C-----SECTION FOR INITIALIZING ICE SHEET, CALLED WHEN TRIPS=0.
      ...
      READ (*,9009) STRTIM
      CALL HPEXIN 
      DO 8100 I=0,INT(1.E6+STRTIM)/1000 
        READ (*,9009) ANOM
 8100 CONTINUE
 9009 FORMAT (E20.12) 
