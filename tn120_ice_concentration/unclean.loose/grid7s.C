...
int inparse(fijpt *center, int *range, int *mag, southgrid <float> b, FILE *cgilog); 

int main(int argc, char *argv[])
{
  southgrid<unsigned char> slandu;
  southgrid<float> b, sland;
  grid2<float> d, dland, e, eland;
  palette gg(19,65);
  FILE *nin, *cgilog;
  int i, j, range;
  int x1, y1, x2, y2, mag;
...
//   compute the derived constants.
{
  ...
  if (nin != NULL ) {
    ...
    printf("Failed to open south file\n");
  }
  fclose(nin);
...
////////////////////////////////////////////////////////////////////////
  ...
  nin = fopen("sland.map","r");
  if (nin == NULL ) {
    printf("Failed to open the land file\n");
    return -1;
  }
  slandu.read(slandu,nin);
  fclose(nin);
  for (xl.j = 0; xl.j < b.ypoints() ; xl.j++) {
  for (xl.i = 0; xl.i < b.xpoints() ; xl.i++) {
     sland[xl] = (float) slandu[xl];
  }
  }
  ...
  dland = sland.subset(x1, y1, x2, y2);
