#!/bin/sh
#@ job_type=serial
#@ output=/u/wx21rg/newamsr.rfc2b.out
#@ error=/u/wx21rg/newamsr.rfc2b.err
#@ class=1
...
if [ ! -s $OUTDIR/n3amsr.${tag4} -o \
  ...
  $ASSESS $EXDIR/seaice_seaiamsr amsr.bufr.$tag4 $FIX/seaice_nland127.map $FIX/seaice_sland127.map \
     ...
     #initnorth12.$tag4 initsouth12.$tag4 > v.$tag4
...
#      initnorth12.$tag4 initsouth12.$tag4 \
#      umasknorth12.$tag4 umasksouth12.$tag4

  echo done with seaiamsr
...
#   set for use by drift model
  $EXDIR/seaice_monitor_c12th  fill5min.$PDY ${OUTDIR}/fill5min.$tagm  \\
  ...
  for fn in latlon.$tag4 age.$tag4 fill.$tag4 fill5min.$tag4 \\
            seaice_monitor_${tag4}.kml seaice_monitor_${tag4}.txt \\
            seaice_edge_${tag4}.kml seaice_edge_${tag4}.txt
  do
    if [ -s $fn ] ; then
      cp $fn $OUTDIR
