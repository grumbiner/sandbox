#!/bin/sh
#@ job_type=serial
#@ output=/u/wx21rg/newamsr.rfc2.out
#@ error=/u/wx21rg/newamsr.rfc2.err
#@ class=1
...
if [ ! -s $AMSRDIR/amsr.bufr2.$tag4 ] ; then
  ...
  echo fort.51 > delta
  mv fort.51 amsr.bufr2
  cp amsr.bufr2 $AMSRDIR/amsr.bufr2.$tag4

else
  cp $AMSRDIR/amsr.bufr2.$tag4 .
fi
...
if [ ! -s $OUTDIR/n3amsr.${tag4} -o \
  ...
  $ASSESS $EXDIR/seaice_seaiamsr amsr.bufr2.$tag4 $FIX/seaice_nland127.map $FIX/seaice_sland127.map \
