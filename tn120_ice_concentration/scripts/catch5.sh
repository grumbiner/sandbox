...
if [ $SYSTEM = "IBM" ] ; then
  ...
  OUTDIR=/usr1/dataflow/ice/hires.final2
  TMPDIR=$RUNDIR
...
#Try to work from the operational directories
if [ $SYSTEM = "IBM" ] ; then
  if [ -f /com/fnl/prod/sst.${tag4}/sst2dvar_grb_0.5 ] ; then
    SSTDIR=/com/fnl/prod/sst.${tag4}/
    COM=true
  fi
fi
...
#set -x
qc=false
#If possible, work from the com directory.  Note file naming is different
if [ $SYSTEM = "IBM" -a $COM="true" ] ; then
  cp $SSTDIR/sst2dvar_grb_0.5 oned
  qc=true
else
