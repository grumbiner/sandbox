...
if [ $SYSTEM = "IBM" ] ; then
  ...
  FIX=/nwprod/fix
  EXPTFIX=/u/wx21rg/save/iceconc/fix
  OUTDIR=/u/wx21rg/noscrub/team2b
...
export DCOMROOT RUNDIR TMPDIR EXDIR FIX OUTDIR SSMIDIR SSTDIR ALTSST ADIR1 UNBLKDIR
...
if [ ! -s $OUTDIR/n3ssmi.${tag4} -o \
  ...
  $ASSESS $EXDIR/new.seaice_seaissmi delta $FIX/seaice_nland127.map $FIX/seaice_sland127.map \
...
if [ -s ${SSTDIR}$tag4/rtgssthr_grb_0.083 ] ; then
  ...
  j=$tag4
  while [ ! -s ${SSTDIR}$j/rtgssthr_grb_0.083 -a $qc="false"  -a $trials -le 10 ]
  do
    j=`expr $j - 1`
    j=`${DTGDIR}/dtgfix3 $j`
  ...
  while [ $qc="false" -a $trials -le 30 -a ! -s ${ALTSST}/rtgssthr_grb_0.083.$j ] 
  do
    j=`expr $j - 1`
    j=`${DTGDIR}/dtgfix3 $j`
    if [ -s ${ALTSST}/rtgssthr_grb_0.083.$j ] ; then
      cp ${ALTSST}/rtgssthr_grb_0.083.$j oned
      qc=true
    fi
    trials=`expr $trials + 1`
  done
...
#RBASEF=rmg3@polar.ncep.noaa.gov:/var/ftp/pub/ice/
#RBASEW=rmg3@polar.ncep.noaa.gov:/data/WWW/seaice/expt.analy/
##
## Copy results over to polar, test directory
##
#if [ -s eng.$tag4 ]  ; then
#  #scp eng.$tag4             ${RBASEF}
#  #scp eng5min.$tag4         ${RBASEF}
#
#  scp north12psg.${tag4}   ${RBASEF}
#  scp south12psg.${tag4}   ${RBASEF}
#  scp northpsg.${tag4}     ${RBASEF}
#  scp southpsg.${tag4}     ${RBASEF}
#
#  #7 June 2007 -- comment out the following for now -- there is no
#  #experiment in progress and basew is expt.analy
#  #scp umasknorth12.$tag4   ${RBASEW}/north
#  #scp umasksouth12.$tag4   ${RBASEW}/south
#  #scp fill5min.$tag4       ${RBASEW}/filled5
#  #scp fill.$tag4           ${RBASEW}/filled
#
#fi
#
...
#      rm ${base}.gif
#    fi
#    scp ${base}.gif ${RBASEW}/${base}.gif
#  done
#fi
#
#if [ $newgraphics = "true" ] ; then
#  for remote in $RBASEW 
#  do
...
#  done
#  for remote in $RBASEF 
#  do
#    scp nh12.gif $remote/nh.gif
#    scp sh12.gif $remote/sh.gif
#    scp nh12.gif $remote/nh/nh.${tag4}.gif
#    scp sh12.gif $remote/sh/sh.${tag4}.gif
#  done
#fi
