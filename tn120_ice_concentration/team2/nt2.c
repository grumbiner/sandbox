...
void get_msk(short **msk, int xs, int ys, char *hemi)
{
	...
	if (*hemi == 'n') file = fopen("landmask_north_25", "rb");
	else file = fopen("landmask_south_25", "rb");
...
void get_tbs(short **tb,
	int xsize,int ysize,char *cddate,char *chan,char *pole)
{
    char name[100];
	char hdr[294];
	short int **data;
   ...
   sprintf(name,"%s.%s",cddate,chan);

	if (name == "(null)"){
		printf("No such data on the raid\n");
	}

	printf("%s\n",name);

	if ((chan == "85v")||(chan == "85h")) {
		tempxsize=xsize*2;
		tempysize=ysize*2;
	} 
	else {
	...
	if ((chan == "85v")||(chan == "85h")) {   /** This takes every other pixel ***/
		for(i=0;i<xsize;i++){				  /** of the 85 GHz data           ***/
			for(j=0;j<ysize;j++){
				tb[j][i]=data[j*2][i*2];
			}
		}
		...
		float ***LUTDGR, float ***LUTGR37, char *hemi)
{
	...
	/****** Read model data ****/
	/**** open water TBs ***/
	if (*hemi == 'n') file=fopen("TBowark.tab","r");
	else               file=fopen("TBowant.tab","r");
	for (i=0;i<n_atm;i++)
	    for (f=0;f<7;f++)
	...
	/**** first-year TBs ***/
	if (*hemi == 'n') file=fopen("TBfyark.tab","r");
	else               file=fopen("TBfyant.tab","r");
	for (i=0;i<n_atm;i++)
	    for (f=0;f<7;f++)
	...
	/**** ice type C TBs ***/
	if (*hemi == 'n') file=fopen("TBccark.tab","r");
	else               file=fopen("TBccant.tab","r");
	for (i=0;i<n_atm;i++)
	    for (f=0;f<7;f++)
		...
		for(y=0;y<ysize;y++){
		  if ((v19[y][x] > 500)&&(v85[y][x] > 500)&&(msk[y][x] == 0)){
			...
			if ((gr3719 < 0.05) && (gr2219 < 0.045)){
				...
				icecon[y][x]=(camina[bestk]+ccmina[bestk]);
...
main(int argc, char *argv[])
{
    ...
    *************************/
	...
	/* Here, the xsize and ysize correspond to the NSIDC SSM/I grid */
	if (*pole == 'n'){
		xsize=304;
		ysize=448;
	}
	...
	get_tbs(v19,xsize,ysize,cddate,"19v",pole);
	get_tbs(h19,xsize,ysize,cddate,"19h",pole);
	get_tbs(v22,xsize,ysize,cddate,"22v",pole);
	get_tbs(v37,xsize,ysize,cddate,"37v",pole);
	get_tbs(v85,xsize,ysize,cddate,"85v",pole);
	get_tbs(h85,xsize,ysize,cddate,"85h",pole);
    ...
    sprintf(outfile,"%s%s.nt2",pole,cddate);
