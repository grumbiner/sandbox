...
int main(int argc, char *argv[])
{
  ...
  southgrid<short int> *gland, *gsea;
  southgrid<unsigned char> *gfilt, *oldland;
  southgrid<float> *gpct;
  ijpt x, x1, x2;
  fijpt y;
  latpt ll;
...
//  ny = (int) (180./dll + 0.5);
  ...
  gland = new southgrid<short int>;
  gsea  = new southgrid<short int>;
  nx = gland->xpoints();
  ny = gland->ypoints();
...
// Now incorporate the filtering -- This should be a function:
  gfilt =   new southgrid<unsigned char>;
  gpct =    new southgrid<float>;
  oldland = new southgrid<unsigned char>;
  fin = fopen(argv[2], "r");
  oldland->binin(fin);
  fclose(fin);
  gfilt->set(100);
...
//////// This too should be a function
  ...
  for (x.i = min(x1.i,x2.i); x.i < max(x2.i,x1.i); x.i++) {
    ll = gfilt->locate(x);
    index = x.i + x.j*gfilt->xpoints();
    if (gfilt->grid[index] != oldland->grid[index] ) {
      printf("%6.3f %6.3f New %3d Old %3d \n",ll.lat, ll.lon, 
        gfilt->grid[index], oldland->grid[index] );
    }
  }
  }
