...
// Change to 0.5 degree rtg sst 26 April 2004
  ...
  latpt outloc;
  float flag;
  int range = 1; //min distance to land for a valid concentration
  int filled;
...
// Transfer values, cap, and then fill in gaps.
  ...
  for (ijloc.i = 0; ijloc.i < outmap.xpoints(); ijloc.i++) {
     ...
     if (outloc.lat < -20.) {
       ...
       if (sf.in(tloc) && fabs(outmap[ijloc] - 100.) > 0.01) {
         printf("%4d %4d  %7.3f %8.3f  %8.3f\n",ijloc.i, ijloc.j, 
                          outloc.lat, outloc.lon,  outmap[ijloc]);
       }
     }
     else if (outloc.lat > 20.0) {
       tloc = nf.locate(outloc);
       if (nf.in(tloc) && fabs(outmap[ijloc]  - 100.) > 0.01) {
         printf("%4d %4d  %7.3f %8.3f  %8.3f\n",ijloc.i, ijloc.j, 
                          outloc.lat, outloc.lon,  outmap[ijloc]);
       }
     }
