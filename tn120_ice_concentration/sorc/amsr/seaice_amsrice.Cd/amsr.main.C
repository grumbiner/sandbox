...
int main(int argc, char *argv[]) {
  amsr_scan_points  a_scan_points;
...
/* Begin stripping out data */
  while (!feof(fin) && (fin != (FILE *) NULL) ) {
    ...
    while (!feof(fin) ) {  
      j++;
      nread = fread(&a_scan_points, sizeof(amsr_scan_points), 1, fin);
...
/* Drop in moving to AMSR   -- newfilt(north, south);  */
  ...
  amsr_getfld(north, north_pts, &fld_n[0][0], &rfld_n[0][0], AMSR_BAR_CONC);
  amsr_getfld(south, south_pts, &fld_s[0][0], &rfld_s[0][0], AMSR_BAR_CONC);

/* Fill in pole for output purposes */
  amsr_pole_fill(&fld_n[0][0], 1);
  amsr_pole_fill(&fld_s[0][0], 2);
  fwrite(fld_n, sizeof(unsigned char), north_pts, nraw);
  fwrite(fld_s, sizeof(unsigned char), south_pts, sraw);
...
/* Expt: Write out the data count maps */
    ...
    amsr_getfld(north, north_pts, &fld_n[0][0], &rfld_n[0][0], COUNT);
    amsr_getfld(south, south_pts, &fld_s[0][0], &rfld_s[0][0], COUNT);
