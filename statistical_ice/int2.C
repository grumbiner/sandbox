...
int main(int argc, char *argv[]) {
  northgrid<float> first, last, test;
  northgrid<float> *interp[MAXGAP];
...
///// Start up:
  first.binin(fin1);
  last.binin(fin2);
  printf("first, last, max, min %f %f  %f %f\n",first.gridmax(), first.gridmin(),
        last.gridmax(), last.gridmin() );


  for (i = 0; i < length; i++) {
    interp[i] = new northgrid<float>;
...
// Interpolate
  ...
  for (x.i = 0; x.i < first.xpoints() ; x.i++) {
      if (first[x] > MAXICE && last[x] > MAXICE ) {
        ...
        for (i = 0; i < length; i++) {
           interp[i]->operator[](x) = (float) (first[x] + 
