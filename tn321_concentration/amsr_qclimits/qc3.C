...
// land summing:
    ...
    if (nobs == 2) {
      printf(" hr ");
      for (i = 0; i < nobs; i += 2) { 
        if (fabs(s[i].tmbr) < 400 && fabs(s[i+1].tmbr < 400) ) {
          pr =  (s[i+1].tmbr - s[i].tmbr) / (s[i].tmbr + s[i+1].tmbr);
          printf(" %6.2f %6.2f %f ",s[i].tmbr, s[i+1].tmbr, pr);
        }
      }
    }
    else {
      ...
      for (i = 0; i < nobs; i += 2) { 
        if (fabs(s[i].tmbr) < 400 && fabs(s[i+1].tmbr < 400) ) {
          pr   = (s[i+1].tmbr - s[i].tmbr) / (s[i].tmbr +  s[i+1].tmbr);
          gr19 = (s[i+1].tmbr - s[AMSR2_T19V].tmbr) / (s[i+1].tmbr + s[AMSR2_T19V].tmbr) ;
          gr24 = (s[i+1].tmbr - s[AMSR2_T24V].tmbr) / (s[i+1].tmbr + s[AMSR2_T24V].tmbr) ;
          printf(" %6.2f %6.2f %f %f %f ",s[i].tmbr, s[i+1].tmbr, pr, gr19, gr24);
        }
      }
    }
    printf("\n");
