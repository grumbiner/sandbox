...
!
! NAME:
!       AMSR2_89B_write_bufr
!
! PURPOSE:
!       This is a subroutine for writing the AMSR2_89B BUFR file.
!
! CATEGORY:
!       Subroutine
!
! CALLING SEQUENCE:
!       CALL AMSR2_89B_write_bufr(BUFR_FileName,BUFR_Table_FileName,ORBN,AggregateBeginningTime)
!
! INPUTS:
...
!
   ...
   SUBROUTINE AMSR2_89B_write_bufr(amsr2_1b_rec,BUFR_FileName,BUFR_Table_FileName,ORBN,AggregateBeginningTime)
...
!
   ...
   PRINT*,'Starting AMSR2_89B_write_bufr'
!
...
!
   ...
   P2D_Index=Array_Index(CrossTrack,amsr2_1b_rec%CrossTrack2,AlongTrack,amsr2_1b_rec%AlongTrack)
    IF (ABS(amsr2_1b_rec%Latitude_for_89B(P2D_Index)) < 90.0 ) MISSING_ALL=.FALSE.
    IF (ABS(amsr2_1b_rec%Longitude_for_89B(P2D_Index)) < 180.0 ) MISSING_ALL=.FALSE.
   ENDDO
   ENDDO
   IF (MISSING_ALL) THEN
    print *, 'ALL the AMSR2_89B are missing values in the input HDF5 file'
    CALL EXIT (1)
   ENDIF
...
!
   ...
   IF(Open_Status /= 0)THEN
      CALL error_messaging('AMSR2_89B_write_bufr', &
...
!  subcategory identifier to a value of 5 (CrIS), 6 (ATMS), 7 (VIIRS), 6 (SST), !  8 (AMSR2_1b propose to add microwave scanning radiometer under 21 Radiance (satellite measured). 
!
...
!
!  Loop through all the AMSR2_89B GRANUEL.
!
...
!
         ...
         ARR(15) = amsr2_1b_rec%Latitude_for_89B(P2D_Index)*amsr2_1b_rec%Latitude_89B_SF ! 005001 Latitude (high accuracy) 
         ARR(16) = amsr2_1b_rec%Longitude_for_89B(P2D_Index)*amsr2_1b_rec%Longitude_89B_SF ! 006001 Longitude (high accuracy) 
...
!       set up 89BGHz_H
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_890GHzB_H(P2D_Index)*amsr2_1b_rec%BT_890GHzB_H_SF
...
!       set up 89BGHz_V
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_890GHzB_V(P2D_Index)*amsr2_1b_rec%BT_890GHzB_V_SF

!print *, "NX=", NX, "AMSR2_89B=", ARR(26), ARR(31), ARR(36), ARR(41), ARR(46), ARR(51), ARR(56), ARR(61), ARR(66), ARR(71), ARR(76), ARR(81)
...
!
   ...
   ENDDO MISSING_CHECK ! Cycling through AMSR2_89B FOVs
  ENDDO TIME_CHECK ! Cycling through AMSR2_89B Scan 
   ...
   PRINT*,'Finishing AMSR2_89B_write_bufr'

   RETURN

   END SUBROUTINE AMSR2_89B_write_bufr
