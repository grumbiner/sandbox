...
!
! PURPOSE:
!       This is a subroutine for writing the AMSR2_1b SDR BUFR file.
!
! CATEGORY:
!       Subroutine
!
! CALLING SEQUENCE:
!       CALL AMSR2_1b_write_bufr(BUFR_FileName,BUFR_Table_FileName,ORBN,AggregateBeginningTime)
!
! INPUTS:
...
!
   ...
   SUBROUTINE AMSR2_1b_write_bufr(amsr2_1b_rec,BUFR_FileName,BUFR_Table_FileName,ORBN,AggregateBeginningTime)
...
!
   ...
   INTEGER(LONG), PARAMETER :: ARRDIM = 82 
!   INTEGER(LONG), PARAMETER :: ARRDIM = 81 
   ...
   REAL(DOUBLE) :: SCCF(8), MSecond, Sat_Height
...
!
   ...
   PRINT*,'Starting AMSR2_1b_write_bufr'
!
!  deal with all the data are missing situation
!
   MISSING_ALL=.TRUE.
   DO AlongTrack = 1,amsr2_1b_rec%AlongTrack
   DO CrossTrack=1,amsr2_1b_rec%CrossTrack1
   ODD_NUM=2*CrossTrack-1
   P2AD_Index=Array_Index(ODD_NUM,amsr2_1b_rec%CrossTrack2,AlongTrack,amsr2_1b_rec%AlongTrack)
    IF (ABS(amsr2_1b_rec%Latitude_for_89A(P2AD_Index)) < 90.0 ) MISSING_ALL=.FALSE.
    IF (ABS(amsr2_1b_rec%Longitude_for_89A(P2AD_Index)) < 180.0 ) MISSING_ALL=.FALSE.
   ENDDO
   ENDDO
   IF (MISSING_ALL) THEN
    print *, 'ALL the AMSR2_1b are missing values in the input HDF5 file'
    CALL EXIT (1)
   ENDIF
...
!
   DATA SCCF/6.9e9,7.3e9,10.7e9,18.7e9,23.8e9,36.5e9,89.0e9,89.0e9/
...
!
   ...
   IF(Open_Status /= 0)THEN
      CALL error_messaging('AMSR2_1b_write_bufr', &
...
!  subcategory identifier to a value of 5 (CrIS), 6 (ATMS), 7 (VIIRS), 6 (SST), 7 (AMSR2_1b). 
!
...
!
!  Loop through all the AMSR2_1b GRANUEL.
!
...
MISSING_CHECK: DO CrossTrack=1,amsr2_1b_rec%CrossTrack1
        P2D_Index=Array_Index(CrossTrack,amsr2_1b_rec%CrossTrack1,AlongTrack,amsr2_1b_rec%AlongTrack) 
        P2AD_Index=Array_Index(2*CrossTrack-1,amsr2_1b_rec%CrossTrack2,AlongTrack,amsr2_1b_rec%AlongTrack)
...
!
         ...
         ARR(15) = amsr2_1b_rec%Latitude_for_89A(P2AD_Index)*amsr2_1b_rec%Latitude_89A_SF ! 005001 Latitude (high accuracy) 
         ARR(16) = amsr2_1b_rec%Longitude_for_89A(P2AD_Index)*amsr2_1b_rec%Longitude_89A_SF ! 006001 Longitude (high accuracy) 
         ARR(17) = amsr2_1b_rec%Sun_Azimuth(P2D_Index)*amsr2_1b_rec%Sun_Azimuth_SF ! 005022 Solar azimuth 
         IF (amsr2_1b_rec%Sun_Azimuth(P2D_Index) <0) then
         ARR(17) = 360.0 + amsr2_1b_rec%Sun_Azimuth(P2D_Index)*amsr2_1b_rec%Sun_Azimuth_SF ! 005022 Solar azimuth 
         ENDIF 
         ARR(18) = amsr2_1b_rec%Sun_Elevation(P2D_Index)*amsr2_1b_rec%Sun_Elevation_SF ! 007022 Solar elevation
         ARR(19) = amsr2_1b_rec%Earth_Incidence(P2D_Index)*amsr2_1b_rec%Earth_Incidence_SF ! 025081 Incidence angle
         ARR(20) = amsr2_1b_rec%Earth_Azimuth(P2D_Index)*amsr2_1b_rec%Earth_Azimuth_SF ! 025082 Azimuth angle 
         IF (amsr2_1b_rec%Earth_Azimuth(P2D_Index) <0) then
         ARR(20) = 360.0 + amsr2_1b_rec%Earth_Azimuth(P2D_Index)*amsr2_1b_rec%Earth_Azimuth_SF ! 025082 Azimuth angle 
         ENDIF 
...
!       set up 69GHz_H
         ...
         CH_2D_Index=Array_Index(CrossTrack,amsr2_1b_rec%CrossTrack1,AlongTrack,amsr2_1b_rec%AlongTrack,CHNM,amsr2_1b_rec%Sector1)
         ALFR = (100.0 - amsr2_1b_rec%Land_Ocean_Flag_6_to_36(CH_2D_Index)*amsr2_1b_rec%Land_Ocean_6_SF)/100.0 ! 021166 Land fraction
         NX=NX+1
         ARR(NX)=ALFR
         POS=0
         VIIRSQ=(2**10)*IBITS(amsr2_1b_rec%Pixel_Data_Quality_6_to_36(P2D_Index),POS,1)
         NX=NX+1
         ARR(NX)=VIIRSQ
         ANPO=0
         NX=NX+1
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_69GHz_H(P2D_Index)*amsr2_1b_rec%BT_69GHz_H_SF
...
!       set up 69GHz_V
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_69GHz_V(P2D_Index)*amsr2_1b_rec%BT_69GHz_V_SF
...
!       set up 73GHz_H
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_73GHz_H(P2D_Index)*amsr2_1b_rec%BT_73GHz_H_SF
...
!       set up 73GHz_V
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_73GHz_V(P2D_Index)*amsr2_1b_rec%BT_73GHz_V_SF
...
!       set up 107GHz_H
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_107GHz_H(P2D_Index)*amsr2_1b_rec%BT_107GHz_H_SF
...
!       set up 107GHz_V
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_107GHz_V(P2D_Index)*amsr2_1b_rec%BT_107GHz_V_SF
...
!       set up 187GHz_H
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_187GHz_H(P2D_Index)*amsr2_1b_rec%BT_187GHz_H_SF
...
!       set up 187GHz_V
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_187GHz_V(P2D_Index)*amsr2_1b_rec%BT_187GHz_V_SF
...
!       set up 238GHz_H
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_238GHz_H(P2D_Index)*amsr2_1b_rec%BT_238GHz_H_SF
...
!       set up 238GHz_V
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_238GHz_V(P2D_Index)*amsr2_1b_rec%BT_238GHz_V_SF
...
!       set up 365GHz_H
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_365GHz_H(P2D_Index)*amsr2_1b_rec%BT_365GHz_H_SF
...
!       set up 365GHz_V
         ...
         ARR(NX)=ANPO
         NX=NX+1
         ARR(NX)=amsr2_1b_rec%BT_365GHz_V(P2D_Index)*amsr2_1b_rec%BT_365GHz_V_SF

!print *, "NX=", NX, "AMSR2_1b=", ARR(26), ARR(31), ARR(36), ARR(41), ARR(46), ARR(51), ARR(56), ARR(61), ARR(66), ARR(71), ARR(76), ARR(81)
...
!
   ...
   ENDDO MISSING_CHECK ! Cycling through AMSR2_1b FOVs
  ENDDO TIME_CHECK ! Cycling through AMSR2_1b Scan 
   ...
   PRINT*,'Finishing AMSR2_1b_write_bufr'

   RETURN

   END SUBROUTINE AMSR2_1b_write_bufr
