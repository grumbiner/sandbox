...
for k in range(1,8):
  if (k == 1):
    ...
    for i in range(0, len(tbfilters)):
      rerun_bayes(lrmatch, stats, landmask, icemask, watermask, known, tbfilters[i])
  ...
  for i in range(0, len(tbfilters)):
      ...
      if (tbfilters[i].stats[0] == 0. and tbfilters[i].npts > 0 ):
...
for k in range(1,4):
  ...
  for i in range(0, len(tbfilters)):
      tbfilters[i].show(foute)
      if (tbfilters[i].stats[0] <= base_rate/10. ):
        perfect_filters.append(tbfilters[i])
        tbfilters[i].show(fout)
        perfect += 1
  fout.close()
