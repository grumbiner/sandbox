...
#endif
void mapxy_old(float *alat, float *along, int i, int j, const float xorig,
...
/* filtbg.c : routine for old-style mapping of ice concentrations onto */
/*   lat-long grids.  Retained for UKMO, Nick Rayner originally  */
/* Robert Grumbine                                               */
/* First version in place by early 1994                          */
/*   -- that version contained an erroneous radius of the earth, */
/*      6738 vs 6378 km, fixed ca. 9/15/1994    */
/* Oldest file-dated version on hand 5 Apr 1996 */
/* 11 Dec 1996: Fix subtle error in mapping function per NSIDC note */
/* 27 Aug 1997: Test for existence of input files, improve code typography
                  and variable names           */
/* 29 Apr 2004: Add an explicit 'min' function */
/*  7 Jul 2006: Remove unused variables, cleanly convert float -> int, char */
...
int main(int argc, char *argv[])
{
  ...
  int in_nx = innx, in_ny = inny;

  xrat = (int) (0.5 + indx / outdx);
  yrat = (int) (0.5 + indy / outdy);
  ...
  #ifdef OPERATIONS
    j = DEGRIB(&in_nx, &in_ny, &sst[0][0]);
    if (j != 0) { fprintf(stderr,"Failed to degrib sst file \n"); return j; }
  #else
    in12 = fopen(argv[1],"r");
    if (in12 == NULL) { printf("failed to open - global sst file\n");}
    fread(sst,  sizeof(float),         in_nx*in_ny,   in12);
  /*  fread(errs, sizeof(float),         in_nx*in_ny,   in12);
...
/* Now average from polar stereographic on to the lat-long grid */
  for (j = 0; j < nny; j++) {
    for (i = 0; i < nnx; i++) {
       mapxy_old(&llat, &llon, i, j, -npolei*dx, -npolej*dy, dx, dy, 
              slat, nslon, 1.0, rearth, eccen2); 

       lli = (int) (llon / outdx + 0.5); 
       if (lli < 0. ) lli += (int) (0.5 + 360./outdx);
  ...
  for (j = 0; j < sny; j++) {
    for (i = 0; i < snx; i++) {
       mapxy_old(&llat, &llon, i, j, -spolei*dx, -spolej*dy, dx, dy, 
              slat, sslon, -1.0, rearth, eccen2); 

       lli = (int) (llon / outdx + 0.5);
       if (lli < 0. ) lli += (int) (360./outdx + 0.5);
       llj = (int) ((90. - llat) / outdy  - 0.5);
       if ( smap[j][i] < 126 ) {
         count[llj][lli] += 1;
...
/*              printf("zeroing %d %d %f %f\n" ,lli, llj, sst[llj][lli], 
            ...
            cout2[llj][lli] = (unsigned char) (0.5 + outmap[llj][lli]);
          }
        }
/* end SST filtering */
