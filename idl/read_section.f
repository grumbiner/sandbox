...
!-----------------------------------------------------------
      ...
      real, allocatable::depth1(:,:),depth2(:,:) 
!------------------------------------------------------------
...
!
   96 format(5x,'im = ',i5,' jm = ',i5) 
      ...
      enddo
      enddo 
! end of normal sectioning 
!-----------------------------------------------------------------
      else
! special processing, domain crosses International Date Line 
      jskip=30*( 90-nlat) 
      jm=(nlat-slat)*30.0+1
      iskip=30*(180+wlon)
      i1=(180.-wlon)*30.
      ...
      write(6,96) im,jm    
      allocate (depth(im,jm),alon(im,jm),alat(im,jm)) 
      allocate (depth1(i1,jm),depth2(i2,jm)) 
! section out part in Eastern Hemisphere 
      do j=1,jm
      do i=1,i1
      depth1(i,j)=idepth(iskip+i,jskip+jm+1-j) 
      depth(i,j)= depth1(i,j)
      enddo
      enddo 
! section out part in Western Hemisphere
      do j=1,jm
      do i=1,i2
      depth2(i,j)=idepth(i,jskip+jm+1-j) 
      depth(i1+i,j)=depth2(i,j)
      enddo
      enddo 
!
! end of special sectioning 
      endif 
!--------------------------------------------------------------
