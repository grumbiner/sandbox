...
// Added 29 April 2008 for scoring
  ...
  char forecast_base[900];
  strncpy(forecast_base, argv[7], 900);
...
//Note that tm_mon is 0-11, not 1-12
  ...
  start_time.tm_mday = 1; start_time.tm_mon = 6; start_time.tm_year = 98;
  end_time.tm_mday = 31;  end_time.tm_mon = 12;   end_time.tm_year = 108;
  mktime(&start_time);
    ...
    for (i = curno+1; i < ntrimmed; i++) {
      ...
      if (date != datelast) {
        ...
        if (tmpfin == (FILE *) NULL) {
          ...
          //ftn getfcst(&date, &funit, &dir[0][0], &dist[0][0], &retcode);
      ...
      if (abs(retcode) >= t1) { // The fabs is so that incomplete forecasts 
        ...
        // Observed direction of drift:
           ...
           //printf("%2d %2d %2d  %3d %2d  %5.2f %6.2f to %5.2f %6.2f  %5.1f %5.1f  %5.1f %5.1f %s\n",
